...
> **현재 버전**: **v2.18.7 (HOTFIX-10 후면 카메라 강제, 2026-05-21)** — FE 1 파일(qr_scanner_web.dart) / 사용자 catch (v2.18.6 직후): "카메라 방향이 셀카인데 전환 버튼 돌려달라" — 모바일에서 `facingMode:'environment'` hint 영역 OS 무시 → user silent fallback → 셀카 / fix: `environment` 영역 `{exact:...}` 강제 + 3차 cameraId fallback 영역 'back'/'rear'/'후면' label 매칭 안전망 추가 / 데스크톱은 자동 user fallback (1차 fail 후) / flutter build web GREEN / LOC 533→551 🟡 / **별 sprint**: `BUG-42-CAMERA-SWITCH-BUTTON-DEFERRED` BACKLOG 등록(전환 버튼 UI) / **실기기 manual QA 재위탁**
