...
> **현재 버전**: **v2.46.0 (Sprint 95 / FEAT-SHIPMENT-DAILY-COMPLETED-LIST — 출하 일일 알림 완료 리스트 추가, 2026-06-15)** — BE only minor, additive, migration 0. 매일 07:30 출하 미처리 메일에 "어제 출하 완료" 리스트 추가(출하 현황 종합). 신규 get_completed_shipments(actual_date=어제). ⚠️ Codex M-Q4 동반 버그 fix: get_overdue/completed 둘 다 serial NOT LIKE TEST% 보강(미처리 메일 TEST-2221~6 섞임 fix). send completed_items 하위호환+완료 섹션. Codex 2R DEPLOY_SAFE M=0. pytest 6/6+회귀 21. **⏭️ 다음**: VIEW FE 연동 / BACKLOG advisory 등록(메일 row 상한 등) / request 검토.
> **선행 release**: **v2.45.0 (FIX-ZEROTAP-AUTOCLOSE-SEPARATION — 0초탭/자동마감 분리 4개 지표 통일, 2026-06-15)** — BE only minor, read-only, additive, migration 0. "0초탭"이 자동마감(close_reason)을 섞어 4배 부풀림(623 vs 진짜 149, 사용자 catch). 진짜 0초탭=close NULL AND active≤1 로 4개 지표(tagging-coverage #94/close-type-trend/data-quality/partner_discipline) 통일. tagging은 instant_* 신규+zero_* deprecated, 나머지 zerotap 값 정정. partner_discipline 분모+분자 close NULL(Codex M-1). _TRACKED/_COVERAGE 불변→reliability/partner-reliability 영향 0. Codex 3R GO. pytest 영향 77+회귀 29 GREEN. **⏭️ 다음**: VIEW #94 연동(instant_* 사용·zero_* fallback 금지) / request 추가 검토 / VIEW FE 연동.
> **선행 release**: **v2.44.0 (Sprint 94-b / FEAT-PARTNER-RELIABILITY-MONTHLY — 매트릭스 월별/필터 + 추이 분리, 2026-06-15)** — BE only minor, read-only, additive, migration 0. 매트릭스 기본 누적→교육 타겟 "이번달" 월별 파악 위해 공통 필터(오늘/주간/월간/분기+협력사) 매트릭스 적용 + 추이 시계열 분리. `?period=&reference_date=&partner=&process=&model=` (미지정=현행 누적 back-compat). 매트릭스=period 윈도우+partner/process/model / trend=trust_start~현재월 월별·process/model 적용·period/partner 제외(사용자 결정). Codex M-1(day 못 자름) 해소=매트릭스 day SQL+trend 월별 SQL 분리. 기획 독립판정=월별 타당. Codex 4R DEPLOY_SAFE M=0. 실데이터 period=month BAT·GAIA·MECH 13.0%(단월). pytest 13/13. **⏭️ 다음**: ① 0초탭 정의 정정(보류 — VIEW request 연계, active≤1 OR close_reason 합집합이라 자동마감 135 중복) ② request 추가 내용 검토 ③ VIEW FE 연동.
> **선행 release**: **v2.43.0 (Sprint 94 / FEAT-PARTNER-RELIABILITY / #93 — 협력사×모델×공정 추적률 분해, 2026-06-15)** — BE only minor, read-only, 신규 endpoint, migration 0. reliability-summary(모델 합산)를 협력사로 분해 → "표준 가능=FNI / 교육 타겟=BAT" 행동 가능. 신규 `GET /api/ct/partner-reliability`: by_cell(협력사×모델×공정)+by_model_process(invariant anchor+batch_n)+by_partner+trend(batch omit)+meta. MECH/ELEC only, GST·SH 제외, batch(TMS(M)) 일괄, invariant 성립(by_cell n 가중합=by_model_process). 실데이터 BAT·GAIA·MECH 17.2%(교육) vs FNI 65.2%. Codex 2R(CONDITIONAL_GO→DEPLOY_SAFE M=0). pytest 8/8+회귀 24 passed 1 skip. **⏭️ 다음**: VIEW FE(별 repo): #93 usePartnerReliability→PartnerReliabilityCard / #92 필터 / reliability 게이트(v2.41.0) 연동.
> **선행 release**: **v2.42.1 (FIX-INSTANT-WHITELIST-SELF-INSPECTION-REMOVE — one-click whitelist 자주검사 2개 제거, 2026-06-15)** — BE only patch, read-only, 상수 정정, migration 0. 사용자 catch: one-click은 tank docking·si_shipment 2개. 자주검사(SELF_INSPECTION 평균57분·INSPECTION 98분)는 실작업인데 one-click 오분류 → 추적률 과소. `_INSTANT_WHITELIST` 2개로(자주검사 제거), 6지표 미러 자동 전파. Codex Q5 동반: data-quality trend_sql zerotap 하드코딩 → 동적 미러 복원. 효과: tagging-coverage MECH 34→49%·ELEC 47→58%, reliability 헤드라인 53→63%. Codex 2R(NO-GO Q5/Q3/Q6→DEPLOY_SAFE M=0). pytest 6지표 회귀 93 passed 1 skip. **⏭️ 다음**: #93(협력사×모델×공정 추적률 분해 — 정정된 추적률 base 위 설계, A-2 분모 one-click 제외 유지) / VIEW FE(#92 필터 + reliability 게이트 연동).
> **선행 release**: **v2.42.0 (Sprint 93 / FEAT-TAGGING-COVERAGE-FILTERS / #92 — tagging-coverage period/reference_date/partner 필터, 2026-06-15)** — BE only minor, read-only, additive, migration 0. 종료 누락 페이지 0초탭 카드만 월 단위 from/to라 필터 불일치 → `GET /api/ct/tagging-coverage?period=&reference_date=&partner=` 추가(미지정=현행 back-compat). period(day 윈도우, `_resolve_period_range` 재사용)+partner(협력사 필터, 분모 협력사 기준, PI/QI/SI=GST). 공유 상수 회귀 방지=`_COVERAGE_WHERE` byte 동일 유지(reliability-summary v2.41.0 무변경). Codex R1 GO M=0. 실데이터 partner=FNI MECH만 / reliability tracking 53·standard_ready 17.6 동일(회귀 0). pytest 7+회귀 32 passed 1 skip. **⏭️ 다음**: VIEW FE(별 repo, useTaggingCoverage 필터 연동) / CT 신뢰도 게이트 VIEW 연동(v2.41.0) / #87 Phase 2b.
> **선행 release**: **v2.41.0 (Sprint 92 / FEAT-CT-RELIABILITY-SUMMARY / B — CT 데이터 신뢰도 게이트 재설계, 2026-06-14)** — BE only minor, read-only, 신규 endpoint, migration 0. CT "데이터 신뢰도" summary 가 FE 파생 가중평균(풀링 부풀림 80.6%)이라 과대평가 → **count 게이트+생산량 가중**으로 재설계(3모델 합의+사용자 catch 누적). 신규 `GET /api/ct/reliability-summary`: 표준가능=셀(모델×task×dual) 중 (n≥30 AND 모델공정추적≥70%) 비율, 생산량 가중 Σ(모델 12mo 생산비중·표준가능비율)(주력 GAIA 88% 지배=정직), 2조건 분리(n_met/tracking_met), 모델 기준, 라벨 정정, 헤드라인=추적율. Codex 3R(NO-GO→CONDITIONAL_GO Q2/Q4/Q5→DEPLOY_SAFE M=0)+Fable 2차. 실데이터 추적53%/standard_ready17.6%/병목=추적(tracking_met22.5%). pytest 9+1skip+회귀 44+1skip. **⏭️ 다음**: VIEW FE(별 repo, VIEW_FE_Request.md 인계 — 가중평균 제거→endpoint 소비, 드릴 계층).
