...
> **현재 버전**: **v2.38.1 (FEAT-OPS-FE-PENDING-GUARD — OPS app 미종료 가드 FE, 2026-06-13)** — FE only patch(3화면). A 퇴근 다이얼로그(open_tasks → [모두 일시정지] idempotent pause/[나중에]) / B 방치 뱃지(무활동 N시간+⏸ metadata Row) / C 종료 다이얼로그 무활동 문구. Codex R1 정정 반영. build GREEN + Netlify prod 배포. **⏭️ 다음**: 실기기 QA / VIEW B5 / #87 Phase 2b.
