...
> **현재 버전**: **v2.36.1 (FIX-CT-FORCE-CLOSE-POLLUTION — CT 표준 강제종료 오염 정정, 2026-06-12)** — BE only patch, read-only, migration 0. **버그**: 강제종료(`force_close_task`)가 v2.22.0/v2.28.0에서 `compute_task_work`로 ct/duration 계산·저장하나 `duration_source` 미설정(NULL) → CT clean 필터 통과 → CT 표준 표본 오염. 클린 코어 원칙(2026-04-20 "강제종료=버리는 데이터")과 drift. **실측(2026-05~, 1,244건)**: 강제종료 128건(10.3%), MECH 배관 median 왜곡(WASTE_GAS_LINE_1 1.40h→2.98h +112%). **Fix**: `_CLEAN_CORE`에 `force_closed=FALSE` 1줄(→ task-stats #82/partner-breakdown #83/training 전파). **자동마감 유지**: AUTO_CLOSED_BY_* 65건 work_completion_log 전건 보유=작업자완료라 제외 X(Codex Q3 실데이터 0건). Codex R1 NO-GO→실데이터 해소→**R2 DEPLOY_SAFE M=0**. pytest 신규 test_ct17 + CT회귀 18/18 GREEN. read-only·저장값 무변경(man-hour 리포트 영향 0). VIEW §8-7 #1 정정. **⏭️ 다음**: VIEW CT/종료누락 페이지 표준값 정정 자동 반영 / #87 Phase 2b(checklist).
