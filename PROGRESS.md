...
> **현재 버전**: **v2.30.0 (Sprint S-2 ⓑ 진짜 CT across-worker union + 동시작업자 — VIEW #82 ⓑ, 2026-06-08)** — BE minor (migration 061 ct_time_minutes 컬럼+백필 + task_detail/admin/statistics_service/ct_analysis). 현 active/duration 둘 다 M/H(across-worker SUM)이지 CT 아님 → 진짜 CT = 작업자 정제세션 across-worker **UNION**(벽시계). ① 검증: 다중작업자 ≠ 병렬(세션 겹침=병렬만 CT<M/H), PANEL 4명동시 2.0x/UTIL 릴레이 1.0x, 전체 ~9% 병렬. migration 061 `(session∩BH−pause∪식사)` per-worker → range_agg union → LEAST(union, active)(CT≤M/H), active=0→ct=0/active NULL→ct NULL 멱등. compute_task_work {manhour,active,ct} + 완료 4경로 동봉. `basis=ct` + effective_concurrency_median + coverage(active,ct) + n_used(Tukey전)/n_sample(Tukey후). Codex 설계 R1 M=3→R2 M=1→R3 GO / 구현 R1 M=2→fix→**DEPLOY_SAFE M=0**. pytest 신규 15+회귀 67 GREEN. additive(man-hour/active/duration 불변). 배포검증 백필 갭 0·CT>active 위반 0. **⏭️ 다음**: VIEW FE basis=ct 연동 / **S-3 ⓒ DAG garbage 제외 + 무결성 KPI**.
