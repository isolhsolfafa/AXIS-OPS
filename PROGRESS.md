...
> **현재 버전**: **v2.28.1 (CT task-stats dual 파라미터 — DUAL/단일 분리, VIEW #81, 2026-06-07)** — CT ②가 DUAL/단일 합산해 표준 왜곡(PANEL 단일 5.6h vs DUAL 11.8h) → `?dual=dual\|single` 추가. `_dual_clause` = model ILIKE '%DUAL%'(포함, iVAS 중간DUAL 엣지 커버). meta dual_scope, 스키마 불변(VIEW v1.59.x 선반영). BE 2파일, DB 0. pytest CT-16/CT-14 GREEN.
