...
> **현재 버전**: **v2.42.1 (FIX-INSTANT-WHITELIST-SELF-INSPECTION-REMOVE — one-click whitelist 자주검사 2개 제거, 2026-06-15)** — BE only patch, read-only, 상수 정정, migration 0. 사용자 catch: one-click은 tank docking·si_shipment 2개. 자주검사(SELF_INSPECTION 평균57분·INSPECTION 98분)는 실작업인데 one-click 오분류 → 추적률 과소. `_INSTANT_WHITELIST` 2개로(자주검사 제거), 6지표 미러 자동 전파. Codex Q5 동반: data-quality trend_sql zerotap 하드코딩 → 동적 미러 복원. 효과: tagging-coverage MECH 34→49%·ELEC 47→58%, reliability 헤드라인 53→63%. Codex 2R(NO-GO Q5/Q3/Q6→DEPLOY_SAFE M=0). pytest 6지표 회귀 93 passed 1 skip. **⏭️ 다음**: #93(협력사×모델×공정 추적률 분해 — 정정된 추적률 base 위 설계, A-2 분모 one-click 제외 유지) / VIEW FE(#92 필터 + reliability 게이트 연동).
