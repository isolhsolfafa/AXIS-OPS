...
> **현재 버전**: **v2.18.6 (HOTFIX-09 cameraIdOrConfig 1-key 위반 fix, 2026-05-21)** — FE 1 파일(qr_scanner_web.dart) / v2.18.5 배포 직후 사용자 실기기 catch: `'cameraIdOrConfig' object should have exactly 1 key, found 4 keys` — html5-qrcode `start()` 1번째 인자 1-key 제약 위반 → 3시도 모두 reject → "카메라 없음" 에러 / fix: helper 1-key 객체만 반환 + `__qrScanConfig.videoConstraints` 영역으로 해상도/focusMode 이동 / flutter build web GREEN / LOC 530→533 🟡 / Codex 라운드 1 (v2.18.5) 놓친 라이브러리 spec catch / **실기기 manual QA 재위탁**
