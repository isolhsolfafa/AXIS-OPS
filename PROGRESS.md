...
> **현재 버전**: **v2.39.1 (Sprint 91-FE-b — 미종료 큐 방치 기준 통일 BE, 2026-06-13)** — BE only additive. `/open-tasks` 에 inactive_hours+meta.by_partner(칩)+meta.buckets+basis='realtime_abandoned' 추가 → VIEW 미종료 카드 근사(254)→방치(127) 통일 계약. Codex R2 DEPLOY_SAFE M=0. pytest 34/34. **⏭️ 다음**: VIEW MissedTaskQueueCard progress→/open-tasks 교체(별 repo) / #87 Phase 2b / 설계서 분할.
