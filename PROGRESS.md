...
> **현재 버전**: **v2.18.3 (SEC-CONFIG-DATABASE-URL-FALLBACK-REMOVED + GCP migration HOLD, 2026-05-20)** — BE 1 파일(config.py) / Railway DB URL 하드코딩 fallback 제거 + GCP migration 준비 100% 완료 후 비용 협의로 hold / GCP standby: 월 ~$20 (storage만) / 재오픈 30분 절차는 `GCP_MIGRATION_STANDBY.md` 참조 / Railway prod 운영 그대로 (Auto Deploys ON 복원, 사용자 150명)
