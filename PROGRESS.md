...
> **현재 버전**: **v2.29.0 (Sprint S-1 ⓐ CT basis=active + 미추적 제외 + 신뢰컷오프 + 월범위 — VIEW #82 ⓐ, 2026-06-08)** — BE only minor (statistics_service.py + ct_analysis.py, DB/migration 0 read-time). 14개 운영 쿼리 검증: 현 "CT"=M/H, **act=0=미추적**(시작=완료 즉시태깅, MECH 배관 ~50%), act=0 제외 시 MECH CT 0.5h→2.2h, 미추적율 BAT 59% vs FNI 7%(완료율이 숨김), duration 058 미적용→6월+만 신뢰. `GET /api/ct/task-stats` 신규 `basis`(duration기본\|active) + `from`/`to`(YYYY-MM KST 반열림) + period 제거. basis=active→act>0 미추적 제외, 신뢰 기준 `CT_TRUST_START_MONTH='2026-05'` 단일, 생존편향 `tracking_coverage_by_partner`(FNI 0.71/BAT 0.34), basis별 n + provisional(n<30). Codex 설계 R1 M=4→R2 CONDITIONAL_GO→구현 R1 M=2→**재검증 DEPLOY_SAFE M=0**. pytest 신규 18+회귀 sprint85 16 GREEN. 회귀 0(additive). 분리: ⓑ CT union=S-2(새 컬럼+migration)/ⓒ DAG+무결성KPI=S-3. **⏭️ 다음**: VIEW FE basis=active 연동 / S-2 / S-3.
