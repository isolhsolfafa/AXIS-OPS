...
> **현재 버전**: **v2.18.18 (인증 메일 도메인 안내 + whitelist 검증, 2026-05-22)** — BE 1 + FE 2 파일 / 사용자 catch (카카오/다음 가입 시 인증메일 미수신, KT Biz Office SMTP 외부 도메인 차단) / `ALLOWED_EMAIL_DOMAINS = {gst-in.com, naver.com, gmail.com}` BE/FE 동기화 + register_screen 안내 메시지 / `EMAIL_DOMAIN_NOT_ALLOWED` 에러 응답 / CLAUDE.md L626 문서와 코드 일치
