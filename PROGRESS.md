...
> **현재 버전**: **v2.28.0 (Sprint 86 순수 작업시간 active-time + CT 표준 격상 — FEAT-ACTIVE-TIME-PURE-WORK, 2026-06-07)** — man-hour는 세션−수동pause라 휴게(160분/일)·영업시간 밖 idle 미차감 → active-time = 세션∩영업창 − (수동pause∪휴게)∩... 순수 작업시간. 영업창=attendance 우선/fallback 평일[08,20]·주말[08,17]. 신규 `compute_task_work`(man-hour+active 단일 CTE) + 기존 compute_task_manhour 위임 + migration 059(active_time_minutes 컬럼+백필) + 완료 4경로 active 동봉 + statistics_service active_*_hours. **man-hour(duration_minutes) 불변(additive)**. Codex 6라운드(설계 R5 GO / 구현 R6 M-1 force_complete·M-2 active_available 해소). 운영 검증 active 중앙 PANEL 8.9→6.1h. pytest active 12 + 회귀 relay 38·CT 15 GREEN. VIEW FE 후속 별 세션.
