...
> **현재 버전**: **v2.21.2 (위치 검증 토글 Off 시 출근 GPS 권한 요청 제거, 2026-05-30)** — admin `geo_check_enabled` Off 인데도 FE 가 무조건 GPS 호출 → BE `/hr/attendance/today` 응답에 `geo_check_enabled` additive + FE `(_geoCheckEnabled && work_site=='GST')` 일 때만 GPS (BE hr.py:132 정합). `_attendanceStatusLoaded` 게이트 (Codex M-Q2). Codex R1 M=1 반영.
