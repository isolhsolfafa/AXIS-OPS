...
> **현재 버전**: **v2.17.0 (Sprint 68: 출하 완료 ship-complete endpoint, 2026-05-19)** — BE 신규 2 파일(shipment_service.py + work_shipment.py) + __init__.py 1줄 + pytest 1 / `POST /api/app/work/ship-complete` — SI task 2개(SI_FINISHING+SI_SHIPMENT) 완료 + completed_at 지정 / admin·manager 대행 / audit close_reason='SHIP_COMPLETE'+closed_by, force_closed=FALSE / Codex 라운드 1 M=5/A=1 전건 반영 / pytest test_ship_complete 12/12 GREEN / migration 불필요 / OPS FE·VIEW Sprint 47 후속
