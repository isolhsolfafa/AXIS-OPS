...
## Sprint 52 BUG-FIX: FE 키 불일치 + checklistReady 자동 전환 (2026-04-02)

- `tm_checklist_screen.dart` — group→group_name, check_name→item_name, id→master_id (7곳)
- `task_service.dart` — completeTask 리턴에 checklistReady 추가
- `task_provider.dart` — 리턴 타입 ({bool success, bool checklistReady})
- `task_detail_screen.dart` — checklistReady=true → TmChecklistScreen push
- `task_management_screen.dart` — 동일 + result 변수 충돌 수정
...
| Sprint 52 | test_sprint52_tm_checklist | 39 | 39/39 ✅ |
| **합계** | **신규 131건 + 수동 21건** | | **전체 PASSED** |
