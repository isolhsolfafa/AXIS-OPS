...
**목표**: HR 테이블 보호를 위한 DB 백업 정책 수립 + PM Role push

**배경**:
- Railway 코드 배포 시 DB 데이터는 변경되지 않음 (코드와 DB 분리)
- 단, ALTER TABLE 등 스키마 변경을 직접 실행하면 영향 있음
- **HR 테이블(workers, partner_attendance, qr_registry)은 운영 데이터 — 절대 손실 불가**
- PM Role 코드 완료 상태, Railway DB ALTER (role enum 추가) 후 push 필요

**Teammate**: 불필요 (운영 절차 + DB 작업)

#### Phase A: DB 백업 정책

**구현 내용**:
1. HR 관련 테이블 (보호 대상):
   - `public.workers` — 근무자 정보, 계정
   - `public.partner_attendance` — 출퇴근 기록
   - `public.qr_registry` — QR 매핑 (status 포함)
2. plan 관련 테이블 (ETL 재적재 가능):
   - `plan.product_info` — ETL로 재적재 가능, 백업 우선순위 낮음
3. 운영 규칙:
   - ALTER TABLE 실행 전 반드시 `pg_dump` 백업
   - HR 테이블 관련 마이그레이션은 별도 검증 후 실행
   - ETL은 plan 스키마만 접근 — HR 테이블 접근 금지

#### Phase B: PM Role Push

**구현 내용**:
1. Railway DB에서 role enum에 'pm' 값 추가:
   ```sql
   ALTER TYPE worker_role ADD VALUE IF NOT EXISTS 'pm';
   ```
2. PM Role 관련 코드 push (이미 완료된 코드)
3. Admin UI에서 PM 역할 할당 가능하도록 확인

**체크리스트**:
- [ ] pg_dump 백업 스크립트 준비
- [ ] ALTER TABLE 전 백업 절차 문서화 (BACKLOG.md)
- [ ] Railway DB role enum에 'pm' 추가
- [ ] PM Role 코드 push
- [ ] 기존 기능 회귀 테스트
