...
### 리드 에이전트 (Lead — 설계 조율)
- **모델**: 하이브리드 — **기본 `claude-opus-4-8`(Opus 최상위) / 복잡 판단만 `claude-fable-5`(Mythos-class, Opus 상위 / 2026-06-09 출시)** ⚠️ 신 모델 출시 시 갱신
  - 직전 고착: `claude-opus-4-7` (~2026-06-09) — Opus 4.8 출시 후에도 한 단계 낮은 모델 운영했던 문제. 기본을 4.8로 정정
  - **Fable 5 승격 트리거** (1개+ 해당 시): ① 아키텍처 전환·3페이지+ cross-cutting 설계 ② God Component 분할 등 복잡 리팩토링 설계 ③ BE 응답 계약·데이터 모델 모호한 설계 판단 ④ 회귀 리스크 큰 root-cause 디버깅(Opus로 막힐 때) ⑤ 다중 제약 trade-off 의사결정 ⑥ Codex 합의 실패 최종 판정 설계
  - 일반 Sprint 설계·리뷰·문서·단순 버그픽스 = Opus 4.8로 충분
  - ⚠️ Fable 비용 = $10/M in · $50/M out (Opus 4.8 약 2배) → 복잡 판단에만 한시 투입(상시 X)
  - ⚠️ Fable 고위험 쿼리(사이버보안·생물/화학·distillation)는 자동 Opus 4.8 fallback (세션 ~5% 미만) — GST 업무 영향 거의 없음
...
- **모델**: Sonnet 최상위 — **2026-06-10 현재 `claude-sonnet-4-6`** (기본) ⚠️ 신 모델 출시 시 갱신
  - ⚠️ Opus 티어 승격 시 **`claude-opus-4-8`** 사용 (4.7 = 4.8에 밀린 구버전, 고착 금지)
  - **하이브리드 원칙**: 일반 기능 Sprint = Sonnet 4.6 (처리량·비용 균형) / **복잡 리팩토링·회귀 리스크 큰 Sprint(God Component 분할 등)만 한시적으로 Opus 4.8** 워커
- **역할**: FE, BE, TEST 각각 담당 영역의 코드 구현
- **모드**: 사용자 승인 후 코드 수정 가능 (위임 모드)

### ⚠️ 모델 버전 관리 규칙 (2026-04-21 추가, 2026-04-24 Codex 섹션 보강)

#### Claude 모델 (리드·워커 모두 하이브리드, 2026-06-10 갱신)

- **원칙**: 양 티어 모두 **기본 모델 + 복잡 시 한시 승격** 하이브리드 (Fable 2배 단가 → 상시 사용 X)
  - **리드**: 기본 `claude-opus-4-8`(Opus 최상위) / 복잡 설계 판단만 `claude-fable-5` 승격 (트리거 6종 → 리드 섹션 참조)
  - **워커**: 기본 `claude-sonnet-4-6`(Sonnet 최상위) / 복잡 리팩토링·회귀 큰 Sprint만 `claude-opus-4-8` 승격
  - ⚠️ Opus 승격 시 **항상 4.8** (`claude-opus-4-8`) — 4.7 고착 금지 (2026-06-09 리드 4.7 고착 사례)
...
| v2.39.1 | 2026-06-13 | Sprint 91-FE-b — 미종료 큐 방치 기준 통일 BE | **BE only patch — additive, migration 0**. `discipline /open-tasks`(build_open_tasks) additive: inactive_hours(LAST_ACTIVITY_SQL, 음수방어) + meta.by_partner(칩) + meta.buckets{h24_48,h48plus} + basis='realtime_abandoned'. VIEW MissedTaskQueueCard 근사(progress 제품×공정 254)→방치(task 127) 통일용 계약. tasks[] 불변. Codex R2 DEPLOY_SAFE M=0. pytest 34/34. VIEW 후속=카드 데이터소스 교체(별 repo). |
| v2.46.0 | 2026-06-15 | Sprint 95 / FEAT-SHIPMENT-DAILY-COMPLETED-LIST — 출하 일일 알림에 어제 출하 완료 리스트 추가 | **BE only minor — additive, migration 0**. 매일 07:30 KST 출하 미처리 알림 메일(Sprint 79)이 "못 나간 것"만 발송 → 사용자 제안으로 "어제 실제 출하 완료" 리스트 추가(출하 현황 종합). **신규 `get_completed_shipments(yesterday)`**(actual_date=어제, `_get_actual_date_subquery` 재사용, ship_plan_date 무관). **⚠️ Codex M-Q4 동반 버그 fix**: 기존 `get_overdue_shipments`(미처리 메일)가 customer만 필터해 **TEST S/N 섞여 발송 중**(TEST-2221~6) → 두 함수 `serial NOT LIKE 'TEST%'` 보강. `send_shipment_overdue_alert(completed_items=None)` 하위호환+`_render_completed_section`(0건 health check). subject 기존 prefix(⚠️/✅) 유지+완료 정보. scheduler 완료 조회+전달. Codex 2R(NO-GO M-Q4→DEPLOY_SAFE M=0). pytest 6/6+회귀 21. 다음 07:30부터 완료 리스트 포함. 후속 BACKLOG SHIPMENT-MAIL-ROW-CAP. 설계: `AGENT_TEAM_LAUNCH.md` § Sprint 95. |
| v2.45.0 | 2026-06-15 | FIX-ZEROTAP-AUTOCLOSE-SEPARATION — 0초탭/자동마감 분리 4개 지표 통일 | **BE only minor — read-only, additive, migration 0**. "0초탭(zerotap)"이 자동마감(close_reason)을 섞어 4배 부풀림(623 vs 진짜 149, 사용자 catch). 전수: 4개 중 3개 `active≤1 OR close_reason` 자동마감 76% 섞임. 자동마감 active=평균 8h 방치라 0초탭 판정 불가. **컨셉**: instant(진짜 0초탭)=`NOT oneClick AND close_reason IS NULL AND active≤1` / autoclose=`close NOT NULL`(별, reserved). **tagging-coverage(#94 additive)**: `_INSTANT_SQL`/`_CLOSED_SQL`/`_AUTOCLOSE_SQL` 신규, instant_pct(분모=closed_n, =0→null)+instant_n/closed_n/autoclose_n, partners=instant 기준, zero_* DEPRECATED 병기, meta.coverage_partition. **close-type-trend/data-quality**: zerotap→close NULL AND active≤1(auto와 배타, 474 겹침 제거). **partner_discipline**: 분모+분자 둘 다 close NULL(Codex M-1 희석 해소). 영향 격리: `_TRACKED_SQL`/`_COVERAGE_BASE` 불변→reliability-summary/partner-reliability 영향 0. **Codex 3R**: 컨셉 R1 NO-GO(M-1)→R2 GO / 구현 R3 NO-GO(meta.note)→GO. test_ct12/ctt01 zerotap 2→1 + 영향 77 GREEN + 회귀 29 passed 1 skip. 실데이터 tagging instant MECH34%/ELEC7%·autoclose ELEC358 / data-quality 206→67 / close-type 35. **VIEW(#94)**: instant_* 사용, zero_* fallback 금지. 설계: `AGENT_TEAM_LAUNCH.md` § FIX-ZEROTAP-AUTOCLOSE-SEPARATION. |
| v2.44.0 | 2026-06-15 | Sprint 94-b / FEAT-PARTNER-RELIABILITY-MONTHLY (#93b) — 매트릭스 월별/필터 + 추이 분리 | **BE only minor — read-only, additive, migration 0**. v2.43.0 매트릭스 기본 누적인데 교육/개선 타겟은 "이번달 누가 garbage" 월별로 봐야(사용자 catch). 종료 누락 분석 공통 필터(오늘/주간/월간/분기+협력사)를 매트릭스도 받게 + 추이 시계열 분리. **`?period=today\|week\|month\|quarter&reference_date=&partner=&process=&model=&from=&to=`**(미지정=누적 back-compat). 매트릭스(by_cell/by_model_process/by_partner)=period 윈도우(`_resolve_period_range` #92 정합)+partner/process/model 필터. trend=trust_start~현재월 월별·process/model 적용·period/partner 제외(사용자 결정). **Codex M-1 해소**: SQL month 집계라 day period 못 자름 → 매트릭스 day SQL+trend 월별 SQL 분리(2 쿼리). batch(TMS(M)) 매트릭스 합산포함+batch_n·trend omit, confidence, cache 분리, route 검증. **Codex 4R**: 기획 독립판정=월별 타당 → R1 NO-GO(M-1)→R3/R4 DEPLOY_SAFE M=0. 실데이터 period=month BAT·GAIA·MECH 13.0%(단월)·trend 누적·partner 필터·invariant. pytest 13/13. **VIEW 후속**: usePartnerReliability 공통 필터 연동. 설계: `AGENT_TEAM_LAUNCH.md` § FEAT-PARTNER-RELIABILITY-MONTHLY. |
| v2.43.0 | 2026-06-15 | Sprint 94 / FEAT-PARTNER-RELIABILITY (#93) — 협력사×모델×공정 추적률 분해 | **BE only minor — read-only, 신규 endpoint, migration 0**. reliability-summary(v2.41.0)는 추적률을 모델 단위(협력사 합산)로만 줌 → 협력사 분해로 "표준 가능=FNI / 교육 타겟=BAT" 행동 가능. v2.42.1(whitelist 정정) 추적률 base 위. **신규 `GET /api/ct/partner-reliability?process=MECH\|ELEC&model=&from=&to=`**(`partner_reliability_service.py`, jwt+gst_or_admin). 추적률 = `_TRACKED_SQL`/`_COVERAGE_BASE`(reliability-summary 동일 basis). **응답**(flat): `by_cell`(협력사×모델×공정) + `by_model_process`(합산 invariant anchor+batch_n) + `by_partner` + `trend`(batch·결측 omit) + meta(gate70/n_min30/whitelist echo), row confidence. **스코프**: MECH/ELEC only, **GST·SH·(미지정) 명시 제외**(M-Q4). 협력사 `_COV_PARTNER_SQL`(TMS→TMS(M)/(E)), prefix `_norm_model_prefix`(Python raw Σ, pct 평균 금지). **batch(TMS(M))=일괄**: 합산 포함(invariant)+batch_n, trend omit. **invariant**: by_cell n 가중합=by_model_process (실측 GAIA·MECH 157+256+1=414, pct 47.1%=raw Σ). **실데이터**: BAT·GAIA·MECH **17.2%**(교육 타겟) vs FNI **65.2%**(표준 근접). **Codex 2R**: CONDITIONAL_GO(M-Q4 GST·SH/A-Q2 batch_n/A-Q5 confidence)→**DEPLOY_SAFE M=0**. pytest 8/8+회귀 24 passed 1 skip. ⚠️ VIEW 비교 주석: by_model_process↔reliability-summary 는 GST·SH 제외 scope 차이(meta.scope echo). **VIEW 후속**(별 repo): `usePartnerReliability`→PartnerReliabilityCard. 설계: `AGENT_TEAM_LAUNCH.md` § FEAT-PARTNER-RELIABILITY. |
| v2.42.1 | 2026-06-15 | FIX-INSTANT-WHITELIST-SELF-INSPECTION-REMOVE — one-click whitelist 자주검사 2개 제거 | **BE only patch — read-only, 상수 정정, migration 0**. 사용자 catch: "one-click은 2개야 — tank docking, si_shipment". 실측: TANK_DOCKING 104·SI_SHIPMENT 40건 전부 active NULL(진짜 즉시완료, 추적률 분모 자동 제외) / SELF_INSPECTION(MECH 자주검사) 평균 57분 즉시완료 0%·INSPECTION(ELEC 자주검사) 평균 98분 즉시완료 15% = **실작업인데 one-click 오분류** → 추적률 부당 과소. **변경**: `_INSTANT_WHITELIST = frozenset({"TANK_DOCKING","SI_SHIPMENT"})` (자주검사 2개 제거). 6개 지표 `is_instant_whitelisted`/`_WHITELIST_SQL`/`_wl_sql` 미러 자동 전파(tagging-coverage·reliability-summary·partner-breakdown #83·data-quality·close-type-trend #90·partner_discipline Sprint 89). **⚠️ Codex Q5 핵심 결함 동반**: data-quality `trend_sql` zerotap 이 whitelist 4개 **하드코딩**(`NOT IN ('INSPECTION',...)`) → 상수 변경 자동 전파 안 됨 → `_wl_sql` 동적 미러(f-string) 교체, 단일 미러 복원(하드코딩 잔존 0). **효과(정직화)**: 자주검사 tracked 정상 포함 → tagging-coverage MECH 34→49%·ELEC 47→58%, reliability-summary 헤드라인 53→63%(표준가능 17.6% 유지). TANK_DOCKING/SI_SHIPMENT active NULL → 추적률 영향 0(whitelist 잔류=안전망). **Codex 2R**: R1 NO-GO(Q5 하드코딩/Q3 test/Q6 주석)→R2 **DEPLOY_SAFE M=0**. 주석 2곳+test fixture(SELF_INSPECTION→TANK_DOCKING) 갱신. pytest 6지표 회귀 93 passed 1 skip. **연관**: #93(협력사 데이터 입력 신뢰도, 협력사×모델×공정 추적률) **추적률 base 정정 선행** — #93은 정정된 추적률 위 설계. 설계: `AGENT_TEAM_LAUNCH.md` § FIX-INSTANT-WHITELIST-SELF-INSPECTION-REMOVE. |
| v2.42.0 | 2026-06-15 | Sprint 93 / FEAT-TAGGING-COVERAGE-FILTERS (#92) — tagging-coverage period/reference_date/partner 필터 | **BE only minor — read-only, additive, migration 0**. AXIS-VIEW #92 — 종료 누락 분석 페이지 상단 필터(오늘/주간/월간/분기+협력사)가 auto-close-summary 카드는 다 받는데 0초탭 카드(`tagging-coverage`)만 월 단위 from/to라 불일치(사용자 catch). **`GET /api/ct/tagging-coverage?period=&reference_date=&partner=`**(전부 optional, 미지정=현행 월누적 back-compat). period=today\|week\|month\|quarter(`_resolve_period_range` 재사용)+reference_date(YYYY-MM-DD)→**day 윈도우** / partner(company)→coverage·zero_tap_tasks 협력사 필터(**분모도 협력사 기준**, PI/QI/SI=GST 고정→partner='FNI'면 MECH/ELEC만). **⚠️ 공유 상수 회귀 방지**: reliability-summary(v2.41.0)가 `_COVERAGE_WHERE` import → **`_COVERAGE_WHERE=_COVERAGE_BASE+_COVERAGE_WINDOW_MONTH` byte 동일 유지** + 신규 `_COVERAGE_WINDOW_DAY`/`_COVERAGE_PARTNER_FILTER` 별도 조각(reliability-summary 무변경, byte 동일 pytest 검증). meta echo(period/reference_date/partner/window)+cache_key+route 검증(400 INVALID_PERIOD/INVALID_DATE). RBAC 그대로(partner=표시 필터). `_resolve_period_range` KST naive+`(completed_at AT TIME ZONE 'Asia/Seoul')>=%(start)s` 명시변환(DB tz 무관). **Codex R1 GO M=0**. 실데이터: back-compat 월누적 / partner=FNI MECH만(PI/QI/SI=0) / period=quarter / **reliability-summary tracking 53·standard_ready 17.6=v2.41.0 동일(회귀 0)**. pytest test_tagging_coverage_filters 7 + 회귀(sprint90 tagging+reliability) 32 passed 1 skip. **VIEW 후속**(별 repo): `useTaggingCoverage({period,reference_date,partner})`. 설계: `AGENT_TEAM_LAUNCH.md` § FEAT-TAGGING-COVERAGE-FILTERS. |
| v2.41.0 | 2026-06-14 | Sprint 92 / FEAT-CT-RELIABILITY-SUMMARY (B) — CT 데이터 신뢰도 게이트 재설계 (가중평균 폐기→count 게이트+생산량 가중) | **BE only minor — read-only, 신규 endpoint, migration 0**. CT 분석 "데이터 신뢰도" summary 가 FE 파생 **가중평균**(`Σweight(n)·n/Σn`, weight 계단식 100→1.0/30→0.5/else 0.1)이라 **풀링할수록 부풀려짐**(task 풀링 80.6% vs 실사용 협력사×모델 13%). 표본 많은 task 강조+위험(적은) task 희석=과대평가. 3모델(Claude+Codex+Fable) 합의 + 사용자 catch 누적으로 재설계. **신규 `GET /api/ct/reliability-summary?from=&to=`**(`statistics_service.get_reliability_summary`, jwt+gst_or_admin). 기존 `get_partner_breakdown`(rows)+`get_data_quality`(dist) 재사용 + 모델×공정추적율·12mo 생산비중 자체집계, 캐시1h. **핵심**: ① 가중평균 폐기 → **count 게이트**: 표준가능=셀(모델×task×dual) 중 (n≥30 AND 모델공정추적≥70%) 비율. ② **생산량 가중** standard_ready_pct=Σ(모델 12mo rolling 생산비중·모델 표준가능비율) — 주력 GAIA 88%(DUAL54+SINGLE34) 품질이 전체 좌우(정직). 표본수 가중(부풀림 폐기)과 별개(생산량=비즈니스 임팩트 정당). ③ **2조건 분리** n_met/tracking_met/both → 게이트 0%여도 진행 gradient. ④ **모델 기준**(CT 표준=모델별 적용): 게이트=모델×task×dual(협력사 합산), 협력사 원인=매트릭스 drill. Fable Q2(협력사 분리)는 협력사별 표준 전제라 모델별 표준 사용자 판정으로 무효. ⑤ 라벨 정정("CT 신뢰도"→"표준 가능 비율","입력 정합"→"마감 출처 정합"), 헤드라인=추적율(행동지표 Σtracked/Σn, tagging_coverage 정의 정합), force_closed 분리(A=v2.40.0). **Codex 3R**: R1 NO-GO(M-Q1 tracked)→R2 CONDITIONAL_GO(Q2 category=partner_scope/Q4 무자료 union+r0 분모유지 no_ct_cells/Q5 `_norm_model_prefix` 단일 ORDER BY LENGTH DESC)→R3 **DEPLOY_SAFE M=0**. Fable 2차(PR2 brief) 반박 반영. **실데이터**: 헤드라인 추적 53%(hold) / standard_ready 17.6%(생산량 가중) / n_met 51.7% vs tracking_met 22.5%(병목=추적/질) / input_integrity 71.5%·force_closed 9.2% (가중평균 80.6%→17.6% 정직화). pytest test_reliability_summary 9 passed+1 skip(운영 보존) + CT 회귀 44 passed+1 skip. 기존 endpoint touch 0 → 회귀 0. **VIEW 후속**(별 repo, VIEW_FE_Request.md 인계): CtAnalysisPage FE 파생 가중평균 제거→endpoint 소비, DataQualityGate 연결, 가중평균 카드 제거, 드릴 계층(모델→task→협력사). 설계: `AGENT_TEAM_LAUNCH.md` § FEAT-CT-RELIABILITY-SUMMARY v1~v4. |
| v2.40.0 | 2026-06-13 | FIX-FORCE-CLOSE-DURATION-SOURCE (A) — 강제종료 duration_source='FORCE_CLOSED' 근본 정합 | **BE only minor — migration 062 동반**. 강제종료(`force_close_task`)가 `duration_source` 미설정(NULL) → 입력정합(get_data_quality)에서 NULL=clean 과대 집계(force-close 199건 섞임, **81.8% 과대**). 클린 코어 원칙(2026-04-20)상 강제종료=버리는 데이터인데 source 표시 누락(v2.36.1 은 CT 표본만 force_closed=FALSE 가드, 입력정합 미정정). **변경**: ① migration 062 — duration_source CHECK enum `'FORCE_CLOSED'` 추가(057 패턴 BEGIN/COMMIT + table_constraints DROP + pg_get_constraintdef 검증 DO block, Codex R1 M-Q1 반영) + **backfill** `UPDATE force_closed=TRUE AND (source IS NULL OR ='NORMAL_COMPLETION') → 'FORCE_CLOSED'`(멱등). FALLBACK_TRIGGER_DATE_17/PREV_DAY_CAP force-close(이미 비-clean)는 cap source 보존 미변경(약불변식, Codex Q5 A 합의). ② `admin.py force_close_task` UPDATE SET 절에 `duration_source='FORCE_CLOSED'` 추가(신규 강제종료 자동 기록, 측정값 컬럼은 audit·표시용 유지). ③ `statistics_service.py _ESTIMATED_SOURCES` 에 `'FORCE_CLOSED'` 추가(`_CLEAN_SOURCES` 불변). **prod 적용 실측**: force_closed=TRUE 253건 → 251 FORCE_CLOSED + 2 FALLBACK, clean 잔존 0 → **입력정합 72.7% 정직값**. CT 표본 = v2.36.1 가드 + FORCE_CLOSED source 이중 제외. force_complete_task(force_closed 미설정)·정상완료·자동마감(NORMAL+force_closed=FALSE) 무영향 → 회귀 0. Codex R1 NO-GO(M-Q1) → R2 **DEPLOY_SAFE/GO M=0**. pytest test_fix_force_close_duration_source 5/5 + 회귀 69 passed 1 skip GREEN. **연관**: v2.36.1(FIX-CT-FORCE-CLOSE-POLLUTION, CT 표본 가드) 의 입력정합 지표 보완 + 클린 코어 원칙 근본 정합. 후속 B(별 논의)=CT 신뢰도/입력정합 summary weighted-avg 과대(80.6%) headline 재설계. 설계: `AGENT_TEAM_LAUNCH.md` § FIX-FORCE-CLOSE-DURATION-SOURCE. |
| v2.39.0 | 2026-06-13 | Sprint 90-BE-C-b (#90b) — close-type-trend 주별(ISO week) 버킷 | **BE only minor — additive, migration 0**. `?bucket=week|month`(기본 month back-compat). week=ISO 라벨(IYYY-"W"IW)+meta.bucket, zero-fill=_week_range(isocalendar↔PG 동치, 연말 W53/W01 Codex 확인), INVALID_BUCKET 400. A-1 경계 주 부분집계 허용(의미 정합). Codex R1 DEPLOY_SAFE M=0. pytest 9/9. VIEW [주|월] 토글 자동 실데이터 전환. |
| v2.38.2 | 2026-06-13 | Sprint 91-FE 연장 — 퇴근 다이얼로그 [모두 내작업완료] | **FE only patch (home_screen), BE 0**. 실기기 QA(일시정지 ✅) 후 사용자 요청. completeTask(finalize=false, 내 작업만 종료·릴레이 유지) 순차 — 실패(일시정지중/이미완료)는 요약 안내. Sprint 번호 부여(90-BE-D/91/91-FE). 공지 117 게시. Netlify prod. |
| v2.38.1 | 2026-06-13 | FEAT-OPS-FE-PENDING-GUARD — OPS app 미종료 가드 FE | **FE only patch (3화면), BE 0**. v2.38.0 방치 기준 FE part. A 퇴근 다이얼로그(open_tasks 소비 — [모두 일시정지] 순차 pause·TASK_ALREADY_PAUSED=idempotent/[나중에], SnackBar 분기 Q4) / B 방치 뱃지(무활동 N시간 48h+ danger + ⏸, metadata Row A-1) / C 종료 다이얼로그 "무활동 N시간 경과" 문구(단건, 일괄=M-3 삭제). Codex R1 NO-GO(M=3, M-1 경로오탐)→정정→구현. build GREEN + Netlify prod. 실기기 QA 위탁. 설계: AGENT_TEAM_LAUNCH.md §. |
| v2.38.0 | 2026-06-13 | FEAT-PENDING-DISCIPLINE-REFINE — 미종료="방치" 단일 기준 + 0초탭 force 제외 + 퇴근 open_tasks | **BE only minor — read-only+additive, migration 0**. BAT 매니저 일괄 강제종료(98건) 재발 방지 1차. **방치 기준**(신규 `pending_task_standard.py`) = canonical + 활성(미pause)세션≥1 + 마지막활동(시작/**재개**) 24h+ (`GREATEST(MAX(wsl.started_at),MAX(wpl.resumed_at))`, resume=wpl만 update Codex M-1/M-2). 적용 4표면 통일: 데이터종합 openTasks(월 anchor 유지)/discipline 큐/대시보드 grouped/OPS 메뉴(+additive last_activity_at·inactive_hours·has_paused_worker). zerotap force_closed=FALSE(CT·coverage 정합). 활성 task 강제종료 시 close_reason `[활동중 종료]` prefix(차단X 추적O, force write 전용). checkout 'out' 응답 open_tasks[] additive(OPS 토스트용). Codex R1 CONDITIONAL_GO(M-3 API가드→사용자 기각, view단 방지+audit 대체)→R2 DEPLOY_SAFE M=0. pytest 신규 6 + 회귀 57 = 63/63 GREEN. 후속: OPS FE 토스트·뱃지 / VIEW B5 / work_log 인덱스(BACKLOG). 설계: AGENT_TEAM_LAUNCH.md § FEAT-PENDING-DISCIPLINE-REFINE. |
| v2.37.0 | 2026-06-12 | #90 협력사 마감유형 추이 + #91 협력사 매니저 group_avg 노출 | **BE only minor — read-only, migration 0**. VIEW OPS_API_REQUESTS #90/#91. **#90** `GET /api/ct/close-type-trend?from=&to=&partner=&group=`(신규 `close_type_trend_service.py` + ct_analysis route, gst_or_admin_required): 협력사×그룹×월×{auto,zerotap,force} flat series(VIEW CloseTrendChart [비교] 오버레이). zerotap=90-BE-B 정의, partner=MECH/ELEC TMS(M)/(E) GST+SH 제외, 빈 달 zero-fill. pytest 7. **#91** `partner_discipline_service._group_avg` `is_global` 분기 제거→`cand=partners`(자사 포함 group-wide avg): 협력사 매니저도 그룹평균 노출(기존 3곳서 자사제외 2<3 영구 suppress 정정). 역추론 가드=`len(vals)≥3`(자사 외 미지수≥2, 개별 역산 불가; 1곳 누락 시 suppress). admin·협력사 동일 값, VIEW 변경 0. pytest test_sprint89 32. Codex #90·#91 각 R1 DEPLOY_SAFE M=0. 설계: AGENT_TEAM_LAUNCH.md § Sprint 90-BE-C / § #91. |
| v2.36.1 | 2026-06-12 | FIX-CT-FORCE-CLOSE-POLLUTION — CT 표준 강제종료 오염 정정 | **BE only patch — read-only, migration 0**. CT 표준(basis=ct/duration/active)에 강제종료가 섞여 표준값 부풀리던 버그 정정. **버그**: 강제종료(`force_close_task`)가 v2.22.0/v2.28.0에서 `compute_task_work`로 ct/duration 계산·저장하나 `duration_source` 미설정(NULL) → `_CLEAN_CORE`(duration_source NULL 허용) 통과 → CT 표본 오염. 클린 코어 원칙(2026-04-20 "강제종료=버리는 데이터")과 drift. **실측(2026-05~, 1,244건)**: 강제종료 128건(10.3%), WASTE_GAS_LINE_1 median 1.40h→2.98h(+112%). **Fix**: `statistics_service.py _CLEAN_CORE`에 `AND COALESCE(td.force_closed,FALSE)=FALSE` 1줄(→ #82/#83/training 전파). **자동마감 유지**: AUTO_CLOSED_BY_* 65건 work_completion_log 전건 보유=작업자완료(NORMAL_COMPLETION)라 제외 X(Codex Q3 실데이터 0건 확인). Codex R1 NO-GO(Q3)→실데이터 해소→**R2 DEPLOY_SAFE M=0**. pytest 신규 test_ct17 + CT회귀 18/18 GREEN. read-only·저장값 무변경(man-hour 리포트 영향 0)·factory/dashboard 무영향. VIEW `CT_VISUALIZATION_GUIDE.md` §8-7 #1 정정. 설계: `AGENT_TEAM_LAUNCH.md` § FIX-CT-FORCE-CLOSE-POLLUTION. |
| v2.36.0 | 2026-06-11 | Sprint 90-BE / 90-BE-B — 태깅 커버리지/0초탭 드릴다운 + 마감 유형 월별 추이 zerotap | **BE only minor — read-only, migration 0**. AXIS-VIEW `종료 누락 분석` mockup 2종 실데이터. **① Sprint 90-BE** `GET /api/ct/tagging-coverage?from=&to=`(신규 `tagging_coverage_service.py` + `ct_analysis.py` route, `@jwt_required+@gst_or_admin_required`) — 공정별(MECH/ELEC/PI/QI/SI) `{tracking_pct, zero_tap_pct, confidence}` + `well_tracked_pct`(추적율≥80% serial) + `zero_tap_tasks{공정:[{task_id,ko,zero_pct,n,oneClick,partners[{partner,share}]}]}`. **분모 `_COVERAGE_WHERE`** = 완료+active NOT NULL+applicable+`force_closed=FALSE`, TEST/TMS모듈 제외, **duration_source 필터 미적용**(자동마감·admin대행=미추적 분모 포함, 사용자 결정 — mock PI/QI/SI 고미추적 신호). **3분류 oneClick>zero_tap>tracked**: zero_tap=NOT oneClick AND (active≤1 **OR close_reason 존재**) / tracked=NOT oneClick AND active>1 AND close_reason IS NULL(R3 M-3 — active>1만으론 자동완료 미추적 보장X, 정상완료=close_reason NULL 전수검증). PI/QI/SI partner='GST' 단일. DUAL L/R per-row. share=largest-remainder(정수, 결정적 tie-break) Σ100. Codex **4라운드 M=3→2→1→0**. pytest 16/16 + CT회귀 16/16. **② Sprint 90-BE-B** `data-quality.auto_close_trend[]`에 `zerotap`/`zerotap_rate` additive(`statistics_service.py` trend FILTER+dict 2줄, CloseTrendChart용, zerotap 정의=90-BE 동일, 신규route 불필요=DRY). Codex 1라운드 DEPLOY_SAFE. pytest 17/17(test_ct12 신규+test_ct09 invariant 불변). **advisory(BACKLOG)**: A-1 SELF_INSPECTION 자동마감 시 월별 auto>zerotap 역전 가능(VIEW tooltip) / A-2 private `_INSTANT_WHITELIST` import. VIEW FE 후속: 2 card 연동+배지 제거. 설계: `AGENT_TEAM_LAUNCH.md` § Sprint 90-BE/90-BE-B. |
| v2.35.0 | 2026-06-10 | Sprint 89-BE (#87) 규율 대시보드 보정 (미종료 월단위·기준통일 + 중복지표 교체) | **BE only minor — read-only, migration 0**. v2.34.0 운영 catch 보정. **① 중복지표 교체**: `checkinNoTag`(=taggingRate 카운트 버전, 중복) 폐기 → `checkoutMiss`(퇴근미체크율, `reference_only:true` 참고/평가제외) — `_query_checkout_miss`(#86 checkout_status 일별 cutoff D+1 02:00 KST 월×협력사 가중집계), `_envelope` 에 `reference_only` 필드. **② openTasks 월단위(A)**: 실시간→`started_at`∈월 AND completed_at IS NULL (`_query_open_tasks_count` 시그니처 +start/end), meta `openTasks_basis='monthly_started_open'`. **③ 미종료 기준 통일(B)**: openTasks+`/open-tasks` 큐 = 정식 `/tasks/pending/grouped` 정합 (`is_applicable=TRUE`+`COALESCE(force_closed,FALSE)=FALSE`+TEST 제외). **④ 공통 하이진(C)**: autoClose/zeroTap/universe is_applicable+TEST. **⑤ 큐 realtime 유지(D)**: summary openTasks(월)≠큐(realtime), 의도된 차이 meta `basis` 명시. **⑥ 비협력사 제외**: `_EXCLUDED_PARTNERS`=('GST','SH')(자체수행+비협력사 구데이터, 사용자 지시) — `_PARTNER_SQL` NULLIF 중첩 + worker 쿼리 NOT IN. Codex 중복지표→미종료 설계 R1 GO/실코드 R2 DEPLOY_SAFE M=0. pytest 32/32 GREEN + 실스모크(SH/GST 제외, 6 협력사). VIEW FE 영향: checkinNoTag→checkoutMiss 키 + reference_only/basis meta + openTasks 월단위 의미. |
| v2.34.0 | 2026-06-10 | Sprint 89-BE (#87) 협력사 규율 대시보드 Phase 2a | **BE only minor — read-only, migration 0**. Phase 1 placeholder 4지표 중 3개 실측 + trend endpoint. **taggingRate**(`_query_tagging`: GST 현장 출근(work_site='GST') DISTINCT worker 분모 ÷ work_start_log 태깅, 월 DISTINCT, 의미론 A/B) / **checkinNoTag**(onsite−tagged) / **zeroTap**(`_query_zerotap`: active_time_minutes≤1 미추적 ÷ substantive, one-click=`statistics_service.is_instant_whitelisted()` Python 제외, 의미론 C=#83 정합 — 실데이터 BAT/MECH 0.66·FNI 0.17·ELEC ~0) / **`GET /api/admin/discipline/trend?months=N(1~12)`**(월별 autoClose/taggingRate/zeroTap, openTasks 제외, 매니저 자사 series만). union universe(task파생 ∪ worker파생 partner, 의미론 D) + `partner_provenance` meta. group_avg 불변식(raw None 가능 지표 tagging/zerotap/checkinNoTag=missing None / count open/auto=0.0). **Phase 1 버그 2건 동반 정정**: ① `_PARTNER_SQL` `NULLIF(…,'GST')`(GST 자체수행 noise row 제외, openTasks/autoClose 포함) ② `build_open_tasks` 타사/NULL company 작업자 `worker_name`/`worker_id` 마스킹 + `worker_masked`(M-Q8/M-Q8-NULL). Codex 설계 R1 GO(의미론 A/B/C/D)/실코드 R2 M=2(M-Q5 checkinNoTag missing·M-Q8 누수)→R3 M=1(M-Q5→A 2자합의·M-Q8-NULL)→**R4 DEPLOY_SAFE M=0**. pytest test_sprint89 31/31 GREEN + 실데이터 스모크(rows=7 GST제외). 회귀 0. **다음**: Phase 2b(checklist)→계획·납기(Sprint 87-BE). |
| v2.33.0 | 2026-06-10 | Sprint 89-BE (#87) 협력사 규율 종합 대시보드 Phase 1 | **BE only minor — 신규 `services/partner_discipline_service.py` + `routes/admin_discipline.py` + blueprint, read-only, migration 0**. VIEW PartnerDashboardPage 실데이터 1차. endpoint 2개: `GET /api/admin/discipline/summary?month=`(partner×group + 지표 envelope — Phase 1 실측 `openTasks`/`autoClose` + `group_avg` peer-only k≥3, Phase 2 4지표 placeholder `available=false`) + `GET /api/admin/discipline/open-tasks`(미종료 즉시조치 큐 + repeat 30d/2). 권한 `@jwt_required+@manager_or_admin_required` + `resolve_company_scope` try-**밖**(#86 누수 차단). 공정성: BE는 raw+group_avg만(등급·줄세우기=VIEW), MECH/ELEC only, 협력사 매니저=자사 row만+group_avg suppress(현 3사 peer<3 `insufficient_peers`), 추적율<70% `grade_eligible=false`. `_PARTNER_SQL`=`NULLIF(TRIM(CASE…),'')`(빈값 partner 제외). `statistics_service.is_instant_whitelisted()` 공개 helper(A3, Phase2 zeroTap용). **근태(출근율·퇴근미체크) 협력사 평가 제외**(2026-06-10 결정, 출근=태깅율 분모로만). Codex 설계 R1 M=8→R2 조건부 GO(A-spec 5건)/실코드 R3 M=1(_PARTNER_SQL 빈값)→fix→**R4 DEPLOY_SAFE M=0**(추적 6→4→1→0). pytest 신규 test_sprint89 22 GREEN, 회귀 0. **다음**: Phase 2(taggingRate/checkinNoTag/zeroTap/checklist/trend)→계획·납기(Sprint 87-BE). 설계: `AGENT_TEAM_LAUNCH.md` § Sprint 89-BE. |
| v2.32.2 | 2026-06-10 | #88 출하상세 출하 처리자(shipment_worker) 1필드 | **BE only patch — additive, read-only, migration/route 0**. `shipment_history_service.get_shipment_details()` items[]에 `shipment_worker`(string\|null) — SI_SHIPMENT 완료자 `COALESCE(closed_by 대행 매니저, worker_id 현장 작업자)`→workers.name (LEFT JOIN LATERAL, DUAL 대표 1명), 엑셀 backfill→null. pytest test_shipment_history 32/32 GREEN. 기존 items 필드 불변. AXIS-VIEW #88 선반영 자동 표시. |
| v2.32.1 | 2026-06-10 | 출고처리 실패 메시지 명확화 (FIX-SHIP-COMPLETE-MSG) | **BE only patch — 메시지 문구만, 에러코드/로직 변경 0**. pre-transition 제품(QR 이미지 전환 ~1달, task seed 부재)이 출고 도달 시 SI task 없어 실패 → `SI_TASK_NOT_FOUND`/`SI_FINISHING_NOT_STARTED` 한글·실무 친화("먼저 QR 스캔으로 공정을 시작한 뒤 출고처리"). 박승록(GST) GBWS-6971/6997 catch. **근본원인=task seed 부재**(자동 seed 보류, BACKLOG `FEAT-SHIP-COMPLETE-AUTO-SEED-PRETRANSITION` — 재발 시 재검토). pytest test_ship_complete 15/15 GREEN. |
| v2.32.0 | 2026-06-10 | Sprint 88-BE (#86) 협력사 데이터 RBAC + 근태 checkout_status | **BE minor — 협력사 대시보드 공유 1번 모듈, migration 0, VIEW URL 변경 0**. 3파트: **PR1 [SECURITY]** `jwt_auth.resolve_company_scope()` 신규(SSoT) — admin/GST→전체 / 협력사 매니저→자사 exact(TMS(M)/(E)) / **company 없는 매니저·None·비매니저→CompanyScopeError→403**, `@app.errorhandler` 등록. 근태 admin 4 + 자동마감 2 라우트에 resolver를 try **밖** 호출(except가 삼키면 errorhandler 우회 차단). 기존 admin/GST/협력사 동작 100% 동일·**company 없는 매니저 전체누수만 차단**(근태 `_get_manager_company_filter`+자동마감 `_build_partner_filter` 두 누수 통일). **step① [REFACTOR]** `_get_attendance_data`/`_kst_date_range`/`_get_attendance_trend_data` → 신규 `services/hr_attendance_service.py`(verbatim), admin.py 2,626→2,456줄, Before/After pytest 37==37(동작 동일). **step② [FEAT additive]** `get_checkout_status_map()`(not_started/working/missed/done — day-row[같은날 재출근 0.04%→세션모델 미채택], check_in=MIN(in), cutoff=LEAST(익일이후 첫in, D+1 02:00 KST)[야간조 오탐0], check_out check_in<t<cutoff MAX(out)+orphan 가드) + `get_attendance_data_with_checkout()`(기존 status 불변 병합 + by_work_site[GST/HQ] 미체크률 missed/checked_in 분모0→null). admin `/today`·`/hr/attendance?date=` 노출. 단일 scoped_workers→records, summary=Python(누수 차단). 실데이터 정합(6-08 미체크 14.53%). **Codex**: PR1 라운드3 DEPLOY_SAFE / PR2 설계 라운드2 GO / step② 구현 GO 전부 M=0. **Fable 판정**으로 초기 B안(신규 endpoint) 차선 catch→A′(admin route 재사용+추출, 계약 L8717 정합). pytest PR1 94 + step① 37 + step② 신규 9+회귀 37 GREEN. **VIEW 후속**(별 repo): PartnerAttendanceCard `enabled:isAdminOrGst` 가드 제거 + checkout_status/miss_rate 렌더. #87(checkout-miss-weekly) 별 sprint. 설계: `AGENT_TEAM_LAUNCH.md` § Sprint 88-BE. |
| v2.31.0 | 2026-06-08 | #83 CT 협력사×모델×task×dual 분해 집계 (partner-breakdown) | **BE only minor — 신규 `GET /api/ct/partner-breakdown`(read-only) + statistics_service.py 신규 함수, DB/migration 0**. CT 분석 본질=협력사 평가인데 task-stats는 task-pooled(partner 차원 전무) → VIEW CtPartnerModelMatrix·협력사 평가 mock. **검증 6개**: 4축 57% n<5→rollup 72% 쓸만(병행 필수) / 즉시완료율 **BAT 61%** vs FNI 11% / vs_std TMS(E) 0.65·P&S 1.07 / BAT 61% 미추적=생존편향 / DUAL +18%(현업 ~1.5배 병렬제작)→vs_std (task,dual) 표준. **변경**: `get_partner_breakdown` → `{rows(4축), rollups{partner_task,partner_model}, meta}`. 트러스트 윈도우(S-1)·basis=ct(S-2)·dual(#81)·partner 정규화(`_COMPANY_SQL` TMS(M)/(E)) 재사용. rollup=독립 pooled median(raw합산 ❌) + **vs_task_standard_ratio**((task,dual) 표준, n<5→null) + **tracking_coverage**(속도+추적 짝, 생존편향) + instant_completion(one-click 화이트리스트 제외) + category_mix. 게이트 n<5 reject/5~29 prov/30+ std. Tukey n≥30만. **BE composite 미합성**(가중=VIEW). meta garbage_excluded=false(S-3 전 명시). **Codex**: 설계 R1 M=3→R2 GO / 구현 R1 M=1(rollup raw-pull+Python vs SQL GROUP BY)→**M→A 2자 합의**(median 시맨틱 정확·~1500행 성능 무관)→**DEPLOY_SAFE**. pytest 신규 14+회귀 33 GREEN. read-only·additive(task-stats/data-quality 불변)·회귀 0. 실데이터 스모크 TMS(E) PANEL vs_std 0.65. **잔여 A**: rollup SQL GROUPING SETS 전환(별 BACKLOG). 설계: `CT_PARTNER_BREAKDOWN_DESIGN.md`. |
| v2.30.0 | 2026-06-08 | Sprint S-2 (ⓑ) 진짜 CT across-worker union + 동시작업자 (VIEW #82 ⓑ) | **BE minor — migration 061(ct_time_minutes 컬럼+백필) + task_detail.py + admin.py + statistics_service.py + ct_analysis.py**. 현 active/duration 둘 다 M/H(across-worker SUM)이지 CT 아님 → 진짜 CT = 작업자 정제세션 across-worker **UNION**(벽시계). ① 검증: 다중작업자 ≠ 병렬(세션 겹침=병렬만 CT<M/H), PANEL 4명동시 1885→944(2.0x)/UTIL 릴레이 1.0x, 전체 ~9% 병렬. **변경**: migration 061 `ct_time_minutes`(nullable)+백필 = `(session∩BH−pause∪식사)` per-worker → across-worker `range_agg` union → `LEAST(union, active)`(A-1 CT≤M/H), active=0→ct=0(LEFT JOIN COALESCE)/active NULL→ct NULL, 멱등. `compute_task_work` `{manhour,active,ct}` + 완료 4경로 ct 동봉. `statistics_service` `basis=ct`(ct>0 모집단) + `effective_concurrency_median`(instance active/NULLIF(ct,0) median) + `avg_workers` + coverage(active,ct) 일반화 + meta n_used(Tukey전)/n_sample(Tukey후). S-1 상속(컷오프 5/1·from/to·Tukey·provisional·캐시키). **Codex**: 설계 R1 M=3→R2 M=1→R3 GO / 구현 R1 M=2(n_used Tukey후·백필 active=0→NULL)→fix→**DEPLOY_SAFE M=0**. pytest 신규 15+회귀 67 GREEN. additive, man-hour/active/duration 불변. 배포 검증: 백필 갭 0·CT>active 위반 0. **ⓒ=S-3 분리**. 설계: `CT_S2_TRUE_CT_UNION_DESIGN.md` v2. |
| v2.29.1 | 2026-06-08 | get_data_quality 트러스트 윈도우 통일 (VIEW #82 ⓐ 후속) | **BE only patch — statistics_service.py + ct_analysis.py, DB 0**. S-1이 task-stats만 트러스트 윈도우 적용 → data-quality는 옛 `lookback_days=90` 잔존(미성숙 3·4월 섞임). `get_data_quality(from_month,to_month)` `_resolve_window`/`_WINDOW_WHERE` 재사용(duration_source 분포만 윈도우 통일, 추이 6mo·교육전후 전기간 유지). meta `lookback_days`→`window{from,to}`+trust_start. `/data-quality?from=&to=`+400. pytest data-quality 5 GREEN, 회귀 0. |
| v2.29.0 | 2026-06-08 | Sprint S-1 (ⓐ) CT basis=active + 미추적 제외 + 신뢰컷오프 + 월범위 (VIEW #82 ⓐ) | **BE only minor — statistics_service.py + ct_analysis.py, DB/migration 0 (active_time_minutes read-time)**. 14개 운영 쿼리 검증: ① 현 "CT"=M/H(across-worker SUM) ② **act=0 = 미추적**(작업자 시작=완료 즉시태깅, work_completion_log d=0): MECH 배관 ~50%/ELEC ~10%, act 분포 이봉형(0 vs ≥10분) ③ act=0 제외 시 MECH CT 0.5h→2.2h ④ 미추적율 **BAT 59% vs FNI 7%**(완료율 둘다 100%가 숨김) → 생존편향 ⑤ duration 058 미적용 → 6월+만 신뢰, active 059/060 전구간 clean. **변경**: `GET /api/ct/task-stats` 신규 `basis`(duration 기본\|active) + `from`/`to`(YYYY-MM KST 반열림), **`period` 제거**(M-6). basis=active → `active_time_minutes>0`(미추적 제외). **신뢰 기준 `CT_TRUST_START_MONTH='2026-05'` 단일**(사용자 확정), duration은 meta `duration_stale_before`(6월 이전 포함 시). 생존편향 `tracking_coverage_by_partner`(basis=active, 표시통계 동일 슬라이스 act>0·Tukey 전 — FNI 0.71/BAT 0.34). basis별 n 분리 + Tukey base_n/clipped_n + n<30 `provisional` + 캐시키 전파라미터 + 입력검증 400. **Codex**: 설계 R1 M=4 → R2 CONDITIONAL_GO(M-5/M-6) → 구현 R1 M=2 → **재검증 DEPLOY_SAFE/GO M=0**. pytest 신규 18 + 회귀 sprint85 16 GREEN. 회귀 0(additive, basis=duration 보존). **분리**: ⓑ CT(union)=S-2(새 컬럼+migration) / ⓒ DAG+무결성KPI=S-3. 설계: `CT_S1_BASIS_ACTIVE_DESIGN.md` v3. |
