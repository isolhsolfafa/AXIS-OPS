...
| v2.18.12 | 2026-05-21 | ROLLBACK qr_scanner_web.dart v2.18.4 상태로 복귀 | **FE 1 파일 patch release**. v2.18.5~v2.18.11 (11번 BUG-42 hotfix 시리즈) 모두 롤백. 실기기 catch (사용자): 콘솔 로그 후면 카메라 (`label=후면 듀얼 와이드 카메라`) 표시되는데 실제 화면 셀카 — html5-qrcode 와 우리 변경(`getUserMedia` 직접 호출 + `videoConstraints` + `{exact:'environment'}` + label 매칭) stream 충돌 추정. **변경**: `git checkout 8a2233f -- frontend/lib/services/qr_scanner_web.dart` → v2.18.4 상태 (506 LOC, 단순 `facingMode:'environment'` hint). 다른 변경 보존. **BACKLOG**: BUG-42 🔴 OPEN reopen + BUG-42-TASK3-AUTO-ZOOM-DEFERRED / CAMERA-SWITCH-BUTTON-DEFERRED / REFACTOR-QR-SCANNER-WEB-FUNCTION-SPLIT 보존. **향후 재시도**: 실제 iPhone + Mac 원격 디버깅 또는 Eruda 콘솔 환경 확보 + minimal change 마다 실기기 검증 |
