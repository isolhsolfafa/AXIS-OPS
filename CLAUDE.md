...
```
① Claude Code → Sprint 설계서 작성 (AGENT_TEAM_LAUNCH.md)
  ↓
...
④ Claude Code → Sprint 코드 구현
  ↓
⑤ pytest → regression 테스트 실행
  ↓ 실패 시 → 수정 내용 Codex 합의 후 수정 → pytest 재실행
  ↓
...
1. **설계서 선행**: Sprint 코드 작성 전 반드시 `AGENT_TEAM_LAUNCH.md`에 설계 완료
2. **설계 단계 교차 검증**: 코드 구현 전 `/codex:review`로 설계서 검증 — 구현 후 재작업 방지
3. **합의 기반 보정**: Codex 지적 사항은 Claude와 대조하여 합의된 항목만 반영 (맹목 수용 금지)
4. **pytest 필수**: BE 코드 변경 후 `pytest` 전체 통과 확인 — 실패 시 머지 금지
5. **테스트 실패 시 합의 수정**: regression 실패 수정 내용도 Codex 합의 후 진행
