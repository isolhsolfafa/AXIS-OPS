...
> interval-union(v9)은 **배포 전** 상태(2026-06-03 확인). 아래 baseline을 기록해두고, **배포 + 백필(§E9) 후 동일 쿼리 재실행 → 기대값과 대조**하면 백필 성공 여부가 1분에 판정됨.
> ⚠️ 배포 전에는 모든 행이 옛 단일구간 로직 → `post_fix` 계열이 0이 아닌 게 정상(아직 새 로직 아님). 배포 후엔 신규 마감은 0이어야 함.
