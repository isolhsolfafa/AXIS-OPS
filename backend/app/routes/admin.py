        ...
        tasks = [
            {
                ...
                'worker_id': row['worker_id'],
                'worker_name': row['worker_name'],
                'company': row.get('worker_company'),
                'serial_number': row['serial_number'],
                'qr_doc_id': row['qr_doc_id'],
                'task_category': row['task_category'],
                'task_id': row['task_id'],
...
def get_attendance_summary() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        start, end = kst_date_range(target_date)
        records, _ = get_attendance_data(start, end, company_filter=company_filter)
...
def get_attendance_trend() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        trend = get_attendance_trend_data(dt_from, dt_to, company_filter)
        return jsonify({
