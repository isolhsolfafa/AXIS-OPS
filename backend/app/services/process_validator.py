...
def validate_process_start(
    ...
    2. MM(기구) 완료 체크 → 미완료 시 app_alert_logs INSERT + MM 관리자 대상 알림
    3. EE(전장) 완료 체크 → 미완료 시 app_alert_logs INSERT + EE 관리자 대상 알림
    ...
    # MM 미완료 시 알림 생성
    if not completion_status.mm_completed:
        warnings.append("MM(기구) 공정이 완료되지 않았습니다.")

        # MM 관리자들에게 알림 생성
        mm_managers = get_managers_for_role('MM')
        for manager_id in mm_managers:
            alert_id = create_alert(
                alert_type='PROCESS_READY',
                message=f"[{serial_number}] {process_type} 공정 대기 중 - MM 공정 미완료",
            ...
            if alert_id:
                alerts_created += 1
                logger.info(f"Alert created for MM manager {manager_id}: alert_id={alert_id}")

    # EE 미완료 시 알림 생성
    if not completion_status.ee_completed:
        warnings.append("EE(전장) 공정이 완료되지 않았습니다.")

        # EE 관리자들에게 알림 생성
        ee_managers = get_managers_for_role('EE')
        for manager_id in ee_managers:
            alert_id = create_alert(
                alert_type='PROCESS_READY',
                message=f"[{serial_number}] {process_type} 공정 대기 중 - EE 공정 미완료",
                serial_number=serial_number,
                qr_doc_id=qr_doc_id,
                triggered_by_worker_id=triggered_by_worker_id,
                target_worker_id=manager_id,
            ...
            if alert_id:
                alerts_created += 1
                logger.info(f"Alert created for EE manager {manager_id}: alert_id={alert_id}")

    # MM과 EE 둘 다 완료되어야 진행 가능
    can_proceed = completion_status.mm_completed and completion_status.ee_completed
