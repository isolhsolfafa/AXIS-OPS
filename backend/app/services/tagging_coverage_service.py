...
핵심 정의 (Codex 합의):
  ...
  - 3분류 우선순위 oneClick > zero_tap > tracked (R3 M-3):
      ...
      zero_tap = NOT oneClick AND (active_time_minutes <= 1 OR close_reason IS NOT NULL)
...
# 드릴다운용 raw zero (oneClick task 포함, oneClick 플래그로 정상 구분).
...
def get_tagging_coverage(
    ...
    # ③ 드릴다운 — task별 n + zero_n (raw, oneClick 포함)
    task_sql = f"""
    ...
    # ③ partners — zero_tap 인스턴스(raw) 협력사 점유
    partner_sql = f"""
        SELECT td.task_category AS process, td.task_id,
    ...
    for r in task_rows:
        ...
        if n < 1:
            continue
        zero_n = int(r["zero_n"])
        partners = _shares(pmap.get((proc, task_id), []))
    ...
    for proc in zero_tap_tasks:
        zero_tap_tasks[proc].sort(key=lambda t: (-t["zero_pct"], t["task_id"]))
