...
def create_app(config_class: type = Config) -> Flask:
    ...
    from app.routes.ct_analysis import ct_analysis_bp  # Sprint 85: CT 분석 허브 (① 데이터 신뢰도 + ② CT 표준 IQR)
    from app.routes.admin_discipline import admin_discipline_bp  # Sprint 89-BE: 협력사 규율 종합 대시보드 (#87)
    ...
    app.register_blueprint(ct_analysis_bp)          # Sprint 85: CT 분석 허브
    app.register_blueprint(admin_debug_bp)          # v2.20.2: scheduler 진단 + cron job 강제 실행
    app.register_blueprint(admin_discipline_bp)     # Sprint 89-BE: 협력사 규율 종합 대시보드 (#87)
    logger.info("Blueprints registered: auth, work, product, alert, admin, sync, gst, checklist, hr, notices, qr, factory, analytics, production, admin_materials, admin_checklists, shipment_history, admin_shipment_flow, admin_dashboard, admin_debug, admin_discipline")
