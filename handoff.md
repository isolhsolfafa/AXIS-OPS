...
> 🚨 **2026-06-08 KST — CT 분석 결정적 진단 (다음 세션 핵심 맥락)**: CT 분석 인프라(S-1/S-2/#83) **완료**됐으나 **app 도입 초기 태깅 미성숙으로 product-level CT/lead/효율/납기 인사이트가 비현실적**. **증거**(출하완료 118제품): 제품당 applicable 14.5 task 중 **완료 태깅 3.2개(22%)** / MECH applicable 5.8개 중 **실시간 잡힘 0.5개(9%)** / MECH 제품당 CT 7.3h (현장 기대 ~30h의 24%). 실작업 70~80% 미태깅(즉시완료 act=0/미입력) → 총 CT 과소 → lead 대비 효율 3% 비현실적. **결론**: CT 계산 로직(S-1/S-2)은 정확, **입력 데이터가 못 따라옴**. → **product-level CT/효율/lead/납기 = 시기상조(해석 금지)**. **현 단계 우선순위 = 운영·교육·UX(태깅 정착)** — 작업자 실시간 태깅 정착(BAT 즉시완료 61%→교육), 완료율 22%·MECH 추적 9% 끌어올리기, OPS PWA 태깅 UX 개선(즉시 시작=완료 방지/누락 알림). lead 시각화는 **계획 DATE 자기충족**(현재=실제 0.0일, change_log 최초값만 frozen baseline) + 태깅 미성숙으로 추가 보류. CT 분석 재가동 = 태깅율 70%+ 후. 상세: BACKLOG `🚨 CT 분석 결론`. ⏭️ **다음 OPS 작업 = CT 아님, 태깅 정착(운영/교육/시스템 UX)**. **[2026-06-10 진척]** #86 협력사 데이터 RBAC(v2.32.0) + #87 협력사 규율 대시보드 Phase 1(v2.33.0) 배포 — 규율 대시보드(미종료·자동마감, Phase 2 태깅율)가 곧 태깅 정착 측정·환원 수단. 다음 = #87 Phase 2 → Sprint 87-BE 계획·납기.
> 마지막 업데이트: 2026-06-15 KST (✅ **v2.46.0 — Sprint 95 / FEAT-SHIPMENT-DAILY-COMPLETED-LIST: 출하 일일 알림 완료 리스트 추가**. BE only minor, additive, migration 0. 매일 07:30 KST 출하 미처리 알림 메일(Sprint 79)이 "못 나간 것"만 발송 → 사용자 제안 "어제 실제 출하 완료"도 추가(출하 현황 종합 일일 리포트). **신규 `get_completed_shipments(yesterday)`**(actual_date=어제, app SI_SHIPMENT.completed_at OR ETL actual_ship_date, `_get_actual_date_subquery` 재사용, ship_plan_date 무관). **⚠️ Codex M-Q4 동반 버그 fix**: 기존 `get_overdue_shipments`(미처리 메일)가 customer만 필터해 **TEST S/N 섞여 발송 중**(실측 TEST-2221~6) → 두 함수 `serial NOT LIKE TEST%` 보강(_TEST_EXCLUDE_SQL 정합). `send_shipment_overdue_alert(completed_items=None)` 하위호환+`_render_completed_section`(0건 health check). subject 기존 prefix(⚠️/✅) 유지+완료 정보(메일 필터 호환). scheduler 완료 조회+전달. **Codex 2R**: R1 NO-GO(M-Q4)→R2 **DEPLOY_SAFE M=0**. pytest test_sprint95 6/6 단위+회귀 21. 실데이터 2026-04-02 TEST 제외 확인·6-11 완료 11건 렌더. **🟢 배포 완료**(health v2.46.0). 다음 07:30 KST부터 완료 리스트 포함 발송. ⏭️ **다음 = VIEW FE 연동(usePartnerReliability #93·#92·reliability 게이트·#94 instant) / BACKLOG advisory 등록(SHIPMENT-MAIL-ROW-CAP 메일 row 상한 + 70% 임계 + CTE 등) / request 추가 검토**. 설계: AGENT_TEAM_LAUNCH.md § Sprint 95.)
> 이전: 2026-06-15 KST (✅ **v2.45.0 — FIX-ZEROTAP-AUTOCLOSE-SEPARATION: 0초탭/자동마감 분리 4개 지표 통일**. BE only minor, read-only, additive, migration 0. "0초탭(zerotap)"이 자동마감(close_reason, 평균 8h 방치)을 섞어 4배 부풀림(623 vs 진짜 149, 사용자 catch). 전수 확인: 4개 지표 중 3개 `active≤1 OR close_reason`. 진짜 0초탭=`close NULL AND active≤1` 통일. **tagging-coverage(#94 additive)**: instant_pct(분모=closed_n)+instant_n/closed_n/autoclose_n, partners=instant, zero_* DEPRECATED 병기, meta.coverage_partition. **close-type-trend/data-quality**: zerotap→close NULL AND active≤1(auto와 배타). **partner_discipline**: 분모+분자 둘 다 close NULL(Codex M-1 희석 해소). **협력사 대시보드는 원래 active≤1만 써서 거의 분리됨(9%)**, 나머지 3개가 76% 섞였던 것. `_TRACKED_SQL`/`_COVERAGE_BASE` 불변→reliability-summary/partner-reliability(#93) 영향 0. Codex 3R(컨셉 R1 NO-GO M-1→R2 GO/구현 R3 NO-GO meta.note→GO). test_ct12/ctt01 zerotap 2→1 갱신+영향 77 GREEN+회귀 29 passed 1 skip. 실데이터 tagging instant MECH34%/ELEC7%·autoclose ELEC358/data-quality 206→67/close-type 35. **🟢 배포 완료**(health v2.45.0). ⏭️ **다음 = VIEW #94 연동(별 repo): instant_pct/instant_n/closed_n 사용, zero_* fallback 금지(분모 다름), 자동마감은 0초탭 카드서 제외(상단 자동마감 카드) / request 추가 검토 / VIEW FE(usePartnerReliability #93·#92·reliability 게이트)**. 설계: AGENT_TEAM_LAUNCH.md § FIX-ZEROTAP-AUTOCLOSE-SEPARATION.)
> 이전: 2026-06-15 KST (✅ **v2.44.0 — Sprint 94-b / FEAT-PARTNER-RELIABILITY-MONTHLY: 매트릭스 월별/필터 + 추이 분리**. BE only minor, read-only, additive, migration 0. v2.43.0 매트릭스 기본 누적(2026-05~06)인데 교육/개선 타겟은 "이번달 누가 garbage" 월별로 봐야(사용자 catch). 종료 누락 분석 페이지 공통 필터(오늘/주간/월간/분기+협력사)를 매트릭스도 받게 + 추이(trend)는 시계열 유지 분리. **`?period=today|week|month|quarter&reference_date=&partner=&process=&model=&from=&to=`**(미지정=현행 누적 back-compat). 매트릭스(by_cell/by_model_process/by_partner)=period 윈도우(_resolve_period_range #92 정합)+partner/process/model 필터. trend(협력사×월)=trust_start~현재월 월별·process/model 적용·period/partner 제외(사용자 결정: 페이지 MECH 선택 시 추이도 MECH, 시간·협력사는 추이 자체라 무관). **Codex M-1 해소**: 현 SQL month 집계라 day period 못 자름 → 매트릭스 day SQL+trend 월별 SQL 분리(2 쿼리). batch(TMS(M)) 매트릭스 합산포함+batch_n·trend omit, confidence, cache 분리, route 검증. **Codex 4R**: 기획 타당성(누적 vs 월별) **독립 판정=월별 타당**(교육 타겟=최신 단면, 추이 분리=정석) → 구현 R1 NO-GO(M-1) → R2 trend 필터 정책(process/model 적용) → R3/R4 **DEPLOY_SAFE M=0**. 실데이터 period=month(6월) 매트릭스 단월 BAT·GAIA·MECH **13.0%**(교육 후에도 낮음)·trend 누적·partner=FNI 매트릭스만·invariant 단월. pytest 13/13. **🟢 배포 완료**(health v2.44.0). ⏭️ **다음 = ① 0초탭 정의 정정(🟡 보류 — VIEW OPS request 연계 추후. 현 `_ZEROTAP_SQL`=active≤1 OR close_reason 합집합이라 자동마감(close_reason)이 0초탭에 중복 포함: 206=진짜즉시탭71+자동마감135. auto⊆zerotap. 마감유형 추이 자동마감/0초탭 135 겹침. 정정 옵션=active≤1만 or 우선순위 배타) ② OPS_API_REQUESTS request 추가 내용 검토 ③ VIEW FE 연동(usePartnerReliability 공통 필터/#92/reliability 게이트)**. 설계: AGENT_TEAM_LAUNCH.md § FEAT-PARTNER-RELIABILITY-MONTHLY.)
> 이전: 2026-06-15 KST (✅ **v2.43.0 — Sprint 94 / FEAT-PARTNER-RELIABILITY (#93): 협력사×모델×공정 추적률 분해**. BE only minor, read-only, 신규 endpoint, migration 0. reliability-summary(v2.41.0)는 추적률을 모델 단위(협력사 합산)로만 줌 → 협력사 분해로 "표준 가능 입력=FNI / 교육 타겟=BAT" 행동 가능. v2.42.1(whitelist 자주검사 제거) 추적률 base 정정 위 구현. **신규 `GET /api/ct/partner-reliability?process=MECH|ELEC&model=&from=&to=`**(`partner_reliability_service.py`, jwt+gst_or_admin). 추적률=`_TRACKED_SQL`/`_COVERAGE_BASE`(reliability-summary 동일 basis, A-2 분모 one-click=active NULL 자동 제외 정합). **응답**(flat, VIEW 피벗): by_cell(협력사×모델×공정)+by_model_process(합산 invariant anchor+batch_n)+by_partner+trend(batch·결측 omit)+meta(gate70/n_min30/whitelist echo), row confidence. **스코프**: MECH/ELEC only, GST·SH·(미지정) 명시 제외(M-Q4). 협력사 `_COV_PARTNER_SQL`(TMS→TMS(M)/(E)), prefix `_norm_model_prefix`(Python raw Σ, pct 평균 금지). **batch(TMS(M))=일괄**: 합산 포함(invariant)+batch_n, trend omit. TMS(E)=정상. **invariant**: by_cell n 가중합=by_model_process (실측 GAIA·MECH 157+256+1=414, pct 47.1%=raw Σ). **실데이터**: BAT·GAIA·MECH **17.2%**(교육 타겟) vs FNI **65.2%**(표준 근접) — 협력사 분해로 누가 garbage/교육 타겟 행동 가능. **Codex 2R**: CONDITIONAL_GO(M-Q4 GST·SH/A-Q2 batch_n/A-Q5 confidence)→**DEPLOY_SAFE M=0**. pytest test_partner_reliability 8/8(invariant·GST·SH·batch·raw 합산·confidence·process)+회귀 24 passed 1 skip. **🟢 배포 완료**(health v2.43.0). ⚠️ VIEW 비교 주석: by_model_process↔reliability-summary 는 GST·SH 제외 scope 차이(meta.scope echo). ⏭️ **다음 = VIEW FE(별 repo): ① #93 `usePartnerReliability`→PartnerReliabilityCard mock→실데이터(히트맵·종합막대·추이, batch 회색표기·trend_excluded 아님 batch) ② #92 useTaggingCoverage 필터 ③ reliability 게이트(v2.41.0) 가중평균 제거→endpoint 소비** / #87 Phase 2b. 설계: AGENT_TEAM_LAUNCH.md § FEAT-PARTNER-RELIABILITY.)
> 이전: 2026-06-15 KST (✅ **v2.42.1 — FIX-INSTANT-WHITELIST-SELF-INSPECTION-REMOVE: one-click whitelist 자주검사 2개 제거**. BE only patch, read-only, 상수 정정, migration 0. 사용자 catch(#93 검토 중): "one-click은 2개야 — tank docking, si_shipment". 실측: TANK_DOCKING 104·SI_SHIPMENT 40건 전부 active NULL(진짜 즉시완료, 추적률 분모 자동 제외) / SELF_INSPECTION(MECH 자주검사) 평균 57분 즉시완료 0%·INSPECTION(ELEC 자주검사) 평균 98분 즉시완료 15% = **실작업인데 one-click 오분류** → 추적률 부당 과소. **변경**: `_INSTANT_WHITELIST = frozenset({"TANK_DOCKING","SI_SHIPMENT"})`. 6지표 미러 자동 전파(tagging-coverage·reliability-summary·partner-breakdown·data-quality·close-type-trend·partner_discipline). **⚠️ Codex Q5 핵심 결함 동반**: data-quality `trend_sql` zerotap 하드코딩(`NOT IN 4개`) → `_wl_sql` 동적 미러(f-string) 교체, 단일 미러 복원. **효과(정직화)**: 자주검사 tracked 정상 포함 → tagging-coverage MECH 34→49%·ELEC 47→58%, reliability-summary 헤드라인 53→63%(표준가능 17.6% 유지). TANK_DOCKING/SI_SHIPMENT active NULL → 추적률 영향 0(whitelist 잔류=안전망). **Codex 2R**: R1 NO-GO(Q5 하드코딩/Q3 test/Q6 주석)→R2 **DEPLOY_SAFE M=0**. pytest 6지표 회귀 93 passed 1 skip. **🟢 배포 완료**(health v2.42.1). ⏭️ **다음 = #93(협력사 데이터 입력 신뢰도 — 협력사×모델×공정 추적률 분해 신규 endpoint `GET /api/ct/partner-reliability`): 정정된 추적률 base 위 설계, A-2(분모 one-click 제외)는 정정된 whitelist 2개 기준이라 자동 정합, OPS_API_REQUESTS.md #93 참조** / VIEW FE(#92 필터 + reliability 게이트 v2.41.0). 설계: AGENT_TEAM_LAUNCH.md § FIX-INSTANT-WHITELIST-SELF-INSPECTION-REMOVE.)
> 이전: 2026-06-15 KST (✅ **v2.42.0 — Sprint 93 / FEAT-TAGGING-COVERAGE-FILTERS (#92): tagging-coverage period/reference_date/partner 필터**. BE only minor, read-only, additive, migration 0. 배경=종료 누락 분석 페이지 상단 필터(오늘/주간/월간/분기+협력사)가 auto-close-summary 카드는 다 받는데 0초탭 카드(tagging-coverage)만 월 단위 from/to라 불일치(사용자 catch). **`GET /api/ct/tagging-coverage?period=&reference_date=&partner=`**(전부 optional, 미지정=현행 월누적 back-compat). period=today|week|month|quarter(`_resolve_period_range` 재사용, auto-close 동일)+reference_date→**day 윈도우** / partner(company)→협력사 필터(**분모도 협력사 기준**, PI/QI/SI=GST 고정→FNI면 MECH/ELEC만). **⚠️ 공유 상수 회귀 방지**: reliability-summary(v2.41.0)가 `_COVERAGE_WHERE` import → **`_COVERAGE_WHERE=_COVERAGE_BASE+_COVERAGE_WINDOW_MONTH` byte 동일 유지**(reliability-summary 무변경, 회귀 0) + 신규 `_COVERAGE_WINDOW_DAY`/`_COVERAGE_PARTNER_FILTER` 별도 조각. meta echo+cache_key+route 검증(400 INVALID_PERIOD/INVALID_DATE). **Codex R1 GO M=0**. 실데이터: back-compat 월누적 / partner=FNI MECH만(PI/QI/SI=0) / **reliability-summary tracking 53·standard_ready 17.6=v2.41.0 동일(회귀 0)**. pytest test_tagging_coverage_filters 7 + 회귀(sprint90 tagging+reliability) 32 passed 1 skip. **🟢 배포 완료**(health v2.42.0). ⏭️ **다음 = VIEW FE(별 repo): ① `useTaggingCoverage({period,reference_date,partner})` 필터 연동(#92) ② CT 신뢰도 게이트 reliability-summary 연동(v2.41.0, VIEW_FE_Request.md 상단 mockup brief — 가중평균 제거→endpoint 소비)** / #87 Phase 2b. 설계: AGENT_TEAM_LAUNCH.md § FEAT-TAGGING-COVERAGE-FILTERS.)
> 이전: 2026-06-14 KST (✅ **v2.41.0 — Sprint 92 / FEAT-CT-RELIABILITY-SUMMARY (B): CT 데이터 신뢰도 게이트 재설계**. BE only minor, read-only, 신규 endpoint, migration 0. 배경=CT "데이터 신뢰도" summary 가 FE 파생 **가중평균**(`Σweight(n)·n/Σn`, 풀링 부풀림 80.6% vs 실사용 협력사×모델 13%)이라 과대평가 — 사용자 catch("품질 게이트여야 하는데 가중평균이라 과대"). 3모델(Claude+Codex+Fable) 합의 + 사용자 catch 누적(모델 기준/생산량 가중)으로 재설계. **신규 `GET /api/ct/reliability-summary`**(statistics_service.get_reliability_summary, jwt+gst_or_admin). **핵심**: ① 가중평균 폐기→**count 게이트**(표준가능=셀[모델×task×dual] 중 n≥30 AND 모델공정추적≥70% 비율) ② **생산량 가중** Σ(모델 12mo rolling 생산비중·표준가능비율, 주력 GAIA 88% 지배=정직, 표본수 가중과 별개) ③ **2조건 분리**(n_met/tracking_met/both, 게이트 0%여도 gradient) ④ 모델 기준(협력사=매트릭스 drill, Fable Q2 협력사분리는 협력사별 표준 전제라 무효) ⑤ 라벨 정정+헤드라인=추적율+force_closed 분리(A 정합). **Codex 3R**: NO-GO→CONDITIONAL_GO(Q2 partner_scope/Q4 무자료 no_ct_cells 0기여/Q5 _norm_model_prefix 단일)→**DEPLOY_SAFE M=0**. Fable 2차 brief 반박 반영. **실데이터**: 헤드라인 추적 53%(hold)/standard_ready 17.6%(가중평균 80.6%→정직화)/n_met 51.7% vs tracking_met 22.5%(병목=추적·질)/input_integrity 71.5%·force_closed 9.2%. pytest 9+1skip+회귀 44+1skip. **🟢 배포 완료**(health v2.41.0). ⏭️ **다음 = VIEW FE(별 repo, VIEW_FE_Request.md 상단 mockup brief 인계): CtAnalysisPage FE 파생 가중평균 제거→reliability-summary 소비, DataQualityGate 연결, 가중평균 카드 완전제거(Fable Q4), 드릴 계층(모델→task→협력사). mock 먼저 가능** / #87 Phase 2b. 설계: AGENT_TEAM_LAUNCH.md § FEAT-CT-RELIABILITY-SUMMARY v1~v4.)
> 이전: 2026-06-13 KST (✅ **v2.40.0 — FIX-FORCE-CLOSE-DURATION-SOURCE (A): 강제종료 duration_source='FORCE_CLOSED' 근본 정합**. BE only minor, **migration 062 동반**. 배경=강제종료(`force_close_task`)가 `duration_source` 미설정(NULL) → 입력정합(get_data_quality)에서 NULL=clean 과대 집계(force-close 199건 섞임, 81.8% 과대). 클린 코어 원칙(2026-04-20)상 강제종료=버리는 데이터인데 source 표시 누락. **fix**: migration 062 = duration_source CHECK enum 'FORCE_CLOSED' 추가(057 패턴 BEGIN/COMMIT+pg_get_constraintdef 검증, Codex R1 M-Q1 반영) + backfill(`force_closed=TRUE AND source∈(NULL,NORMAL)→FORCE_CLOSED` 멱등). admin force_close UPDATE 'FORCE_CLOSED' 기록 + statistics_service `_ESTIMATED_SOURCES` 추가. FALLBACK/PREV_DAY_CAP force-close는 cap source 보존(약불변식, Codex Q5 A). **prod 적용 실측**: force_closed=TRUE 253건→251 FORCE_CLOSED+2 FALLBACK, clean 잔존 0 → **입력정합 72.7% 정직값**. CT 표본은 v2.36.1 가드+source 제외 이중 정합. Codex R1 NO-GO(M-Q1)→R2 **DEPLOY_SAFE M=0**. pytest 5/5 + 회귀 69 passed 1 skip. **🟢 배포 완료**(health v2.40.0). ⏭️ **다음 = B 재논의(CT 신뢰도/입력정합 summary weighted-avg 과대 80.6%→headline 추적커버리지 단독+per-granularity mini-table+count-based 표준가능task비율, 3모델 합의안, 사용자와 재논의)** / VIEW MissedTaskQueueCard 교체 / #87 Phase 2b. 설계: AGENT_TEAM_LAUNCH.md § FIX-FORCE-CLOSE-DURATION-SOURCE.)
> 이전: 2026-06-13 KST (✅ **v2.39.1 — Sprint 91-FE-b: 미종료 큐 방치 기준 통일 BE**. BE only additive, migration 0. 배경=VIEW MissedTaskQueueCard 가 옛 progress 근사(제품×공정·제품 last_activity·방치무관, 협력사합 254)라 데이터종합 방치(task 127)와 불일치 — 사용자 catch. `build_open_tasks`(/open-tasks, 이미 v2.38.0 방치) additive: ① per-task `inactive_hours`(=now-마지막활동, LAST_ACTIVITY_SQL import, max(0,) 음수방어) ② `meta.by_partner`(방치 협력사별=칩) ③ `meta.buckets{h24_48,h48plus}`(방치=24h+) ④ meta basis='realtime_abandoned'+missed_task_standard 갱신. tasks[] 기존 필드 불변. Codex R1 NO-GO(M-1~M-4 미구현=설계착수)→구현→**R2 DEPLOY_SAFE M=0**(A-1 test fixture 자사-only=BACKLOG). pytest test_sprint89 34/34(신규 test_inactive_hours_and_buckets·test_manager_by_partner_self_only). **🟢 배포 완료**. ⏭️ **다음 = VIEW MissedTaskQueueCard 교체(별 repo): progress→/open-tasks, 후보=meta.count·버킷=meta.buckets·칩=meta.by_partner·리스트=tasks[inactive_hours], getSNTasks·[전체]토글·24h미만박스 제거. 잔존: 데이터종합=월방치/미종료현황=realtime방치(ADR-034 의도)** / #87 Phase 2b / 설계서 분할(truncate 재발 방지). 설계: AGENT_TEAM_LAUNCH.md § Sprint 91-FE-b.)
> 이전: 2026-06-13 KST (✅ **v2.39.0 — Sprint 90-BE-C-b/#90b: close-type-trend 주별(ISO week) 버킷**. BE only additive, migration 0. `GET /api/ct/close-type-trend?bucket=week|month`(기본 month=back-compat 완전) — week=`series[].month` ISO 라벨(IYYY-"W"IW)+`meta.bucket`+주 zero-fill(`_week_range` isocalendar↔PG IYYY-IW 동치, 연말 W53/W01 Codex 독립계산 확인). A-1 월초 경계 주 부분집계 허용(의미 정합, 문서화). Codex 실코드 R1 **DEPLOY_SAFE M=0**. pytest test_sprint90c 9/9(TC-CTT-08/09 신규+회귀 7). **🟢 배포 완료**. VIEW [주|월] 토글(선반영) meta.bucket 수신 시 자동 실데이터 전환. ⚠️ **AGENT_TEAM_LAUNCH.md 에디터 truncate 사고**(48,558줄 잘림→git 복원) — 에디터 저장 금지, **파일 분할 권장(다음 세션)**. ⏭️ **다음 = VIEW 수정 잔여(데이터종합 툴팁/B5/#91 툴팁/커버리지 연동) / #87 Phase 2b / 설계서 분할**.)
> 이전: 2026-06-13 KST (✅ **v2.38.2 — Sprint 91-FE 연장: 퇴근 다이얼로그 [모두 내작업완료]**. FE only(home_screen). 실기기 QA = 일시정지 정상 확인(Twin파파) 후 요청 반영 — completeTask(finalize=false, 내 작업만 종료·릴레이 유지) 순차, 실패(일시정지중/이미완료)는 "N건 실패" 안내. 액션 3개(나중에/모두 내작업완료/모두 일시정지). **Sprint 번호 부여**: 90-BE-D(#91 group_avg)/91(PENDING-REFINE BE)/91-FE(FE 가드) — AGENT_TEAM_LAUNCH.md 헤더 갱신(섹션 LOC: 48643/48784/48849/48935/48980/49074/49154). **공지 117 게시**(OPS 2.% unpin 규칙 준수). Netlify prod 배포. ⏭️ **다음 = VIEW 수정 6건**(① 데이터종합 openTasks "방치" 툴팁 ② MissedTaskQueueCard B5 근사→정확 ③ #91 "그룹평균(자사포함)" 툴팁 ④ #90 CloseTrendChart 비교 실데이터+윈도우라벨 ⑤ TaggingCoverageCard·ZeroTap 연동+배지제거 ⑥ CT값 하락 자동) / #87 Phase 2b / work_log 인덱스.)
> 이전: 2026-06-13 KST (✅ **v2.38.1 — FEAT-OPS-FE-PENDING-GUARD: OPS app 미종료 가드 FE**. FE only patch(home_screen + manager_pending_tasks_screen + admin_options_screen), BE 0. A 퇴근 다이얼로그(open_tasks[] 소비 — [모두 일시정지] 순차 pause·`TASK_ALREADY_PAUSED`=idempotent 성공·실패 요약/[나중에], 성공 SnackBar 분기) / B 방치 뱃지(시작시간 아래 metadata Row — 무활동 N시간 48h+ danger·24~48h warning + ⏸ 일시정지 작업자, null 방어) / C 강제종료 다이얼로그 "⚠️ 무활동 N시간 경과" 문구(단건 inactiveHours, 일괄=Codex M-3 미구현). Codex R1 NO-GO(M=3: M-1 설계서 경로오탐 — backend/ 오기록 catch 후 본 파일 이전 / M-2 idempotent / M-3 일괄삭제) → 정정 → 구현. flutter build web GREEN + **Netlify prod 배포 완료**(gaxis-ops.netlify.app). 🟢 **실기기 QA 위탁**(퇴근 다이얼로그/pause/뱃지). ⏭️ **다음 = 실기기 QA 피드백 / VIEW B5(카드 정확값) / work_log 인덱스 / #87 Phase 2b**. 설계: AGENT_TEAM_LAUNCH.md § FEAT-OPS-FE-PENDING-GUARD.)
> 이전: 2026-06-13 KST (✅ **v2.38.0 — FEAT-PENDING-DISCIPLINE-REFINE: 미종료="방치" 단일 기준 + 0초탭 force 제외 + 퇴근 open_tasks**. BE only minor, read-only+additive, migration 0. 배경=BAT 매니저가 미종료 메뉴에서 갓 시작 task까지 일괄 강제종료(98건). **방치 기준**(신규 `services/pending_task_standard.py`) = canonical + 활성(미pause) 세션≥1 + 마지막 활동(시작/**재개**) 24h+ — resume은 work_pause_log.resumed_at만 update라 GREATEST(MAX(wsl.started_at),MAX(wpl.resumed_at)) 필수(Codex M-1/M-2). **4표면 통일**: 데이터종합 openTasks(월 anchor 유지) / discipline `/open-tasks` 큐 / 대시보드 `tasks/pending/grouped` / OPS 메뉴 `tasks/pending`(+additive `last_activity_at`/`inactive_hours`/`has_paused_worker`). **zerotap force_closed=FALSE**(CT v2.36.1·coverage 정합). **audit 태그**: 활성 task 강제종료 시 close_reason `[활동중 종료]` prefix(차단X 추적O — M-3 API가드는 사용자 정책 기각, view단 실수방지 노선). **checkout 'out' `open_tasks[]`** additive(OPS app 토스트 BE). Codex R1 CONDITIONAL_GO→R2 **DEPLOY_SAFE M=0**. pytest 신규 test_pending_abandoned_standard 6/6 + 회귀 sprint79 25·sprint89 32 = 63/63 GREEN(sprint79 PG2 seed 방치기준 갱신). **🟢 배포 완료**(health v2.38.0). ⏭️ **다음 = FEAT-OPS-FE-PENDING-GUARD 구현(설계+Codex R1 정정 완료 — AGENT_TEAM_LAUNCH.md § 참조, 잔여 M=0 구현 진입 가능: A 퇴근 토스트(pause 400=idempotent 성공)/B 뱃지 metadata Row/C 단건 다이얼로그 문구·일괄삭제) / VIEW 카드 근사→정확값(B5, work.py 분할 후) / work_log 인덱스 migration(BACKLOG) / #87 Phase 2b**. 설계: `AGENT_TEAM_LAUNCH.md` § FEAT-PENDING-DISCIPLINE-REFINE(v2+as-built).)
> 이전: 2026-06-12 KST (✅ **v2.37.0 — #90 협력사 마감유형 추이 + #91 협력사 매니저 group_avg 노출**. BE only minor, read-only, migration 0. VIEW OPS_API_REQUESTS #90/#91. **#90** `GET /api/ct/close-type-trend?from=&to=&partner=&group=`(신규 `close_type_trend_service.py` + ct_analysis route, gst_or_admin_required): 협력사×그룹×월×{auto,zerotap,force} flat series → VIEW CloseTrendChart [비교] 오버레이 실데이터. zerotap=90-BE-B 정의, partner=MECH/ELEC(TMS(M)/(E)) GST+SH 제외, 빈 달 zero-fill. pytest 7/7. **#91** `partner_discipline_service._group_avg` `is_global` 분기 제거→`cand=partners`(자사 포함 group-wide avg): 협력사 매니저도 그룹평균 노출(3곳서 자사제외 2<3 영구 suppress 정정). 역추론 가드 `len(vals)≥3`(자사 외 미지수≥2, 개별 역산 불가; 1곳 누락 시 suppress). admin·협력사 동일 값, VIEW 변경 0. pytest test_sprint89 32/32. Codex #90·#91 각 R1 DEPLOY_SAFE M=0 + advisory 4 BACKLOG. **🟢 배포 완료**(health v2.37.0). ⏭️ **다음 = 값 정합성 확인(사용자 요청) / VIEW FE(#90 mockup→실데이터 compareCloseSeries / #91 "그룹평균(자사포함)" 툴팁) / #87 Phase 2b**. 설계: `AGENT_TEAM_LAUNCH.md` § Sprint 90-BE-C / § #91.)
> 이전: 2026-06-12 KST (✅ **v2.36.1 — FIX-CT-FORCE-CLOSE-POLLUTION: CT 표준 강제종료 오염 정정**. BE only patch, read-only, migration 0. **버그**: 강제종료(`force_close_task`)가 v2.22.0/v2.28.0에서 `compute_task_work`로 ct/duration 계산·저장하나 `duration_source` 미설정(NULL) → CT clean 필터(`_CLEAN_CORE` duration_source 기반) 통과 → CT 표준 표본 오염. 클린 코어 원칙(2026-04-20 "강제종료=버리는 데이터")과 drift. **실측(2026-05~, basis=ct 1,244건)**: 강제종료 128건(10.3%) 오염, WASTE_GAS_LINE_1 median 1.40h→2.98h(+112%). **Fix**: `statistics_service.py _CLEAN_CORE`에 `AND COALESCE(td.force_closed,FALSE)=FALSE` 1줄(→ #82/#83/training 전파). **자동마감 유지**: AUTO_CLOSED_BY_* 65건 work_completion_log 전건 보유=작업자완료(NORMAL_COMPLETION)라 제외 X(Codex Q3 실데이터 0건 확인). Codex R1 NO-GO(Q3)→실데이터 해소→**R2 DEPLOY_SAFE M=0**. pytest 신규 `test_ct17_force_close_excluded` + CT회귀 = 격리 18/18 GREEN(8분 full-run 실패는 transient 연결 끊김). **🟢 배포 완료**(health v2.36.1). read-only·저장값 무변경(man-hour 리포트·대시보드 영향 0)·factory/dashboard 무영향. VIEW `CT_VISUALIZATION_GUIDE.md` §8-7 #1 정정(force-close 자연제외 ❌→오염 ✅). 잔여 INFO: admin대행 2건 / checklist orphan duration_source(현재 0건). ⏭️ **다음 = VIEW CT/종료누락 표준값 정정 자동 반영 / #87 Phase 2b(checklist)**. 설계: `AGENT_TEAM_LAUNCH.md` § FIX-CT-FORCE-CLOSE-POLLUTION.)
> 이전: 2026-06-11 KST (✅ **v2.36.0 — Sprint 90-BE/90-BE-B: 태깅 커버리지/0초탭 드릴다운 + 마감 유형 월별 추이 zerotap**. BE only minor, read-only, migration 0. AXIS-VIEW `종료 누락 분석` mockup 2종 실데이터. **① 90-BE** `GET /api/ct/tagging-coverage`(신규 `tagging_coverage_service.py`+`ct_analysis.py` route, `gst_or_admin_required`): 공정별(MECH/ELEC/PI/QI/SI) 추적율/0초탭 + `well_tracked_pct`(추적율≥80% serial) + `zero_tap_tasks` 드릴다운(task별 partner share). 분모=완료+active NOT NULL+applicable+force_closed=FALSE(자동마감·admin대행=**미추적 포함**, 사용자 결정 — mock PI/QI/SI 고미추적 신호). 3분류 oneClick>zero_tap(active≤1 OR close_reason)>tracked(active>1 AND close_reason NULL). PI/QI/SI=GST 단일, DUAL per-row, share largest-remainder. Codex **4라운드 M=3→2→1→0 GO**. pytest 16/16+CT회귀 16/16. **② 90-BE-B** `data-quality.auto_close_trend[]`에 `zerotap`/`zerotap_rate` additive(`statistics_service.py` trend 2줄, CloseTrendChart용, 신규 route X=DRY, zerotap 정의=90-BE 동일). Codex DEPLOY_SAFE. pytest 17/17(test_ct12+test_ct09 invariant 불변). **🟢 배포 완료**(health v2.36.0). **advisory(BACKLOG)**: A-1 SELF_INSPECTION 자동마감 시 월별 auto>zerotap 역전(VIEW tooltip) / A-2 private `_INSTANT_WHITELIST` import. ⏭️ **다음 = VIEW FE 2 card 연동(별 repo, TaggingCoverageCard→/tagging-coverage / CloseTrendChart·ZeroTapKpiCard→data-quality.zerotap, 배지 제거) / #87 Phase 2b(checklist) → 계획·납기(Sprint 87-BE)**. 설계: `AGENT_TEAM_LAUNCH.md` § Sprint 90-BE/90-BE-B.)
> 이전: 2026-06-10 KST (✅ **v2.35.0 — #87 규율 대시보드 보정 (미종료 월단위·기준통일 + 중복지표 교체)**. BE only minor, read-only, migration 0. 사용자 catch 보정 묶음: ① **중복지표 교체** `checkinNoTag`(taggingRate와 중복) 폐기 → `checkoutMiss`(퇴근미체크율, reference_only 참고/평가제외, #86 checkout_status 월 가중집계). ② **openTasks 월단위** `started_at`∈해당월+미완(autoClose 월 anchor 일관). ③ **미종료 기준 통일** 정식 `/tasks/pending/grouped` 정합(is_applicable=TRUE+force_closed=FALSE+TEST 제외) — openTasks+큐. ④ **공통 하이진** autoClose/zeroTap/universe is_applicable+TEST. ⑤ **큐 realtime 유지** summary openTasks(월)≠큐(realtime) 의도 + meta basis. ⑥ **비협력사 제외** `_EXCLUDED_PARTNERS`=GST+**SH** → 6 정식 협력사. Codex 중복지표→미종료 설계 R1 GO→실코드 R2 DEPLOY_SAFE M=0. pytest 32/32 GREEN + 실스모크. **🟢 배포 완료**(health v2.35.0). ⏭️ **다음 = Phase 2b(checklist) → 계획·납기(Sprint 87-BE)**. VIEW FE = checkinNoTag→checkoutMiss 키 + reference_only/openTasks_basis meta + 월단위 의미 렌더.)
> 이전: 2026-06-10 KST (✅ **v2.34.0 — #87 협력사 규율 대시보드 Phase 2a (Sprint 89-BE)**. BE only minor (partner_discipline_service + admin_discipline, read-only, migration 0). Phase 1 placeholder 3개 실측 + trend endpoint. **taggingRate**(GST 현장 출근 분모 ÷ work_start_log 태깅, 월 DISTINCT) / **checkinNoTag**(onsite−tagged) / **zeroTap**(active≤1 미추적 ÷ substantive, one-click is_instant_whitelisted 제외 — 실데이터 BAT/MECH 0.66·ELEC ~0, CT 발견 일치) / **`GET /api/admin/discipline/trend?months=N`**(월별, openTasks 제외, 매니저 자사만). union universe(task∪worker partner)+provenance. group_avg 불변식(raw None 가능 ⟺ missing None). **Phase 1 버그 2건 동반 정정**: `_PARTNER_SQL` NULLIF(,'GST')(GST 자체수행 noise 제외) + open-tasks 타사/NULL 작업자 실명 마스킹(M-Q8/NULL). Codex 설계 GO→실코드 R2 M=2→**R4 DEPLOY_SAFE M=0**(M-Q5→A 2자합의). pytest 31/31 GREEN + 실데이터 스모크. **🟢 배포 완료**(health v2.34.0). ⏭️ **다음 = Phase 2b(checklist, applicable resolver 추출, 별 Codex) → 완료 후 계획·납기(Sprint 87-BE)**. VIEW FE = placeholder→실측 렌더 + trend 차트.)
> 이전: 2026-06-10 KST (✅ **v2.33.0 — #87 협력사 규율 종합 대시보드 Phase 1 (Sprint 89-BE)**. BE only minor (신규 `services/partner_discipline_service.py` + `routes/admin_discipline.py` + blueprint, read-only, migration 0). endpoint 2개: `GET /api/admin/discipline/summary?month=`(partner×group + 지표 envelope) + `GET /api/admin/discipline/open-tasks`(미종료 즉시조치 큐 + repeat 30d/2). **Phase 1 실측 = openTasks(미종료)+autoClose(자동마감)+group_avg(peer-only k≥3)**, Phase 2 4지표(taggingRate/checkinNoTag/zeroTap/checklist) = placeholder(`available=false`). 권한 `@manager_or_admin_required`+`resolve_company_scope` try-밖(#86 누수 차단). 공정성: BE는 raw+group_avg만(등급=VIEW), MECH/ELEC only, **협력사 매니저=자사 row만+group_avg suppress(현 3사 peer<3)**, 추적율<70% grade_eligible=false. `_PARTNER_SQL`=NULLIF(TRIM,'')(빈값 제외). `statistics_service.is_instant_whitelisted()` 공개 helper(A3). **근태(출근율·퇴근미체크) 협력사 평가 제외**(출근=태깅율 분모로만). Codex 설계 R1 M=8→R2 조건부 GO / 실코드 R3 M=1(_PARTNER_SQL 빈값)→fix→**R4 DEPLOY_SAFE M=0**. pytest 신규 22 GREEN, 회귀 0. **🟢 배포 완료** (health v2.33.0). namespace 확정 = `/api/admin/discipline/*`(VIEW #87 doc 정정 커밋). ⏭️ **다음 = #87 Phase 2 (taggingRate/checkinNoTag/zeroTap/checklist/trend) → 완료돼야 계획·납기(Sprint 87-BE) 진행**. 설계: `AGENT_TEAM_LAUNCH.md` § Sprint 89-BE.)
> 이전: 2026-06-10 KST (✅ **v2.32.2 — #88 출하상세 출하 처리자(shipment_worker) 1필드** (additive, LEFT JOIN LATERAL SI_SHIPMENT 완료자, 엑셀=null, pytest 32/32) / ✅ **v2.32.1 — 출고처리 실패 메시지 명확화** (pre-transition 제품 task seed 부재 = 출고 실패 정상, 메시지만 한글 친화, 자동seed 보류 BACKLOG, pytest 15/15) / ✅ **v2.32.0 — #86 협력사 데이터 RBAC + 근태 checkout_status (Sprint 88-BE)** (`jwt_auth.resolve_company_scope()` SSoT — admin/GST 전체·협력사 매니저 자사 exact·company 없는 매니저·None→403 누수 차단 + errorhandler / 근태로직 `hr_attendance_service.py` 추출 admin.py 2626→2456 / checkout_status not_started·working·missed·done + by_work_site 미체크율. Codex PR1 R3 DEPLOY_SAFE. A′=admin route 재사용. VIEW 후속=가드 제거+렌더).)
> 이전: 2026-06-08 KST (✅ **v2.31.0 — #83 CT 협력사×모델×task×dual 분해 집계 (partner-breakdown)**. BE only minor (신규 `GET /api/ct/partner-breakdown` read-only + statistics_service `get_partner_breakdown` 신규 함수, DB/migration 0). CT 분석 본질=협력사 평가인데 현 task-stats는 task-pooled(partner 차원 전무) → VIEW CtPartnerModelMatrix·협력사 평가 페이지 mock. **검증 6개**(5월+ basis=ct): 4축 57% n<5→rollup partner×task 72% 쓸만(병행 필수) / 즉시완료율 **BAT 61%** vs FNI 11%(태깅 KPI) / vs_std TMS(E) PANEL 0.65·P&S 1.07 / **BAT 61% 미추적=생존편향** / 모델 편중 45~56% / **DUAL +18%**(현업 ~1.5배 병렬제작)→vs_std (task,dual) 표준. **변경**: `{rows(4축), rollups{partner_task,partner_model}, meta}`. 트러스트 윈도우(S-1)·basis=ct(S-2)·dual(#81)·partner 정규화(`_COMPANY_SQL` v2.20.11 TMS(M)/(E)) 재사용. rollup=독립 pooled median(raw합산 ❌) + **vs_task_standard_ratio**((task,dual) 표준, 표준 n<5→null+insufficient_standard+standard_n) + **tracking_coverage**(속도+추적 짝, 생존편향 방어) + instant_completion_rate(one-click 화이트리스트 TANK_DOCKING/SI_SHIPMENT/SELF_INSPECTION/INSPECTION 제외) + category_mix(GST 다중). 게이트 n<5 reject/5~29 prov/30+ std. Tukey n≥30만. **BE composite 미합성**(즉시완료율·coverage·vs_std 단독 노출, 가중=VIEW). meta garbage_excluded=false·dag_ver='none'(S-3 전 명시). **Codex**: 설계 R1 M=3(rollup 독립median/vs_std n<5/partner rollup 키)→R2 GO / 구현 R1 M=1(rollup raw-pull+Python 버킷화 vs SQL GROUP BY)→**M→A 2자 합의**(median 시맨틱 정확·~1500행 성능 무관·FE 재집계 금지 충족)→**DEPLOY_SAFE**. pytest 신규 test_partner_breakdown 14(rollup 독립median≠raw합산·vs_std (task,dual)·TMS정규화·composite 미합성) + 회귀 33(S-1 18·S-2 15, task-stats 불변) GREEN. **배포 검증**: 실데이터 스모크 rows 79/partner_task 35, TMS(E) PANEL vs_std 0.65, dual 분리, 속도+coverage 짝. read-only·additive·회귀 0. 설계: `CT_PARTNER_BREAKDOWN_DESIGN.md`. **잔여 A**: rollup SQL GROUPING SETS 전환(성능 무관, 별 BACKLOG). **⏭️ 다음**: VIEW FE partner-breakdown 연동(CtPartnerModelMatrix/협력사 평가 mock→실데이터) / **S-3 ⓒ DAG 선후행 garbage 제외 + 무결성 KPI**(act=0율·선후행 모순율, side매칭, TANK_DOCKING anchor 금지, PI가압≠TMS가압) — BACKLOG `📊 CT 분석 후속` 참조.
> 이전: 2026-06-08 KST (✅ **v2.30.0 — Sprint S-2 (ⓑ) 진짜 CT across-worker union + 동시작업자 (VIEW #82 ⓑ)**. BE minor (migration 061 ct_time_minutes 컬럼+백필 + task_detail/admin/statistics_service/ct_analysis). 현 active/duration 둘 다 M/H(across-worker SUM)이지 CT 아님 → 진짜 CT = 작업자 정제세션 across-worker **UNION**(벽시계, 겹침 합산 안 함). **① 검증**: union dry-run(M/H합=저장 active 일치) + **다중작업자 ≠ 병렬**(세션 겹침=병렬만 CT<M/H): PANEL 4명동시 1885→944(2.0x)/UTIL 6명교대 409→410(1.0x), 정량화 ELEC 49%/MECH 13%/전체 ~9% 병렬. **변경**: migration 061 `ct_time_minutes`(nullable)+백필=`(session∩BH−pause∪식사)` per-worker → across-worker `range_agg` union → `LEAST(FLOOR(union), active)`(A-1 불변식 CT≤M/H), active=0→ct=0(LEFT JOIN COALESCE)/active NULL(TANK_DOCKING)→ct NULL, 멱등. `compute_task_work` `{manhour,active}`→`{,,ct}` + 완료 4경로 ct 동봉. `statistics_service` `basis`=duration\|active\|**ct**(ct>0 모집단) + `effective_concurrency_median`(instance active/NULLIF(ct,0) median) + `avg_workers` + coverage(active,ct) 일반화 + meta `n_used`(Tukey전)/`n_sample`(Tukey후). S-1 상속(컷오프 5/1·from/to·Tukey·provisional·캐시키). **Codex**: 설계 R1 M=3(concurrency ratio-of-medians/basis=ct coverage/엣지TC)→R2 M=1(coverage denominator)→R3 GO / 구현 R1 M=2(n_used Tukey후 오염·백필 active=0→ct NULL 방치)→fix→**재검증 DEPLOY_SAFE/GO M=0**. pytest 신규 test_sprint_s2_ct_union 15(부분겹침 active240/ct180·완전겹침 ct=active/2·릴레이·캡·멀티데이·basis=ct·effective_concurrency·백필 set-based) + 회귀 67(active 16·S-1 18·relay 38, man-hour/active/duration 불변) GREEN. **배포 검증**(운영): migration 061 적용·백필 갭 0·CT>active 위반 0, UTIL 릴레이 CT=M/H 5.5h / PANEL 병렬 CT<M/H. 설계: `CT_S2_TRUE_CT_UNION_DESIGN.md` v2 + AGENT_TEAM_LAUNCH.md. **⏭️ 다음**: VIEW FE basis=ct 연동(별 세션) / **S-3 ⓒ DAG 선후행 garbage 제외 + 무결성 KPI**(act=0율·선후행 모순율, side매칭, TANK_DOCKING one-action anchor 금지, PI가압≠TMS가압) — BACKLOG `📊 CT 분석 후속` 참조.
> 이전: 2026-06-08 KST (✅ **v2.29.1 — get_data_quality 트러스트 윈도우 통일**. data-quality도 옛 90일→`CT_TRUST_START_MONTH` 윈도우 통일(duration_source 분포만, 추이 6mo·교육 전기간 유지). meta lookback_days→window. pytest 5 GREEN).
> 이전: 2026-06-08 KST (✅ **v2.29.0 — Sprint S-1 (ⓐ) CT basis=active + 미추적 제외 + 신뢰컷오프 + 월범위 (VIEW #82 ⓐ)**. BE only minor (statistics_service.py + ct_analysis.py, DB/migration 0 read-time). **14개 운영 쿼리 검증**: ① 현 "CT"=M/H(across-worker SUM) ② **act=0 = 미추적**(작업자 시작=완료 즉시태깅, work_completion_log d=0): MECH 배관 ~50%/ELEC ~10%, 원본 로그 추적 확인(계산정상·데이터부재), act 분포 이봉형 ③ act=0 제외 시 MECH CT 0.5h→2.2h ④ 미추적율 **BAT 59% vs FNI 7%**(완료율 둘다 100%가 숨김) ⑤ duration 058 미적용→6월+만 신뢰. **변경**: `GET /api/ct/task-stats` 신규 `basis`(duration기본\|active)+`from`/`to`(YYYY-MM KST 반열림), **`period` 제거**(M-6). basis=active→`active_time_minutes>0`(미추적 제외). 신뢰 기준 `CT_TRUST_START_MONTH='2026-05'` 단일(사용자 확정), duration은 meta `duration_stale_before`(6월 이전 포함 시). 생존편향 `tracking_coverage_by_partner`(basis=active 전용, 표시통계 동일 슬라이스 act>0·Tukey 전 — FNI 0.71/BAT 0.34). basis별 n + Tukey base_n/clipped_n + n<30 provisional + 캐시키 전파라미터 + 입력검증 400. **Codex**: 설계 R1 M=4→R2 CONDITIONAL_GO(M-5 coverage슬라이스/M-6 period)→구현 R1 M=2(duration_stale 무조건/regex 2026-13)→**재검증 DEPLOY_SAFE/GO M=0**. pytest 신규 test_sprint_s1_ct_basis 18 + 회귀 test_sprint85 16 GREEN(test_ct07 메타키·test_ct14 period제거 수정). 실데이터 스모크 MECH active 중앙 UTIL_1 6.6h. 회귀 0(additive, basis=duration 보존). 설계: `CT_S1_BASIS_ACTIVE_DESIGN.md` v3 + AGENT_TEAM_LAUNCH.md § Sprint S-1. **⏭️ 다음**: VIEW FE basis=active 연동(별 세션) / **S-2 ⓑ 진짜 CT union**(새 ct_time_minutes 컬럼+migration, 다중작업자 24~28%) / **S-3 ⓒ DAG 선후행 garbage 제외 + 무결성 KPI**(act=0율·선후행 모순율, side매칭, TANK_DOCKING one-action anchor 금지).
> 이전: 2026-06-07 KST (✅ **v2.28.2 — active-time 식사시간만 제외 정정**. 사용자 catch: Sprint 86(v2.28.0) active-time이 4개 휴게(오전 10:00-10:20/점심 11:20-12:20/오후 15:00-15:20/저녁 17:00-18:00) 전부 제외했으나, **오전·오후 20분 휴게는 작업시간 인정** = **식사(점심·저녁)만 제외**. `compute_task_work` breaks_day = `[11:20-12:20, 17:00-18:00]` 만 + `migrations/060_active_time_meals_only_rebackfill.sql` active_time 전체 재백필(OVERWRITE, IS NULL 가드 없음, duration_minutes 불간섭). 일 제외 160→120분, active 상승(PANEL 6.1→6.7h / man 8.9h, active≤man 유지). pytest active 12 GREEN(AT-03 320→360/AT-04 220→240/AT-15 280→300). 배포 시 migration 060 자동 재백필. **⏭️ 다음**: VIEW FE active 우선 표시(별 세션).
> 이전: 2026-06-07 KST (✅ **v2.28.1 — CT task-stats `dual` 파라미터 (DUAL/단일 분리, VIEW #81)**. CT ②(IQR)가 DUAL/단일 합산 → 작업시간 2배 차(PANEL 단일 5.6h vs DUAL 11.8h) 표준 왜곡 → `GET /api/ct/task-stats?dual=dual\|single`(미지정=합산 하위호환). `_dual_clause()` = `p.model ILIKE '%DUAL%'`(포함 — iVAS GAIA-I DUAL PUMP RACK 3CH 등 중간 DUAL 명 정확, 요청안 %DUAL 접미사 보강). meta dual_scope. 응답 스키마 불변(VIEW v1.59.x 선반영, BE 미적용 시 무해). BE 2파일(statistics_service+ct_analysis), DB 0. 운영 PANEL 단일 man 5.6h/active 4.0h vs DUAL 11.8h/7.4h. pytest CT-16(dual)+CT-14 GREEN. 연계 active-time=v2.28.0 완료. **⏭️ 다음**: VIEW FE 후속(active 우선 표시 + dual 토글, v1.59.x 이미 선반영) / VIEW #81 로드맵 잔여(협력사 task breakdown Sprint 72 · 적응기간 cutoff).
> 이전: 2026-06-07 KST (✅ **Sprint 86 (FEAT-ACTIVE-TIME-PURE-WORK) v2.28.0 완료 — 순수 작업시간 active-time + CT 표준 격상**. man-hour(현 CT 표준)는 세션−수동pause라 휴게(160분/일)·영업시간 밖 idle 미차감 → 주말/도킹 task 부풀음. **active-time = Σ_w FLOOR(GREATEST(0, len(session∩영업창) − len((수동pause∪휴게)∩session∩영업창)))** 순수 작업시간. 영업창=attendance[MIN(in),MAX(out)] 우선(86%)/fallback 평일[08,20]·주말[08,17] KST. 휴게=admin 표준 시간표(일 160분). 저장 LEAST(active,duration)→active≤man-hour. **신규** `task_detail.py compute_task_work()`(man-hour+active 단일 CTE, PG16 multirange)+기존 compute_task_manhour .manhour 위임 / `migrations/059_add_active_time.sql`(컬럼+백필, 058=v2.22.0 예약 회피) / 완료 4경로 active 동봉(complete_task_unified·auto_close_relay_task·force_close_task·force_complete_task) / statistics_service active_*_hours + meta. **man-hour(duration_minutes) 불변(additive)**. **Codex 6라운드**: 설계 R1~R4 NO-GO→R5 GO / 구현 R6 NO-GO(M-1 force_complete raw-elapsed 우회→compute_task_work 정합 / M-2 active_available Tukey비율→clean 모집단 비율)→해소. **검증**: 운영 SQL man-hour=배포 일치(238673 둘다 1071)+active≤man-hour 위반0, 백필 set-based=per-task 일치(87480→265), active 중앙 PANEL 8.9→6.1h/CABINET 5.2→3.9h. pytest active 12(AT-01~16)+회귀 relay 38(mock 4건 갱신)·CT 15·duration/v2.15.16/hotfix04 GREEN. **⏭️ 다음**: VIEW FE 후속(별 세션) active 우선 표시 + man-hour 비교 / 배포 시 migration 059 자동 백필. 설계: AGENT_TEAM_LAUNCH.md § Sprint 86.
> 이전: 2026-06-05 KST (✅ **Sprint 85 (FEAT-CT-ANALYSIS-HUB-BE-MVP) v2.27.0 완료 — CT 분석 허브 BE 연동 MVP**. VIEW CT 분석 페이지(`app/src/pages/ct/` 13 컴포넌트 4섹션, mockData.ts 계약) mock → OPS BE 실데이터 연동. MVP = **①데이터신뢰도 + ②CT표준(IQR, man-hour)** 만 (③ M/H·병렬·협력사 = 관리자 preview skeleton / ④ M/M·APS = 미구현). 결정 동결 `CT_ANALYSIS_ROADMAP §15`. **신규 2파일**: `statistics_service.py`(`get_task_ct_stats` ② + `get_data_quality` ①) + `routes/ct_analysis.py`(`GET /api/ct/task-stats?period=last_90d&model=&category=` + `GET /api/ct/data-quality`, `@jwt_required+@gst_or_admin_required`) + blueprint 1줄. **핵심 정합**: box plot = `percentile_cont(duration_minutes/60)` = v2.22.0 interval-union man-hour SSoT (목업 `mean×인원` 이중계산 폐기). clean = `duration_source IS NULL OR 'NORMAL_COMPLETION'` + TMS(M)·TEST 제외 + Tukey 1-pass. 카테고리 Σmedian 폐기 → pooled_median. KST 월경계/교육 cut(2026-06-02 literal). **Codex 3라운드**: 설계 R1 NO-GO(M=5) → R2 GO(M=0/A=2) → 구현 R3 **DEPLOY_SAFE/GO(M=0/A=4)**. pytest 15/15(CT-01~15 seed) + 회귀 test_factory 21. 운영 스모크 ② 17 task(§14.2 일치)/① 판넬 18.9h→7.0h. DB·migration 0 read-time 회귀 0. **⏭️ 다음 = VIEW FE 후속(별 세션)**: mock→API 교체 + MVP 제외 컬럼(man_hours/기준대비) 숨김 + ③ skeleton badge + ④ 보류. 설계: AGENT_TEAM_LAUNCH.md § Sprint 85.
> 이전: 2026-06-05 KST (✅ **Sprint 84 (FEAT-FACTORY-PHASE-1-2-PROGRESS) v2.26.0 완료 — 생산현황 상세 1차/2차 마일스톤**. 생산현황 상세 진행률을 손님용 rollup + **1차(가압 PI까지)/2차(마무리 SI) 마일스톤** 그룹핑. monthly-detail 응답에 `phase` 필드 additive (Sprint 83 `_compute_stage_completion` 재사용). 1차완료=`pi` / 2차완료=`si`(SI_FINISHING 기준). ⚠️ QI app 미입력(검사자동화시스템 별 시스템, 추후 API) → 2차 진행률 **SI binary**, QI 연동 시 키 1개 추가로 자동 확장. 신규 helper `_phase_pct(stages,keys)`(None 제외) + `_build_phase(sc)` + `_compute_stage_completion` 0-applicable stage → `None` 일반화(non-standard 모델 분모 정확). **Codex 라운드 1 NO-GO(M=2) → 라운드 2 GO(M=0/A=1)**: M-2(p2_pct SI_SHIPMENT-only 50% 오염)→`_PHASE2_KEYS=['si']` binary / M-3(non-standard absent stage False 분모 왜곡)→present_map None / A-1(QI 연동 시 SI reached를 SI_FINISHING 기준 제한)→BACKLOG `FEAT-FACTORY-PHASE2-QI-SI-REACHED-LIMIT-20260605`. pytest 39 GREEN(sprint83 11 + sprint84 8 PH-01~06+PH-04b+PH-05 + test_factory 19 회귀 0). 운영 W23 by_stage 불변, phase 분포 34건(2차완료 22/2차진행중 5/1차진행중 7). BE only `factory.py` additive 2필드+helper 2개, 기존 필드/엔드포인트 무변경 회귀 0. **VIEW 후속**(별 세션): progress_pct(rollup) 큰 바 + phase 마커/뱃지 렌더. 설계: AGENT_TEAM_LAUNCH.md § Sprint 84.
> 이전: 2026-06-05 KST (✅ **Sprint 83 (FEAT-FACTORY-COMPLETION-ROLLUP) v2.25.0/v2.25.1 완료 — 공장 대시보드 공정별 완료율 정합**. 공정별 완료율이 `completion_status` 옛 플래그(lag) 봐 저평가(TM 19 vs 3 / 출하완료 12건 상위 누락 / SI=출하묶음) → 실제 task 완료 + 하위완료→상위 cascade rollup(옵션 B 체크리스트 무관, SI=SI_FINISHING) 재계산. BE only `factory.py` read-time(근본 data·DB·migration 0), 대시보드 보여주기만(생산현황/실적/상세 무변경). 신규 `_compute_stage_completion`+`_progress_from_stages`, `_get_task_progress_by_serial` 무수정 별도 helper. 운영 W23 mech 0→82.4 / tm 10→83.9 / GBWS-7163 100%. **Codex R1 GO(M=0/A=3 반영)**. pytest test_sprint83 10 GREEN(CR-01~10) + 회귀 test_factory 21. 토글 미채택(글로벌=per-user 불가). 후속 BACKLOG `FEAT-FACTORY-ACTUAL-COMPLETION-VIEW`(실제값 표시 위치 고민). 설계: AGENT_TEAM_LAUNCH.md § Sprint 83.
> 이전(진입점): ⏭️ **다음 작업 = Sprint 83 구현 (FEAT-FACTORY-COMPLETION-ROLLUP, v2.25.0) — 설계+Codex GO 완료, 바로 구현 진입**). **진입점**: `AGENT_TEAM_LAUNCH.md § Sprint 83 — FEAT-FACTORY-COMPLETION-ROLLUP-20260605` (§1~7). **본질**: 공장 대시보드 공정별 완료율이 `completion_status` 옛 플래그(lag — TM 19 vs 3 / 출하완료 12건 상위 플래그 통째 누락)를 봄 → **실제 task 완료(app_task_details) + 하위완료→상위 cascade rollup(옵션 B, 체크리스트 무관)** 로 재계산. **근본 data 무변경(read-time, DB/migration 0)**. 대시보드=보여주기(손님용)만 적용, 생산현황/실적/S/N상세는 무변경. **Codex R1 GO(M=0/A=3 반영)**: A-3 DUAL TM=applicable TMS 전부 완료 / A-5 `_get_task_progress_by_serial` 생산현황 공유 시 수정금지→신규 별도 helper 분리 / A-7 CR-09(DUAL)/CR-10(PI cascade) TC. **구현**: factory.py 신규 `_compute_stage_completion` helper + 4 적용지점(get_weekly_kpi by_stage L478 / completion_rate _calc_progress L123 / get_monthly_detail stages L364-367) + pytest CR-01~10 + 회귀 test_factory + v2.25.0 배포 + T+ 검증(GBWS-7163 100%). **운영 검증 끝남**: W23 34개 / 토글 제외 확정 / BACKLOG FEAT-FACTORY-ACTUAL-COMPLETION-VIEW-20260605(실제값 표시 위치 고민) 등록. **선행 완료**: ✅ v2.24.1 quarter period.
> 이전: 2026-06-04 KST (✅ **v2.24.1 — auto-close-details quarter period 지원 (#80 후속)**. VIEW catch: 강제 종료 분포도 분기 매트릭스 ↔ 상세 패널 불일치. 원인: details period 화이트리스트가 summary 와 비대칭(`_VALID_PERIODS_DETAILS` quarter 누락 → `period=quarter` 400) → VIEW month 다운그레이드 → 6월 BAT 거의 0. 서비스 `_resolve_period_range` 는 quarter 이미 지원 → `admin_dashboard.py` 1줄 추가(summary 통일). pytest CT-11 신규 + 기존 12 GREEN, 회귀 0. VIEW 후속(별 repo): detailsPeriod 다운그레이드 제거. **선행**: ✅ Sprint 82 (#80) v2.24.0 (close_type/task_id 필터).
> 이전: 2026-06-04 KST (✅ **Sprint 82 (#80) v2.24.0 완료 — auto-close-details close_type/task_id 필터**. VIEW 강제 종료 상세 패널이 union-50 client-filter 대신 force 전수 서버 조회. `/auto-close-details` 에 `close_type`(auto/manual/force, 미지정=union 하위호환) + `task_id`(마감 공정 t.task_id, 기존 trigger_task_id 와 별 컬럼) additive + **per_page cap 100→500**(운영 Q2 force 125건 이미 초과). BE only `dashboard_service.py`+`admin_dashboard.py`, 응답 스키마 0 변경. Codex R1 **DEPLOY_SAFE(M=0/A=4 전건 반영)** — A5 manager 미시작 force 제외 = Sprint 81 격리 정책 정합. pytest test_sprint82 12 GREEN(CT-01~10). summary·매트릭스·invariant·DB·migration 무변경 → 회귀 0. **VIEW 후속**(별 repo): force 셀 선택 시 close_type='force' 별 호출. **선행**: ✅ Sprint 81 (#79) v2.23.0 (강제 종료 협력사 2축 매트릭스).
> 이전: 2026-06-04 KST (✅ **Sprint 81 (#79) v2.23.0 완료 — 강제 종료 협력사 2축 매트릭스**. `/auto-close-summary` 응답 `force_closed` 에 additive 2키(`partner_task_matrix` 협력사×공정 + `partner_elapsed_matrix` 협력사×처리기간 5버킷). BE only `dashboard_service.py` (OPS 앱 변경 0, VIEW MissedCloseAnalysisPage 소비). 미시작=`started_at IS NULL` 최우선 분리(운영 force 129건 중 미시작 8건 elapsed=0 '1일내' 오분류 차단) + `_COMPANY_SQL` 재사용 + EXISTS(work_start_log) 제거. **Codex 3라운드 GO** — R2 M-1(multi-SELECT snapshot invariant 거짓 위반) → `build_auto_close_summary` REPEATABLE READ 단일 snapshot 고정 fix + finally isolation 복원/복원실패 폐기. pytest test_sprint81 23 GREEN + 회귀 test_sprint71 21p 1s. DB/migration 0. 설계 trail: AGENT_TEAM_LAUNCH.md § Sprint 81 §11. **VIEW 후속**(별 repo): force_closed 타입 optional 2키 + mock fallback + PartnerTaskMatrix 제목 props. **선행**: ✅ FIX-DURATION-MANUAL-PAUSE-RAW v2.22.0 (배포+백필+VIEW fix, 2026-06-03 섹션 참조). 본 세션: (1) v2.21.3 MECH 자재 드롭다운 줄바꿈 (배포 완료). (2) worker 2계정 삭제 (qq.com 4267 hr FK 63+1 정리 + gmail 4575). (3) #79 강제종료 협력사 매트릭스 설계 (AGENT_TEAM_LAUNCH.md Sprint 81, 미구현). (4) **FIX-DURATION 메인 — v2.22.0 배포 완료** — 수동 일시정지 미반영 + 휴게 이중차감 (duration이 raw로 덮어써져 pause 무시, 운영 210건). 설계 v1~v13 **Codex 13라운드 GO**. man-hour = per-worker interval-union(세션) − (수동 pause ∩ session_union), 세션 상한 = LEAST(완료기록, **본인 그날 attendance check_out**, [없으면]시작일 17:00 fallback, LEAD, close_at). 휴게 미차감, 3경로 통일(`complete_task_unified` 단일 UPDATE 원자), FLOOR. **구현**: `compute_task_manhour()`+`complete_task_unified()` (task_detail.py) + 정상완료(task_service.py L810) + ship/admin(shipment_service.py) + 자동마감 위임(auto_close_relay_task) + admin force-close `AUTO_CLOSED_` prefix 400 가드 + work.py LATERAL 세션 페어링(VIEW 9행→3행). pytest 신규 20(DP-01~45) + 회귀 95 GREEN. **백필(migration 058)** 1002건 net −353,617분 / 5월+ 만(4월 베타 제외) / 수동강제 51건 보존 / 단일워커 이상 0건. **남은 후속(별 BACKLOG)**: REF-TASK-DETAIL-CONN-INJECTION / worker_count 과소집계 / AUTOCLOSE-BACKSTOP(3일+ OPEN 244) / 대시보드 세션 표시 집계 / DW 휴게차감. 상세 ↓ 2026-06-03 섹션.
...
> 이전: v2.19.9 — Sprint 79 출하 예정 탭 [출고 완료] 버튼 옵션 B 채택. v2.19.0 ~ v2.19.9 영역 본 sprint 9 release 완료. 사용자 catch 누적 fix: SI 카드 navigate / admin_options 분리 / action 버튼 추가 / 권한 logic 5단계 catch / 카드 디자인 PI/QI 정합 / chip 카드 PI 위임 패턴 / 출하 예정 탭 직접 출고. pytest 18/18 PASS (회귀 + 신규 4 TC: GST name 매칭 / 협력사 제외 / 동명이인 / 비승인 제외). BE schema 변경 (int_list → string_list worker name CSV base). 추후 인큐베이션: FEAT-SHIPMENT-PROCESS-AUDIT-ANALYSIS-20260528 (SI_SHIPMENT.completed_at vs ship_plan_date 비교 → 매니저 패턴 / ETL 정확도 / 누락 비율 분석)
> 이전: v2.19.0 — Sprint 79 FEAT-SI-SHIPMENT-FLOW-3PHASE. 큰 sprint catch — SI 마무리공정 TabBar 3개 (미종료/출하 확정/출하 예정) + admin 메인 메뉴 분리 + 매일 07:30 KST 출하 미처리 자동 알림. BE 신규 2 (shipment_flow_service + admin_shipment_flow) + 수정 4 (admin/scheduler/notification/shipment_history). FE 신규 3 (si_finishing + inactive_users + pending_tasks_grouped) + 수정 2 (home + main). pytest 14/14 PASS / flutter build GREEN. Codex 라운드 1 (M=7/A=2/N=4) 모두 반영. 운영 검증 = D+1 (5-28) 07:30 KST scheduler 발송 catch.
...
- `OPS-65-DOC-DRIFT-20260515` — AXIS-VIEW OPS_API_REQUESTS.md #65 §9 Codex A 5건 (문서 drift) → AXIS-VIEW repo 측 정정 (P3)
