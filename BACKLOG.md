...
| `BUG-OPS-MANUAL-BUTTON-MOBILE-PWA-20260526` | OPS 홈 매뉴얼 버튼 모바일 PWA 영역 페이지 전환 안 됨 (PC 환경 정상). v2.18.31 anchor click + fallback patch 후에도 catch. 진단: `await getToken()` 후 `anchor.click()` 영역 user gesture context lost → 모바일 브라우저 popup 차단. 해결 안: (A) AuthService.tokenSync getter 영역 cache + sync click (~15분) / (B) `window.location.href` 직접 이동 (3분, PWA 떠남) / (C) A+B 조합 (~20분) | 🟠 MEDIUM | 출하 검증 완료 후 catch — 운영 차단 catch (모바일 매니저 영역 매뉴얼 접근 불가) |
