...
| SPRINT-63-BE-MECH-CHECKLIST-20260429 | Sprint 63-BE — MECH 체크리스트 도입 (양식 69항목 / 20그룹 / scope_rule + trigger_task_id 신규 / item_type INPUT 추가 / TM·ELEC·MECH completion 함수 public 통일, 2026-04-29 등록) | 🟢 OPEN (P2, 4.5일, BE 단독 시작 가능 / FE 별 Sprint) | **BE+FE 양쪽 — `backend/app/routes/checklist.py` MECH 분기 + `services/checklist_service.py` `check_mech_completion()` 신설 + `migrations/051_mech_checklist_extension.sql` + `051a_mech_checklist_seed.sql` (69 INSERT) + `frontend/lib/screens/checklist/mech_checklist_screen.dart` 신규 + AXIS-VIEW `ChecklistManagePage.tsx` BLUR 해제 + `ChecklistAddModal.tsx` MECH 토글 활성화**. 트리거: 2026-04-29 Twin파파 양식 공유 (Excel `현황판_260108_MFC Maker추가 260223.xlsm` 의 `공정진행현황-2행` 시트 기구 조립 검사 성적서 + `model_config.csv` tank_in_mech 컬럼 + `checklist_master.csv`/`checklist_record.csv` 기존 schema). **양식 분석 결과**: 69 항목 / 20 그룹 (3Way V/V / WASTE GAS / INLET / BURNER / REACTOR / GN₂ / LNG / O₂ / CDA / BCW / PCW-S / PCW-R / Exhaust / TANK / PU / 설비 상부 / 설비 전면부 / H/J / Quenching / 눈관리). **15개 1차+2차 (phase1_applicable=TRUE)**: INLET S/N(INPUT) 4개 + Speed Controller 방향(CHECK) 2개 + Speed Controller 수량(INPUT) 2개 + MFC Spec(SELECT) 4개 + Flow Sensor Spec(SELECT) 3개. **54개 2차만 (phase1_applicable=FALSE)**: 일반 CHECK 항목. **schema 변경 3건**: (1) `scope_rule VARCHAR(30) DEFAULT 'all'` 신규 — 'all' / 'tank_in_mech' (DRAGON/GALLANT/SWS, 13·14·19 그룹) / 'DRAGON' (INLET S/N 4개) (2) `trigger_task_id VARCHAR(50)` 신규 — UTIL_LINE_1 (Speed 4) / WASTE_GAS_LINE_2 (INLET 4) / UTIL_LINE_2 (MFC+FS 7) / NULL (54개 일반 → SELF_INSPECTION 일괄 2차) (3) `item_type` enum 'INPUT' 추가 (기존 CHECK/SELECT). **권한**: `resolve_managers_for_category(sn, 'MECH')` 호출로 mech_partner 매니저 자동 lookup (4-22 HOTFIX-ALERT-SCHEDULER-DELIVERY 표준 패턴 + v2.10.11 정리 후 그대로 활용, GST QI 무관). **alert_type 확장**: `CHECKLIST_MECH_READY` 신규. **TM/ELEC/MECH completion 함수 public 통일** (memory.md L364): `_check_tm_completion` → `check_tm_completion` rename / `check_elec_completion` 그대로 / `check_mech_completion` 신규. **product_code='COMMON' 단일** (Twin파파: 100+ product_code 분기는 미래 MTO 시점 대비 미리 구현, 현재 미사용). **MFC/Flow Sensor 옵션**: 임의값 임시 입력 (단순 문자열 배열, 단위 포함 복합 텍스트, 예: "MKS GE50A | 5 SLM | 0.5 MPa | 0.1-0.7 MPa"), VIEW 토글로 사후 수정. **INLET S/N**: 일단 텍스트 입력, OCR 후속 별건. **VIEW 측 이미 부분 준비**: `BLUR_CATEGORIES=['MECH']` (블러 처리 중) / `GROUP_POLICY.MECH={fixed:false}` (그룹 자유) / `TYPE_OPTIONS.MECH=['CHECK','INPUT']` (SELECT 추가 필요) / `effectiveProduct = selectedProduct` (모델별 의도, 미래 대비). **수정 파일 (AXIS-OPS 범위)**: BE 5 파일 (~250 LOC) + AXIS-OPS FE 1 신규 (~1000 LOC). **AXIS-VIEW FE 별 repo Sprint** = `AXIS-VIEW/DESIGN_FIX_SPRINT.md` Sprint 39 (~30 LOC, BE 배포 후 착수). **선행 의존성**: 없음, BE 단독 배포 가능. **선행 결정 trail (5단계)**: 1차 입력 타입 4종 → input_type 3종 통합 (CHECK/SELECT/INPUT) → 13/14/19 모델 = tank_in_mech=TRUE 3개 → product_code COMMON 단일 + scope_rule 신규 → 토스트 trigger_task_id 4종. **Codex 이관 권장**: ② 자동 체크리스트 인증·권한 로직 (4-22 동일 영역) + 5 파일 touch + ELEC 패턴 차용. **Post-deploy 1주 관찰**: 작업자 1차 입력률 + 2차 검수 완료율 + Sentry 새 ERROR 0건. **Rollback** git revert (1차 commit) — migration `051` 은 별도 down migration 필요 (column drop). 설계 상세: `AXIS-OPS/AGENT_TEAM_LAUNCH.md` Sprint 63-BE 섹션 + `mech_checklist_seed_extracted.csv` (양식 분석 결과) |
