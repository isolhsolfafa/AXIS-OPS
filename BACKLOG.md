...
| BUG-42 | 명판 소형 QR 접사 인식 실패 | 🔴 OPEN (2026-05-21 ROLLBACK 후 reopen, v2.18.12) | v2.18.5~v2.18.11 (11번 hotfix 시리즈) 시도 후 실기기 catch: 콘솔 로그는 후면 카메라 표시되는데 실제 화면은 셀카(전면) — html5-qrcode 라이브러리와 우리 변경 (`getUserMedia` 직접 호출 + `videoConstraints` + `{exact:'environment'}` 등) 의 stream 충돌 추정. v2.18.4 상태로 전체 롤백. 재시도 조건: ① 실제 iPhone Safari + Mac 원격 디버깅 환경 확보 ② 단일 minimal change 마다 즉시 실기기 검증 ③ 라이브러리 내부 동작 사전 분석 |
| BUG-42-TASK3-AUTO-ZOOM-DEFERRED | 명판 QR 자동 줌 (Task 2 부족 시 추가 검토) | 🟡 BACKLOG (LOW, Task 2 실기기 검증 후) | BUG-42 Codex 라운드 1 Q4 A — 자동 30% 줌 = 명판 도움 / 스티커 QR 과확대·근거리 초점 실패 역효과 가능 + UI 불변 제약(qr_scan_screen.dart 레이아웃 수정 불가) 충돌. Task 2 (v2.18.5) 실기기 검증 결과 명판 인식 여전히 부족 시 추가 검토. 옵션: getCapabilities().zoom 확인 후 30% 자동 적용 (silent) vs 줌 슬라이더 UI 추가 (프레임 영향) 비교. ~15 LOC |
...
| REFACTOR-QR-SCANNER-WEB-FUNCTION-SPLIT | qr_scanner_web.dart 함수 분할 (`startQrScanner` 158줄 → 60줄 이하) | 🟡 BACKLOG (LOW, 리팩토링 안전 규칙 7원칙 준수) | v2.18.5 BUG-42 Task 2 적용 후 파일 LOC 530줄 (🟡 경고 영역, 500+ 초과) + `startQrScanner()` 158줄 (🔴 함수 절대 한도 100줄 초과). CLAUDE.md L463-510 1단계 임계. Helper 분리 후 본체 LOC 변화 0 (호출 1줄 동일). 분할 단계: ① 카메라 시도 3개(env/user/cameraId) → 각각 별 함수 분리 ② 디코더 config 설정 → 별 함수. 리팩토링 안전 7원칙(CLAUDE.md L575) — Before/After pytest 동일 + `[REFACTOR]` prefix + DB migration 0 + git tag `pre-refactor-qr-scanner-web`. **v2.18.12 ROLLBACK 후 LOC 506 (🟡 경고 미충족), 우선순위 LOW 유지** |
| TEST-QR-LIB-CONSTRAINT-PREFLIGHT | 외부 카메라/QR 라이브러리 사용 사전 검증 체크리스트 | 🟡 BACKLOG (LOW, 재발 방지) | 등록 trail: BUG-42 v2.18.5~v2.18.11 (11번 hotfix) 실패 시리즈 + Codex 라운드 1 A-Q5 권고 (2026-05-21). 원인: html5-qrcode `config.videoConstraints` 키 자체가 `cameraIdOrConfig.facingMode` hint 를 무시한다는 라이브러리 내부 동작 미확인. 체크리스트 도입: ① 라이브러리 source 코드 영역 input parameter 우선순위 확인 의무 (특히 conflicting constraints) ② 최소 재현 HTML (qr-test.html 패턴) 영역 iOS/Android 실기기 사전 테스트 ③ `getRunningTrackSettings()` / `getCapabilities()` 로그 QA 증거 보존 ④ Codex 라운드 1 위임 시 "라이브러리 source 확인" 항목 명시 ⑤ 단일 minimal change 마다 실기기 검증 후 다음 단계. CLAUDE.md L98-103 (침묵 승인 거부) + L149 (Codex 독립) 영역 보완 |
