...
| BUG-RELAY-MODE-AUTO-REFRESH-MISSING-AND-COMPLETE-KEY-USELESS-20260514 | "내 작업만 종료" (relay_mode) PUT 응답 후 화면 자동 전환 X + 본인 종료 상태 "완료" 키 무의미 (TASK_ALREADY_COMPLETED 에러) | 🟠 **P1 OPEN (5-14 사용자 운영 catch — FIX-27 v2.15.7 새로고침 키 prod 적용 후에도 잔존)** | **FE only catch 2건 (BE 변경 0). 분석/수정 진행 X — 현재 증상 + 원인 추정만 기록**. **catch 1 — relay_mode 응답 후 화면 갱신 부분만 발동**: 사용자 5-14 실기기 catch (4컷 스크린샷 입증) — (1) 시작 버튼 누름 → 화면 자동 갱신 → "진행 중" 뱃지 + 일시정지/완료 버튼 표시 ✅ (정상) (2) "완료" 버튼 누름 → 다이얼로그 "다음 작업자가 이어서 작업하나요?" → "예, 내 작업만 종료" 선택 (relay) → BE PUT /work/complete (finalize=false) 응답 받음 (3) **화면 변동 X — 여전히 "진행 중" 뱃지 + 일시정지/완료 버튼 그대로 표시 ❌** (4) 수동 새로고침 (FIX-27 v2.15.7 AppBar 새로고침 키) 또는 QR 재태깅 시 → "내 종료 / 재참여 가능" 청록 뱃지 + 4개 버튼 ([내 작업 완료] [재시작 ↻] [일시정지] [완료]) 정상 표시. 사용자 의문: "내 작업 종료 누르면 화면 새로고침이 자동으로 되는 것처럼 보이는데 왜 4개 버튼 메뉴는 수동 새로고침 해야 보이는지 이해 안 됨". **catch 1 원인 추정 (cowork 분석 — 확정 X, claude code 검증 권고)**: relay_mode PUT 응답 = `{relay_mode: true, my_status: 'completed', task_finished: false, ...}` 부분 dict 영역 영역 FE state 일부 필드만 반영 (task.completed_at은 NULL 그대로 — relay 보장 v2.15.3 auto_finalize_blocked 영역) + my_status 필드 영역 영역 미갱신 → myWorkStatus getter (`myStatus ?? status`) → status='in_progress' fallback → "내 종료 / 재참여 가능" 분기 (`myWorkStatus=='completed' && task.status!='completed'`) 조건 불충족 → 진행 중 라벨 유지. **참고 — "공정 마감" (finalize=true) 정상 동작**: task.completed_at 설정 → status='completed' → 자연 갱신 → "완료" 라벨 표시 정상 (사용자 명시: "내작업완료, 공정종료에서 공정종료는 문제 없음"). **catch 2 — 본인 종료 상태 "완료" 키 무의미**: 단계 5 영역 영역 4개 버튼 중 "완료" 키 다시 누름 → BE 응답 `[TASK_ALREADY_COMPLETED] 이미 완료한 작업입니다.` 에러 (스크린샷 입증). 사용자 발화: "완료키는 다시 눌러도 이미 완료된 작업이라고 기능이 없는 키가 마찬가지". 본인 work_completion_log record 이미 존재 → BE upsert reject. **catch 2 원인 추정**: FE 카드 영역 본인 종료 상태 (`myWorkStatus=='completed' && task.status!='completed'`) 분기 영역 "완료" 버튼 조건부 렌더 제거 누락. "완료" 키 = finalize=true 다이얼로그 발동 권한이지만 본인 종료 상태 = 다른 worker 진행 가능성 영역 → 무의미한 권한 (위험 — 동료 작업 강제 마감 시나리오). **잠재 해결 방향 (cowork 추측 — 확정 X)**: (1) FE relay_mode PUT 응답 후 `ref.read(taskProvider.notifier).fetchTasks()` 호출 추가 영역 → 수동 새로고침과 동일 효과 (2) 본인 종료 상태 카드 영역 "완료" 키 조건부 렌더 제거 → [내 작업 완료] [재시작] [일시정지] 3개 버튼만 노출. **변경 추정**: FE only ~10 LoC. **선행 의존성**: v2.15.9 prod 안정화 + FIX-27 v2.15.7 새로고침 기능 prod 적용 완료 ✅. **회귀 위험 추정**: 0 (FE only, additive). **연관**: FIX-27-FE-TASK-CARD-MY-STATUS-AND-PULL-TO-REFRESH-20260514 (FIX-27 후속) + v2.15.3 auto_finalize_blocked (relay 보장 영역 + 본 catch 부산 영역) + v2.15.9 prod (다이얼로그 라벨 "공정 마감" 영역) + Hybrid sprint (FEAT-PROGRESS-MY-COMPLETION-HYBRID 영역 영역 진행률 실시간 갱신 영역 영역 동시 검토 가능). **trail**: 사용자 5-14 4컷 스크린샷 (시작 → 진행 중 → 다이얼로그 → 진행 중 유지 → 수동 새로고침 후 4개 버튼) + 5컷 스크린샷 (TASK_ALREADY_COMPLETED 에러 메시지). **claude code 분석 결과 catch 받은 후 진행 권고** — cowork 측 신뢰도 catch (사용자 5-14 발화) → 수정 진행 X, 분석/기록만. |
