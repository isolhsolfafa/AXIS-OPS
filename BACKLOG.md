...
| ~~`BUG-OPS-MANUAL-BUTTON-MOBILE-PWA-20260526`~~ ✅ CLOSED v2.18.34 | OPS 홈 매뉴얼 버튼 모바일 PWA 페이지 전환 fix. **옵션 (A) sync token getter 채택** (장기 시스템 관점 추천 — DRY + 책임 분리 + 표준 패턴 + 장기 확장성). `home_screen.dart _handleOpenManual()` 영역 `await authService.getToken()` (secure_storage async = user gesture context lost) → `ApiService().token` (런타임 메모리 캐시, sync getter L123 재사용) 변경 + token null SnackBar 안내. flutter build web GREEN. v2.18.34 release | ✅ CLOSED | 5-27 v2.18.34 |
