...
| DOC-SYNC-01 | OPS_API_REQUESTS.md / VIEW_FE_Request.md 잔여 PENDING 13건+ 실구현 상태 교차 검증 | 🟡 BACKLOG (관리 작업, 2026-04-17 등록) | 문서 싱크 지연으로 이미 구현됐으나 헤더가 PENDING으로 남은 항목 검증. 대상: OPS #22 / #40~#42 확인요청 / #46 (상세뷰 workers 누락 🔴) / #48 BUG (reactivate-task TMS mech_partner 🔴) / #51 (progress `sales_order`) / #52 (`_FIELD_LABELS` `finishing_plan_end` 🔴) / #54 (성적서 2건) / #56 2건 / #57 (성적서 ELEC Phase+DUAL) + VIEW FE-08 / FE-16. 방법: 각 항목의 코드 존재 여부 및 최근 Sprint 매칭으로 ✅ DONE 근거 확정 or 미진행 확정 후 헤더 정리. 시간 여유 시 Twin파파가 직접 진행 예정 |
