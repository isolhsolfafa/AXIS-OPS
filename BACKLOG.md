...
| OBSERV-PER-WORKER-POOL-RECOVERY-20260507 | gunicorn 다중 worker 환경에서 scheduler-owner 외 worker 의 풀 dead 자가 회복 메커니즘 부재 (2026-05-07 등록, 5-07 자가 회복 첫 실전 작동 시 발견) | 🟡 OPEN (P2, 0.5~1d) | **BE — `db_pool.py` warmup_pool 호출 분기 또는 신규 per-worker recovery hook**. 트리거: 2026-05-07 20:54 KST v2.11.6 자가 회복 메커니즘 첫 실전 작동 사후 측정 중 발견. **사고 사후 측정 데이터** (5-07 21:46 KST): Sentry 가 `re-initializing pool (pid=2)` 1회 출력 + Boot logs `[2]` + `[3]` 동시 boot (2 worker 정합) + pg_stat_activity OPS 5 conn 만 (단일 클러스터 205810~205814 sequential pid). **🔴 사각지대 본질**: ADR-025 의 'per-worker 카운터 의도' 가 fcntl lock + warmup cron 단일 worker 실행 구조와 결합 시 fragile. `scheduler_service.py` 의 warmup_pool job 은 fcntl lock 으로 단일 worker (scheduler owner) 만 실행 → 다른 worker 의 `_consecutive_zero_warmup` counter 는 영원히 0 (warmup_pool() 호출 자체가 안 됨) → 0/0 detection → init_pool() 자가 회복 chain 도달 불가능. **사용자 영향** (silent degradation): Worker B 의 HTTP 요청은 `_create_direct_conn()` fallback (+0.3~0.5s 지연) 로 처리되며 Sentry warning 으로만 기록 (FIX-DB-POOL-DIRECT-FALLBACK-LOG-LEVEL-20260428 의 logger.warning 강등 효과로 ERROR alert 미발생). p95 latency ↑ 가능, 사용자 체감은 burst 시점에만 가시화. **해결 옵션 3종**: (A) **warmup_pool 을 fcntl 우회** — 모든 worker 가 자기 풀에 대해 warmup 실행 (interval=5분 그대로). 단점: DB 부하 ×N. 변경: scheduler 등록 분기 제거 + 각 worker 자체 timer thread 도입. (B) **HTTP request path 자가 회복 hook** — getconn() 실패 N회 누적 시 worker 자체에서 init_pool() 트리거. 단점: race condition 위험 (동시 N요청). 변경: `db_pool.py` 의 retry 로직에 counter + threshold 추가. (C) **별도 per-worker watchdog thread** — 각 worker 가 자기만의 watchdog thread (5분 cycle) 로 풀 health check + 0/0 시 init_pool. 단점: thread 관리 복잡, gunicorn fork-safe 검증 필요. **권장**: B (가장 자연스럽게 자가 회복 path 흡수, 변경 최소). **A 는 DB 부하 ×N 으로 100명 이상 사용자 시점에 reconsider**. **선행 의존성 0**. **회귀 위험**: 옵션 B 채택 시 race condition 검증 필수 (concurrent getconn() 동시 fail 시 init_pool() 단일 호출 보장). **검증 시나리오 4개**: (1) 정상 path no-op (counter 증가 없음) (2) Worker A scheduler owner 풀 dead → 기존 자가 회복 작동 (3) Worker B HTTP-only 풀 dead + getconn 실패 → 신규 hook 으로 init_pool 호출 (4) 양쪽 worker 동시 풀 dead → 각각 독립 회복. **소요**: 옵션 B 기준 — 분석 1h + 구현 2h + pytest 4 TC 1h + Codex 교차검증 1h = 0.5일 / 옵션 C 기준 — 1일. **데이터 근거**: `DB_POOL_VERIFICATION_QUERIES_20260427.md` V4.4 결과란 (5-07 21:46 KST 측정, 1 worker active + 1 worker silent fail 가설) + handoff.md 5-07 헤더 블록. 최초 등록: 2026-05-07 21:46 KST (자가 회복 사후 분석 중). **연계 sprint**: ADR-025 보강 (memory.md per-worker gap 실측 trail) + V4.4 후속 진단 |
