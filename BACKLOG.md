...
| FIX-24 | OPS 미종료 작업 카드에 O/N(sales_order) 뱃지 표시 | 🔴 OPEN (Sprint 착수, 2026-04-17, v2.9.9 예정) | **OPS 전용** (VIEW는 이미 `salesOrder` 기준 분류되어 대상 아님). 대상 파일 2개: `frontend/lib/screens/admin/admin_options_screen.dart` `_buildPendingTaskCard()` L2467+ / `frontend/lib/screens/manager/manager_pending_tasks_screen.dart` `_buildTaskCard()` L346+. Twin파파 요청 "sn만 보이는데 on도 같이 보이면 좋을거 같아". BE `work.py` pending-tasks 응답에 `sales_order` 이미 포함(L1750/L1839) → **BE 변경 0줄, FE only**. 패턴: S/N Row에 `if (salesOrder.isNotEmpty) ...[Icon(receipt_long) + Text(salesOrder)]` conditional spread, 약 6줄×2파일. 회귀 위험 매우 낮음. HOTFIX-04(BE+VIEW)와 파일 겹침 없어 병행 가능. 설계 상세: `AGENT_TEAM_LAUNCH.md` FIX-24 섹션. 등록: 2026-04-17 / Sprint 착수: 2026-04-17 |
| TECH-REFACTOR-FMT-01 | VIEW 날짜 포맷 함수 중앙 유틸 통합 (format.ts) | 🟡 BACKLOG (중간, HOTFIX-04 배포 후 착수 권장) | **VIEW FE only — 기술 부채 정리**. 배경: Codex FE-19 교차 검증 중 `app/src/utils/format.ts`에 `formatDateTime` 미존재 + `ChecklistReportView.tsx` L25에 `formatDateTime(iso)` 로컬 함수 중복 정의 확인(현재 `format.ts`는 `maskName` 1개만). 유사 로컬 날짜 포맷 함수 2건 추가 존재(`QrManagementPage.tsx` L52 / `InactiveWorkersPage.tsx` L33). 목표: `utils/format.ts`에 `formatDate(iso)` + `formatDateTime(iso)` 중앙 유틸 2종 추가 → 3개 파일 로컬 함수 제거 후 `import { formatDateTime } from '@/utils/format'` 치환. 마이그레이션 前 가비지·중복 제거, 포맷 규칙 단일 진실점 확보(APS-Lite 정확성 원칙). 범위: VIEW FE 4파일(format.ts 1 + consumer 3). 회귀 위험 낮음(순수 함수, 단위 테스트 추가 권장). 등록: 2026-04-17 / 우선순위: 중간 |
