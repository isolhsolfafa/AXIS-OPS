...
| FIX-MODEL-CONFIG-O3-DESTRUCTOR-20260515 | 신규 모델 `O3 Destructor` model_config 미등록 — task_seed default fallback (has_docking/is_tms/tank_in_mech 전부 False) 적용 중 | 🟡 OPEN (P2, 사용자 측 O3 모델 사양 확인 필요) | **BE — `model_config` 테이블 INSERT (필요 시) 또는 무시 확정**. 트리거: 2026-05-15 05:06 Railway logs — `Model config not found for model='O3 Destructor'. Using defaults: has_docking=False, is_tms=False, tank_in_mech=False`. **현상**: 신규 제품 모델 `O3 Destructor` 가 `model_config` 테이블 영역 `O3` 접두사 미등록 → `task_seed.py` 매칭 실패 → default 값 적용. **동작 영향**: default fallback 으로 task 생성은 정상 (멈춤 없음). 단 — O3 Destructor 가 실제로 도킹/TMS/탱크 공정이 있는 모델이면 default(전부 False)가 틀려 task 구성이 의도와 다르게 생성됨. **판정 필요 (Twin파파)**: O3 Destructor 모델이 (1) 도킹 공정 있나 (has_docking) (2) TMS 별도 공정 있나 (is_tms) (3) 탱크가 MECH 포함인가 (tank_in_mech). 셋 다 없으면 default 정합 → 무시 / 하나라도 있으면 `model_config` INSERT 필요. **추정**: 사양 확인 후 INSERT 1건 (~10분) 또는 무시. **회귀 위험 0** (model_config 추가 = additive). **연관**: CLAUDE.md L808-815 model_config 모델별 분기 표. |
