...
### FEAT-CT-MONTHLY-TREND (월별 변화폭/학습곡선 endpoint) 🟡 LOW (2026-06-08 등록 — 데이터 축적 대기)
> **배경**: VIEW CT 분석 "월별 변화폭(학습곡선)" 컴포넌트(CtMonthlyVariabilityCard)가 mock. 월별 CT median 추세 by task/group endpoint 부재. (data-quality 에 교육전후 training_impact·마감유형 auto_close_trend 는 있으나 "월별 CT median 추세"는 아님)
> **범위**: 신규 `GET /api/ct/monthly-trend?task=&group_by=&basis=ct` (또는 task-stats 에 monthly 차원 추가) — 월별 median/iqr/var_ratio/n + 학습곡선(전월 대비 개선율). basis=ct(S-2)·트러스트 윈도우·게이트(월당 n<30 provisional) 재사용.
> **⏳ 2중 대기 조건**: ① **데이터 축적** — 신뢰 데이터 5월1일+ 라 현재 2개월(5월 full / 6월 부분)뿐 → 학습곡선 점 2개. **7~8월 누적되면 의미**(3~4개월+ 추세). ② endpoint 신규 구현. **현재는 "데이터 축적중" 배지가 정답** — 표본/추세 길이 부족이 본질, 조급 구현 무의미.
> **재개 조건**: 신뢰 윈도우 3개월+ (≥2026-08) 또는 사용자 요청.
...
> **결정**: ① **CT 계산 로직(S-1/S-2)은 정확** — 입력 데이터가 못 따라온 것 ② **product-level 총 CT/효율/lead/납기 = 시기상조**(해석 금지, 태깅 정착 후) ③ per-task CT 표준(#82)은 잡힌 task 중앙값 참고용(표본 작음).
> **현 단계 진짜 우선순위 = 태깅 정착 (운영·교육·UX)**:
>   - **운영/교육**: 작업자 실시간 태깅 정착(BAT 즉시완료율 61% → 교육), 제품당 완료율 22%·MECH 추적율 9% 끌어올리기.
...
|---|---|---|---|---|
| admin.py | 2,546 | ≤ 500 | 2,626 | 🟠 MEDIUM 격하 (2026-05-28 catch — Sprint 79 freeze 후 운영 안정화 시기 (5~6월) 신규 mutation 우선, REF-00-TEST 선행 후 7월+ 본격 진입) |
| task_service.py | 1,478 | ≤ 500 | 1,478 | 🟠 MEDIUM 격하 (admin.py 와 병행) |
| work.py | 1,343 | ≤ 500 | 1,343 | 🟠 MEDIUM 격하 (admin.py 와 병행) |
| checklist.py | 1,192 | ≤ 500 | 1,192 | 🟠 MEDIUM 격하 (admin.py 와 병행) |
| auth_service.py | 1,108 | ≤ 500 | 1,108 | 🟠 MEDIUM 격하 (admin.py 와 병행) |
...
| ID | 우선순위 | 내용 |
|---|---|---|
| `REBACKFILL-ACTIVE-TIME-ON-SETTINGS-CHANGE-20260607` | 🟡 LOW (조건부) | **Sprint 86 후속.** active_time_minutes 는 현재 admin_settings(근무/휴게 시간표) 균일 기준 산출(백필+신규 동일). 시간표가 운영상 고정이라 현재 무이슈. **향후 근무/휴게 시간표 변경 시** = 전체 active_time 재백필 필요(과거 row 도 새 기준). migration 059 백필 SQL 재실행(active_time_minutes 전체 재산출). admin 설정 변경 runbook 에 연결. |
| `REFACTOR-FORCE-COMPLETE-DURATION-SOURCE-20260607` | 🟢 INFO | **Sprint 86 R6 부수.** `admin.force_complete_task` 가 duration_source 미설정(NULL) — CT clean 에 포함됨. v2.28.0 에서 raw-elapsed→man-hour 정합(compute_task_work)으로 오염은 해소. duration_source 명시(NORMAL_COMPLETION 등)는 후속 정합(현재 NULL=clean 포함 정상). |
| `REFACTOR-CT-AUTO-CLOSE-LIKE-SHARED-CONST-20260605` | 🟡 LOW | **Sprint 85 Codex R3 A-1.** `statistics_service.py` auto_close_trend 의 `close_reason LIKE 'AUTO_CLOSED_BY_%%'` 하드코딩 = `dashboard_service._AUTO_LIKE` 와 현재 동치이나 분류 규칙 변경 시 drift 위험. 공통 상수 추출(`_AUTO_LIKE` import) 권고. 영향 0(현재 동치). |
...
| ~~`BUG-OPS-MANUAL-BUTTON-MOBILE-PWA-20260526`~~ ✅ CLOSED v2.18.35 (v2.18.34 fix → v2.18.35 HOTFIX) | OPS 홈 매뉴얼 버튼 모바일 PWA 페이지 전환 fix. **옵션 (A) sync token getter 채택 → v2.18.35 HOTFIX**: v2.18.34 영역 `ApiService().token` 직접 호출 = ApiService 영역 싱글톤 아님 → 새 instance + token null → SnackBar 만 뜸. v2.18.35 fix: `ref.read(apiServiceProvider).token` (Riverpod Provider singleton 영역 login setToken 호출된 동일 instance). 사용자 실기기 catch 정상 ✅ | ✅ CLOSED | 5-27 v2.18.35 |
| `REFACTOR-API-SERVICE-SINGLETON-20260527` | ✅ 🟢 COMPLETED (2026-05-28 grep sweep 결과 잠재 catch 0건 입증, closure) — v2.18.35 HOTFIX 영역 root cause 본질 catch — `api_service.dart` L6 ApiService 영역 싱글톤 아님 (매번 `ApiService()` new instance). Riverpod `apiServiceProvider` (auth_provider.dart L296) 영역 유일 정합 instance. **5-28 sweep 결과**: (1) `splash_screen.dart` L44 직접 호출 = `/health` public endpoint catch → token 무관 ✅ (2) service class 4건 (Task/Notice/Auth/Alert) fallback `?? ApiService()` 패턴 = 호출자 전수 (`task_provider/notice 4곳/auth_provider/alert_provider`) apiService 명시 전달 → fallback 실제 trigger 안 됨 ✅. 잠재 catch 0건 입증 — production catch risk 0. **advisory 잔존** (별 sprint): splash_screen.dart Provider 통일 + service class fallback 본격 refactor (미래 safety) | ✅ CLOSED | grep sweep 후 잠재 catch 0건 입증 closure |
| `FEAT-SPRINT77-LEAD-TIME-COMPLIANCE-20260527` | Sprint 77 (가칭) Lead Time 준수율 신규 KPI — 사용자 catch v2.18.33 ("계획대비 영역 자기 충족 catch / 모델별 standard lead time 영역 객관 base 의무"). 정의 안: `(actual_ship_date - LEAST(elec_start, mech_start)) <= 모델별 standard lead time` 영역 비율. 시작/끝/standard 영역 base 결정 의무. admin_settings 등록 + 협력사 평가지수 base + 매니저 KPI 시각화. 운영 actual_date 영역 정확 일자 입력 정책 catch 영역 선행 의무 (현재 ETL 영역 plan=actual 동일 입력 패턴 catch) | 🟢 INFO (인큐베이션) | 운영 6개월+ 영역 actual 정확 일자 누적 후 진입. Sprint 71 (자동 마감) / Sprint 72/73 (CT 분석 / M-M 신뢰성 / 협력사 평가) 영역 연계 |
| `FEAT-VIEW-PLAN-CHANGE-WARNING-WIDGET-20260527` | AXIS-VIEW Sprint 76 — 출하이력 페이지 `kpi.plan_change_warning` 위젯 렌더링 (OPS v2.18.33 BE 응답). 표시 안: KPI 카드 옆 또는 상단 hint badge `⚠️ {count}건 계획 변경됨 ({share_pct}%) — 자기 충족 catch 가능` / 5월 운영 89건 (60%) 표시 예정. null safe 의무 `kpi.plan_change_warning?.share_pct?.toFixed(1) ?? '−'` (v1.52.1 null safe 패턴 정합). VIEW repo 세션 진입 시 작업 | 🟠 MEDIUM | VIEW v1.52.x — OPS v2.18.33 prod 배포 완료 (즉시 가능) |
| `DOC-VIEW-OPS-API-REQUESTS-73-V32-20260527` | AXIS-VIEW `OPS_API_REQUESTS.md` #73 v3.2 갱신 — v2.18.33 신규 응답 명세 `kpi.plan_change_warning` 추가 (`count: int, share_pct: float, hint: string \| null`) + test DB 안전망 명시 (`etl.change_log` 없을 시 count=0 fallback). v3.1 (v2.18.30 model fix trail) 다음 entry로 추가. 코드 영향 0 (docs only) | 🟢 INFO | VIEW repo docs only — VIEW 세션 진입 시 작업 |
| `BACKLOG-SPRINT80-W53-PLACEHOLDER-20260529` | Sprint 80 Codex 라운드 2 A-8 — FE `si_finishing_screen` "미정" 칩이 W53/12-31 = 출하일 미정 placeholder 가정. 실제 ISO W53(연말) 출하가 생기면 정상 출하인데 "미정" 오표시. → BE `get_shipment_week_groups`가 미정 플래그(`is_undated` bool) 별도 필드로 내려줄 때만 FE 표시. 현재 운영 W53 115건 전부 미정이라 무해, 미래 방어 | 🟡 LOW | 실제 연말 출하 발생 또는 ETL 미정 표기 정책 확정 시 |
| `TEST-SPRINT80-WEEK-EDGE-20260529` | Sprint 80 Codex 라운드 2 A-5 — by_week 경계 TC 보강: ISO W53/연초 W01 경계 / SI_SHIPMENT.completed_at 기반 actual_date 제외(현재 actual_ship_date 컬럼만 검증) / cleanup-before-yield stale row 방지. 현재 WK-01~07 happy+경계 충분, 추가 방어 | 🟡 LOW | test infra, 별 세션 |
| `FEAT-SI-FINISHING-SHIPMENT-GROUP-BY-MODEL-20260529` | ✅ DONE (v2.21.0 Sprint 80 — ISO 주차 1차 + 모델 2차로 구현, model 단독 group 대체). OPS SI 마무리공정 > 출하예정/출하확정 탭 — 평면 200개 리스트를 모델(model)별 group by 카운트 카드로 1차 표시 + 탭 시 펼쳐서 세부 S/N 카드 표시 (평소엔 검색 위주). **파일**: `frontend/lib/screens/si/si_finishing_screen.dart` (688 LOC, Sprint 79). 탭3개 (미종료/출하확정/출하예정), API `/admin/shipment/by-status?status=&per_page=200&q=`. **현재**: `_buildShipmentTab` (L487~) → 검색 TextField + ListView.builder (L537, items 200개 평면) → `_buildShipmentCard` (L548~, 필드: serial_number/sales_order/model/customer/ship_plan_date/si_finishing_task_id/si_finishing_worker_id/si_finishing_worker_name/is_si_finishing_in_progress). **구현 힌트**: `_plannedShipments` model 키 groupBy → `Map<String,List>` / ExpansionTile 또는 expanded Set 펼침·접힘 / `_searchQuery` 있으면 group 스킵 평면 분기. **결정 사항 3건 (착수 전)**: ① 검색 중엔 group 무시하고 평면 결과? ② 모델 정렬 — 카운트 많은 순 / 출하예정일 빠른 순 / 모델명 순? ③ 적용 탭 — 출하예정(탭3)만? 출하확정(탭2)도? FE only (BE 영향 0) | 🟡 LOW | 시간 될 때 OPS 세션 이어서 — 결정 3건 확정 후 착수 |
| `FEAT-ACTIVE-ROLE-DYNAMIC-PERMISSION-20260529` | v2.20.15 사용자 요청 — GST 인원 공정(activeRole) 전환 시 권한도 동적 연동. 현재 ship-complete 권한은 정적 `workers.role == 'SI'` 기준 → role='QI'인데 SI 업무 전환한 인원은 출고완료 불가 / role='SI'인데 PI로 전환한 인원도 가능한 허점. 구현 범위: (1) JWT payload 에 activeRole 반영 (현재 기본 role만) (2) BE 권한 데코레이터 activeRole 기준 체크 (3) 공정 전환(changeActiveRole) 시 토큰 재발급 정책. 영향 endpoint: ship-complete + 향후 공정별 권한 전반 | 🟡 LOW | 운영 catch 후 또는 공정 전환 빈도 증가 시. 인증 체계 변경이라 Codex 교차검증 1순위 |
| `REFACTOR-COMPANY-NORMALIZE-20260529` | v2.20.14 Codex 라운드 1 A-Q2 — `workers.company` 'GST' 직접 문자열 비교 다수 (dashboard_service `_build_partner_filter` + jwt_auth.py:291 등). `company VARCHAR(50)` enum/check 제약 없음 → `normalize_company()` 공통 유틸 또는 DB check constraint 정규화. 운영 GST 매니저 13명 모두 정확히 'GST' 확인 → 즉시 위험 없음 | 🟡 LOW | 운영 catch 후 또는 company 입력 경로 추가 시 |
| `TEST-S71-GST-MANAGER-EDGE-20260529` | v2.20.14 Codex 라운드 1 A-Q6 — GST 매니저 권한 경계 TC 보강: details endpoint BAT/FNI row 확인 + `partner=FNI` 필터 확인 + company 공백/소문자 정규화 정책 결정 후 경계 TC. 현재 tc14b(summary 전체) + 기존 tc13/14/15 회귀 충분 | 🟡 LOW | 별 세션 시간 catch 시 (test infra only) |
| `POST-REVIEW-TC20-FIXTURE-MYSTERY-20260529` | Sprint 71 v2.20.1 pytest TC-20 (`test_tc20_unassigned_company_group_exists`) fixture infra 미스터리 — sn9 (mech_partner=NULL) + sn10 (PI category) seed → test `db_conn` 직접 query 검증 시 DB INSERT/commit 정상 (sn9/sn10 catch O) 그러나 Flask app endpoint 호출 시 응답 `auto.count = 4` (sn1~sn4 만, sn9/sn10 미반영). 가설: db_conn 영역 다른 connection / Flask app db_pool direct connection 영역 stale / timezone race condition 모두 검증 시 명확 매칭 X. **운영 영향 0** — 운영 actual data로 PI category 자동 마감 3건 (GBWS-7150/7110/7143) '(미지정)' 분류 정합 검증 완료. TC-19 invariant 정합 PASS로 fix 작동 입증. TC-20 `@pytest.mark.skip` 처리. 별 세션 영역 fixture infra 깊은 debug 의무 (db_pool direct vs db_conn isolation / Flask app DB session 캐시 / fixture commit timing) | 🟡 LOW | test infra only, 운영 영향 0, 별 세션 시간 catch 시 진입 |
| `FEAT-SHIPMENT-PROCESS-AUDIT-ANALYSIS-20260528` | Sprint 79 v2.19.9 옵션 B 채택 trail — 출하 처리 경로 audit 분석. `SI_SHIPMENT.completed_at vs ship_plan_date` 비교 base 매니저 catch 패턴 + ETL 정확도 + 누락 비율 분석. **분석 항목**: (1) 출하 확정 (예정대로) vs 출하 예정 (검색 후 처리) 비율 — `completed_at == ship_plan_date` (2) ETL 입력 정확도 — etl.change_log + completed_at 비교 (3) 매니저 누락 비율 — `completed_at > ship_plan_date` case. **연계**: Sprint 77 (Lead Time 준수율) + plan_change_warning (v2.18.33) 통합 catch. 운영 6개월+ 데이터 누적 후 진입 | 🟢 INFO (인큐베이션) | 운영 6개월+ actual_date 누적 후 / Sprint 77 + plan_change_warning 통합 |
| `FEAT-VIEW-CHECKLIST-PRODUCT-CODE-DROPDOWN-UX-20260527` | AXIS-VIEW 체크리스트 관리 페이지 영역 `product_code` dropdown UX catch. OPS v2.18.34 `/api/admin/product-codes` endpoint 영역 운영 base 200건 숫자 SKU (41000514 등) 반환 → 매니저 catch 어려움. **사용 위치**: `ChecklistManagePage.tsx` L47 `useProductCodes()` → L138 자식 컴포넌트 props (체크리스트 마스터 추가/편집 모달 영역 dropdown). **현재 운영**: `checklist_master.product_code = 'COMMON'` 단일 사용 (모든 모델 공통). **추후 운영**: product_code 모듈화 (50개 미만 catch) 후 모델별 체크리스트 분리 base. **해결 옵션**: (가) FE 검색 기능 추가 (combobox 패턴, 200건 → 입력 base 필터링) / (나) 페이지네이션 / (다) 그룹핑 (모델 prefix base) / (라) 본질 catch — product_code 모듈화 선행 후 50개 미만 후 fix. **추천 (라)** — 본질 catch 영역 product_code 모듈화 영역 운영팀 catch 의무. 즉시 fix X (인큐베이션) | 🟢 INFO (인큐베이션) | product_code 모듈화 (50개 미만 catch) 완료 후 진입 |
| `FEAT-SPRINT78-PRODUCTION-SCHEDULE-CROSS-MONTH-20260527` | ✅ 🟢 COMPLETED (2026-05-28 closure — AXIS-VIEW 측 생산일정 페이지 신규 컨셉 catch 완료, cross-month catch 흡수) — AXIS-VIEW Sprint 78 (가칭) — 생산일정 페이지 cross-month catch. 사용자 catch v2.18.33: 출하 캘린더 = 오늘 4건 (7060/7111/0731/0732) 정합 / 생산일정 5월 화면 = 2건 (7060/7111 pi=5월) — 0731/0732 (pi=3월) 영원히 누락. **Root cause**: factory.py `/monthly-detail` = single month base (`pi_start` 또는 `mech_start`) + FE `STAGE_FILTERS '출하'` 클릭 시 = 그 month 안에서만 `ship_plan_date=오늘` 카운트. 다른 month pi_start S/N (검사 시작은 다른 month 인데 같은 ship_plan_date) = 영원히 누락. **해결 옵션**: (가) VIEW FE — STAGE_FILTERS '출하' 클릭 시 출하이력 calendar API 재사용 (`/api/admin/shipment/summary` month 무관) — 책임 분리 원칙 정합, ~30분 / (나) OPS BE — `/monthly-detail` multi-month support (`months[]=2026-03,2026-05` 또는 `cross_month=true` 옵션) ~1h+VIEW FE / (다) VIEW FE — "오늘 기준 공정별 카운트" 별 위젯 (month 무관) ~1h. **본 catch trail**: v2.18.34 PI 필터 시도 → 사용자 결정 변경 ("오늘 출하 4건 정합") → ROLLBACK 완료. 출하 캘린더 정합 = 수정 X / 생산일정 catch = 본 sprint. 본질 catch — 책임 분리 원칙 (CLAUDE.md L545) VIEW FE 측 책임. **부수 검증 trail**: 5-26 ship_complete = ETL `actual_ship_date` 5건 (GBWS-6856/7099, GPWS-0785/0786, IVAS-275) 정상 카운트 ✅ / 5-27 = 0건 (정상) ✅ / OPS app SI_SHIPMENT task = 0건 (앱 측 출고 미사용, ETL 직접 입력 패턴) | 🟠 MEDIUM | VIEW repo 세션 진입 시 작업 — 옵션 (가) 추천 (책임 분리 원칙 정합 + OPS 측 변경 0 + 매니저 직관 즉시 해소) |
...
| FEAT-SHIPMENT-STATUS-PAGE | 출고 현황 페이지 | ✅ 🟢 COMPLETED (Sprint 79 v2.19.0 ~ v2.19.11 영역 흡수 catch, 2026-05-28 closure) | 등록 trail: 사용자 mockup (2026-05-22). Sprint 79 진행 중 `si_finishing_screen.dart` 3 tabs 추가로 mockup 의도 catch 흡수 — Tab 1 미종료 작업 / Tab 2 출하 확정 / Tab 3 출하 예정 + 권한 catch + 메일 발송 자동화. 사용자 5-28 검증 catch 완료. 잔존 catch 0 |
| FEAT-PENDING-TASK-MANAGEMENT-PAGE | 종료 누락 관리 페이지 | ✅ 🟢 COMPLETED (Sprint 79 v2.19.0 ~ v2.19.11 영역 흡수 catch, 2026-05-28 closure) | 등록 trail: 사용자 mockup (2026-05-22). Sprint 79 진행 중 `pending_tasks_grouped_screen.dart` 신규 화면 catch 흡수 — PI/QI/SI 분리 + admin/manager 권한 catch + 강제 종료 버튼. 기존 admin_options_screen + manager_pending_tasks_screen 영역 정리 catch 동반. 사용자 5-28 검증 catch 완료. 잔존 catch 0 |
...
| FEAT-SHIP-COMPLETE-AUTO-SEED-PRETRANSITION-20260610 | 출고처리(ship-complete) `SI_TASK_NOT_FOUND` 근본 원인 = "마무리공정 미시작"이 아니라 **task seed 자체 부재** (QR 전환 이전 운영 제품) — ship-complete 진입 시 task seed 자동 생성 fallback 검토 | 🟡 OPEN (P2 MEDIUM, 근본 원인 확정 2026-06-10, 사용자 결정 = 현 안내 메시지 유지 / 자동 seed 보류) | **BE — `backend/app/services/shipment_service.py` `ship_complete()` 의 `SI_TASK_NOT_FOUND` (L157-170) 분기에서 `initialize_product_tasks()` 자동 호출 fallback 검토**. **트리거 / 사고 trail**: 2026-06-10 09:11경 GST 소속 박승록(작업자) → GBWS-6971 / GBWS-6997 OPS 앱 출고처리 시 알람 `출고 완료 실패: SI 공정 task(SI_FINISHING/SI_SHIPMENT)를 찾을 수 없습니다`. **근본 원인 (사용자 catch — 내용 정리 중 발견, v2.32.1 message-only fix 당시 오진단 정정)**: 초기 가설 = "SI 마무리공정이 시작되지 않아서" (`SI_FINISHING_NOT_STARTED`, 400) → **오진단**. 실제 = 해당 제품에 **어떤 task seed도 존재한 적 없음** → `get_tasks_by_serial_number(sn, 'SI')` 빈 결과 → `SI_TASK_NOT_FOUND` (404) 경로. **원인 배경**: QR 이미지 배포 전환이 약 1개월 전 → 전환 이전부터 운영되던 제품(GBWS-6971/6997 등)은 기존 방식으로 흘러와서 SI 뿐 아니라 **모든 task 기록이 없음** (cf. 메모리 `project_ship_complete_pretransition.md` — pre-transition 제품 631건 task0). **admin QR 스캔으로는 성공한 이유 (핵심 발견)**: admin 이 QR SCAN 화면으로 진입 → `GET /api/app/product/{qr_doc_id}` → `initialize_product_tasks()` 자동 호출 (CLAUDE.md Task Seed 자동 트리거) → SI_FINISHING/SI_SHIPMENT seed 생성 → 그 다음 출고처리 → task 존재 → 성공 ✅. 즉 "마무리공정 시작"이 조건이 아니라 **"QR 스캔으로 seed 생성"이 숨은 선행 조건**이었음. **현재 상태 (정합)**: v2.32.1 `SI_TASK_NOT_FOUND` 메시지("이 제품은 앱에서 SI 공정이 생성되지 않았습니다... 먼저 QR 스캔으로 공정을 시작한 뒤 출고처리해 주세요")가 이미 정확히 이 행동(QR 스캔 → seed → 출고)을 안내 중 → **메시지 수정 불필요**. **검토 옵션**: ① A — ship-complete 가 `SI_TASK_NOT_FOUND` 시 내부에서 `initialize_product_tasks()` 자동 호출 후 재시도 (admin 이 QR 스캔 화면 안 거쳐도 1-step 출고) ② B — 현 안내 메시지 유지 (2-step: QR 스캔 → 출고), 추가 작업 0 (현재 코드 정합). **검토 필요 사항**: A 채택 시 Location QR 검증 / model_config 분기 (GAIA/DRAGON/iVAS) 우회 위험 — `FEAT-BATCH-START-WITHOUT-QR-TAG-20260511` 와 동일 논점 (자매 건). **사용자 결정 (2026-06-10)**: 현 시스템 정합 양호 + "마무리공정이 무조건 체크된다는 전제하에 문제 없음" → **B 유지 (자동 seed 보류)**, 추후 동일 문제 재발 시 A 재검토. **추정**: A ~2h (initialize_product_tasks 재사용 + 재시도 + Location QR 정책 결정 + pytest). **회귀 위험**: A 낮음 (additive, 실패 경로에만 fallback). **연관**: `FEAT-BATCH-START-WITHOUT-QR-TAG-20260511` (batch start 자매 건, 동일 `_ensure_tasks_exist()` 패턴) + v2.32.1 (CHANGELOG, message-only fix) + 메모리 `project_ship_complete_pretransition.md` + CLAUDE.md Task Seed 자동 초기화 트리거 (QR 스캔 시 `initialize_product_tasks()`) |
| FEAT-PARTNER-DISCIPLINE-PHASE2-20260610 | 협력사 규율 대시보드 Phase 2 — taggingRate/checkinNoTag/zeroTap/checklist 실측 + trend endpoint (Sprint 89-BE Phase 1 후속) | 🟡 **Phase 2a ✅ DONE (v2.34.0 + v2.35.0 보정, 2026-06-10)** — v2.34.0: taggingRate/zeroTap 실측 + trend (Codex R4 DEPLOY_SAFE). **v2.35.0 보정**: checkinNoTag(taggingRate 중복) 폐기→`checkoutMiss`(퇴근미체크율, reference_only 참고) + openTasks 월단위(started_at∈월) + 미종료 기준 통일(is_applicable+force_closed+TEST, 정식 /tasks/pending/grouped 정합) + 비협력사 제외(GST+SH). pytest 32/32. / **Phase 2b OPEN (P2, checklist 만 잔여, 계획·납기 Sprint 87-BE 선행)**. ⚠️ Phase 2b = `checklist` 실측 = A5 applicable resolver 추출(`check_mech_completion_all`/`check_elec_completion`/TM DUAL 로직 → 공용 resolver, DUAL L/R qr_doc_id·phase1_applicable·checker_role·scope_rule 정합, meta applicable/checked/excluded_by_*). **복잡도 高 → 별도 Codex 라운드**. summary 응답에서 `checklist`만 placeholder(available=false, phase='phase2') 유지 중. | **BE — `services/partner_discipline_service.py` + `routes/admin_discipline.py` 확장 (read-only, migration 0)**. Phase 1(v2.33.0, #87)에서 `summary` 응답의 4개 지표를 placeholder(`available:false, phase:'phase2'`)로 내려둠 → 실측 전환. **구현 항목**: ① `taggingRate` = GST 현장(`work_site='GST'`) 출근자 분모 ÷ work_start_log DISTINCT 태깅 worker (A2: HQ 본사 off-site 제외, meta `attendance_total`/`gst_onsite`/`hq_excluded`/`tagged` 분리). ② `checkinNoTag` = GST 현장 출근했으나 work_start_log 0건 worker (태깅 미정착 신호, label_note "근태 불량 아님"). ③ `zeroTap` = `duration_minutes=0 AND NOT statistics_service.is_instant_whitelisted(task_id)` (one-click 제외, A3 공개 helper 이미 v2.33.0 추가됨 — 재사용만). ④ `checklist` = applicable resolver 추출 (A5: 단순 record÷master 금지, `check_mech_completion_all`/`check_elec_completion`/TM DUAL applicable 로직을 공용 resolver 로 추출 → denominator. DUAL L/R qr_doc_id·phase1_applicable·checker_role·scope_rule 정합. meta `applicable_total`/`checked_total`/`excluded_by_role`/`excluded_by_scope`/`excluded_by_phase`. **복잡도 高 → 별도 Codex 라운드**). ⑤ `GET /api/admin/discipline/trend?months=N` 신규 endpoint (월별 추이, by_partner = admin/GST 전용). **공정성 정합 (Phase 1 계약 유지)**: envelope 동일 + group_avg peer-only k≥3 + 추적율<70% grade_eligible=false. **🚫 평가 제외 (2026-06-10 결정)**: checkout-miss(퇴근미체크)·utilization(가동률)·출근율 = 협력사 평가 지표 미포함 (출근 데이터는 taggingRate 분모로만). **선행 의존성**: Phase 1 v2.33.0 배포 완료 ✅. **추정**: taggingRate+checkinNoTag ~3h / zeroTap ~1h / checklist(resolver 추출 + Codex) ~5h / trend ~2h. **회귀 위험**: 낮음 (placeholder → 실측 교체, 응답 shape 불변 additive). **연계**: VIEW OPS_API_REQUESTS.md #87 (Phase 2 대기 항목) + Codex 라운드(checklist resolver는 별도). 설계: AGENT_TEAM_LAUNCH.md § Sprint 89-BE (A2/A3/A5/A9) |
...
| POST-REVIEW-HOTFIX-ALERT-SCHEDULER-DELIVERY-20260422 | HOTFIX-ALERT-SCHEDULER-DELIVERY 배포 사후 Codex 교차검토 (2026-04-22 등록, 2026-05-28 catch — L373 `POST-REVIEW-HOTFIX-DELIVERY-20260422` 와 중복 entry, 동일 catch 다른 ID) | ✅ 🟢 COMPLETED (2026-04-23 v2.10.2 FIX-CHECKLIST-DONE-DEDUPE-KEY M1 해소 + 2026-04-24 v2.10.3 FIX-ORPHAN-ON-FINAL-DELIVERY 숨은 4번째 경로 + 2026-04-28 v2.10.11 FIX-PROCESS-VALIDATOR-TMS-MAPPING 후속 catch 모두 완료) | CLAUDE.md § 🚨 긴급 HOTFIX 예외 조항 S2 적용 — Opus 단독 리뷰 후 배포 → 7일 이내 Codex 사후 검토 의무. 검토 범위: ① `_resolve_managers_for_category` 헬퍼의 정확성 ② 3곳 배치 dedupe 쿼리 index 활용 효율 (`idx_alert_logs_dedupe`) ③ for manager_id 루프의 N+1 query 여부 ④ Codex M1 (Item 2) 해결 완결성 ⑤ 배포 후 48h 관찰 데이터 (실제 delivery 재개 + legacy 69건 수렴 곡선). 예상 소요 30분 |
