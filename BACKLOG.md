...
| BUG-42 | 명판 소형 QR 접사 인식 실패 | ✅ Task 2 수정 완료 (2026-05-21, v2.18.5) | Task 2 (해상도 1920×1080 + focusMode continuous + 3곳 통일) 적용. Task 1 (BarcodeDetector) skip — Codex Q1 M catch: experimentalFeatures 가 잘못된 config 객체에 위치 + 2.3.8 default 이미 true = no-op. Task 3 (자동 줌) BACKLOG 이관 → 아래 `BUG-42-TASK3-AUTO-ZOOM-DEFERRED` 항목. 실기기 manual QA 결과에 따라 close vs reopen 결정 (사용자 위탁) |
