...
| `REFACTOR-API-SERVICE-SINGLETON-20260527` | v2.18.35 HOTFIX 영역 root cause 본질 catch — `api_service.dart` L6 ApiService 영역 싱글톤 아님 (매번 `ApiService()` new instance). Riverpod `apiServiceProvider` (auth_provider.dart L296) 영역 유일 정합 instance. **catch 의무**: 다른 곳 영역 `ApiService()` 직접 호출 grep sweep (예상 catch 영역 — 여러 화면 영역 동일 패턴 catch 가능성 catch) + Provider 영역 통일 또는 ApiService 자체 싱글톤 변환. 본 catch 영역 잠재 catch 영역 — 어떤 화면 영역 `ApiService()` 직접 호출 영역 token null catch 가능성 | 🟠 MEDIUM | grep sweep 후 결정 — 영향 범위 따라 즉시 fix 또는 별 sprint |
