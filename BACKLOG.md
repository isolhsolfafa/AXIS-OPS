...
| BUG-DURATION-VALIDATOR-API-FIELD | `/api/app/work/complete` 응답에 `duration_warnings` 필드 누락 (2026-04-22 등록) | 🟡 OPEN (P2, 독립 Sprint) | **Codex 라운드 2 합의 (2026-04-28, FIX-PROCESS-VALIDATOR-TMS-MAPPING 후속)**: Q1/Q2 모두 **A 라벨** — 본 fail 은 `duration_validator.py:70-93` REVERSE_COMPLETION 브랜치가 `"warnings"` 키로 반환 → `task_service.py:361-363,496-498` + `work.py:261-266` 가 `duration_warnings` 로 forward. 응답 키 생성 경로 4-22 부터 누락된 별건. v2.10.11 회귀 0건 + 본 BUG 별건 확정 + 별도 Sprint 처리. **수정 방향 (별도 Sprint)**: `task_service.complete_task()` 응답 dict 에 `duration_warnings` 키 forward 추가 (work.py L261-266 의 `result.get('duration_warnings', [])` 명시). 작업 소요 30~60분. 관련: CLAUDE.md ⑦ 강제 절차 (2026-04-22 추가) + v2.10.11 Codex 라운드 2 trail |
