...
| HOTFIX-04 | Orphan work_start_log "진행중" 오표시 (task 마감됐으나 개별 worker completion_log 없음) | 🟡 BACKLOG (설계 대기) | `/api/app/tasks/{sn}` worker_entry의 status가 `work_completion_log` 존재 여부만 판정 → task.completed_at 이미 세팅돼도 orphan worker는 `in_progress`로 반환. VIEW S/N 상세뷰 "진행중" + 강제종료 버튼 노출 → 클릭 시 BE가 `TASK_ALREADY_COMPLETED` 400 반환. 해결안 3가지: A(VIEW FE만 task.completed_at 분기), B(BE worker_entry에 `task_closed_at` + `is_orphan` 필드 추가 → VIEW FE orphan 렌더), C(orphan backfill 전용 API 신설). 등록: 2026-04-17 |
