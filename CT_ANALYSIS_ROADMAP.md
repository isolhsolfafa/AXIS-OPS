...
| 2026-06-04 | § 14.6~14.9 신규 — 탐험 2차. CT IQR 페이지 70~80% 연결 가능 + 신규 2차원 M/H·병렬효율 Amdahl 목업 검증. 두 차트 모두 raw elapsed 도킹대기 오염으로 막힘 → active-span(세션겹침) 키스톤. 판넬 PoC: **6/2 정식교육 후 데이터가 진짜 분기점**(연습기 24h → 교육후 8.4h). 결론 "새 코드보다 교육후 데이터 누적이 먼저, 4~8주 후 viable". 사용자 catch: "6/2 정식교육, 5월 중순+ 괜찮은 데이터 예상" |
...
### 14.6 CT 분석 페이지(목업) ↔ 현재 데이터 매핑 (2026-06-04 탐험 2차)

> 트리거: 사용자 "ct분석 페이지 현재 연결할 수 있는 값들이 있을까 / 2차원 M/H·병렬효율 확인". 목업 = AXIS-VIEW `docs/concepts/G-AXIS VIEW(CT분석).html` (Task별 IQR 군집도) + 신규 "2차원 M/H 분석(Phase 2)" 목업 스크린샷 (M/H 효율 산점도 + 병렬효율 Amdahl 곡선).

**CT IQR 군집도 페이지 (기존 목업)** — 70~80% 지금 연결 가능:

| 목업 위젯 | 데이터 | 가능? |
|---|---|---|
| Task별 IQR(Q1/중앙/Q3 박스) / 샘플수 / 신뢰도(sample 기반) / 평균 / 월·모델 필터 | `percentile_cont` + COUNT + plan.product_info.model JOIN | ✅ 지금 |
| 기존 평균시간 / 차이 | **소스 없음** (사용자 확인 — 모델 leadtime은 생산일정 별 트랙) | ❌ 컬럼 제거 |

- 결정 필요: Tukey on/off(평균 신뢰 위해 권장 on) / "기간합산" 기본값(모델 쪼개면 표본 부족) / 평균 컬럼은 outlier 끌림 주의(케비넷 중앙312 vs 평균751).
- 모델 데이터량: GAIA-I DUAL 423 / GAIA-I 354 / GAIA-P 115 ... GAIA 주력 (목업 DRAGON 예시는 소량).

**2차원 M/H 분석 페이지 (신규 Phase 2 목업)** — 명암 갈림:
- 정의: X=평균시간(h, elapsed) / Y=동시작업자 / 점크기=M/H(=평균시간×인원) / 색=카테고리. 병렬효율 = Amdahl 곡선(인원별 실제 단축 vs 이상 선형).

### 14.7 핵심 발견 — 두 차트 모두 "raw elapsed 오염"으로 막힘 (active-span 필요)

**M/H 산점도 (Tukey 후 실측, elapsed 기준)**:
```
배선 5.0h/IQR3.8/1.1명/M/H5.5  ← 목업 4.5h와 비슷, 단 IQR 목업1.1 vs 실제3.8
케비넷 10.4 / 마무리 8.5 / 자주검사 6.5 / PI 1.3 ... (12h 축 OK)
── 도킹 task 절반은 12h 축 초과 ──
IF_1 18.1 / 판넬 27.7 / WasteGas1 28.1 / Util LINE 1 80.6h(IQR126h) ⚠️
```
→ Tukey 정리해도 도킹 task(IF/판넬/WasteGas/Util)가 12h 축 밖. elapsed에 도킹 대기 포함.

**병렬효율 Amdahl (raw elapsed)**: 곡선이 **거꾸로** 감 — 판넬 1명 8.5h → 2명 24.9h (인원↑→시간↑). 2명짜리가 마침 도킹 걸친 건들이라 벽시계 폭발. + 다인 표본 부족(3명 2건).

→ **두 차트 원인 동일 = raw elapsed에 "안 일한 시간(도킹 대기)" 섞임.** 해결 키스톤 = **실작업 구간(active-span = 세션 겹침/합집합)** 계산. `work_start_log`/`work_completion_log` 세션 + v2.22.0 `compute_task_manhour` interval 로직 확장. 이 한 조각이 산점도 X축 + Amdahl + validator + CT IQR 전부 공급.

### 14.8 세션 겹침(active-span) PoC + 교육 타임라인 = 진짜 분기점

**판넬 active-span PoC (PG16 range_agg, 2026-06-04 실측)**:
```
기간              n    다인   man-hour  active실작업  elapsed  병렬도
전체 180일       120   55    25.1h     24.0h       24.4h    1.25
5/15+ (연습 후)   76   47    26.0h     24.0h       25.6h    1.34
6/2+ (정식교육)    9    2     8.4h      8.4h        8.4h    1.00
```

**발견 ① active-span은 만능 아님** — man-hour ≈ active ≈ elapsed (~24h). 세션 겹침은 **작업자 간 idle**만 제거, **한 작업자가 안 쉬고 길게 연 세션 내부 idle**은 못 뺌(도킹 너머로 안 닫으면 세션=24h 통째). 수학만으론 도킹 idle 완전 제거 불가.

**발견 ② 진짜 해결책 = 교육 (이미 6/2 시행)** ⭐ — **데이터 품질은 수학이 아니라 행동(교육)으로 풀림**:
- 5/15~6/1 (연습기): 판넬 24~26h (시작만 하고 방치 = 더러움)
- **6/2+ (정식교육 후): 판넬 8.4h** = 깨끗 (Amdahl "1인 정상 판넬 8.5h"와 일치)
- 사용자 예상("5월 중순+ 괜찮음, 6/2 정식교육") **데이터로 입증**. 단 아직 6/2+ 9건뿐(다인 2건).

**교육 타임라인 = 데이터 품질 분기점 (영구 기록)**:
```
~5/14    연습 전 / 더러움 (분석 제외 권장)
5/15~6/1 연습기 / 개선 중이나 방치 잔존 (24~26h)
6/2+     정식교육 후 / 깨끗 (8.4h, 실작업 반영) ← 신뢰 구간 시작점
```

### 14.9 최종 결론 — "새 코드보다 새 데이터(교육 후 누적)가 먼저"

- M/H 산점도 + 병렬효율 Amdahl 페이지는 **6/2+ 깨끗 데이터가 쌓이면 자연 viable** (다인 표본 충분해지는 데 ~4~8주 예상).
